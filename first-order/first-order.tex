...
substitution one constructor inward instead of copying the whole spine, which is efficient, but its
$S$ rule, $(\lambda a.\,E_1\,E_2)\,b \to ((\lambda a.\,E_1)\,b)\,((\lambda a.\,E_2)\,b)$, turns the
argument into suspensions that grow on every turn of a cycle, so the cycle never re-forms one interned
node and the fold never fires; reducing the suspensions first (call by value) re-forms the node but
...
growing $d$ climbs the $\bot$-completed approximation order, whose limit is the Levy-Longo reading. So
