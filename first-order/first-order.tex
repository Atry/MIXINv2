...
\section{Self-Compilation Across Island-Depth Stages}\label{app:benchmark}
The compiler compiles itself in specialized mode at a chosen island depth bound
(Appendix~\ref{app:compiler}), and the bound trades island size against generation cost. A closed
simply-typable sub-term shallower than the bound is certified as a strict call-by-value island, so a
larger bound compiles more of the compiler as native code and descends further into the deep
combinators to splice their sub-islands, at the price of typing deeper sub-terms while generating.
Table~\ref{tab:self-compilation} benchmarks three versions, each regenerating the self-compiled
compiler in a fresh process so its peak memory is its own: the interpreter-only version at depth 0
splices no island and runs the whole compiler in the interpreter, depth 8 is the small committed
artifact, and depth 128 the large one. The island counts and artifact sizes are deterministic; the
time and memory are a measured snapshot.

\begin{table}[h]
\centering
\input{generated/self-compilation-benchmark}
\caption{The self-hosted compiler regenerated at three island depth bounds: the call-by-value islands
spliced, and the wall-clock time, peak resident memory, and source size of one generation.}
\label{tab:self-compilation}
\end{table}

The compiler's maximal islands are small, at most 36 nodes and all shallower than depth 32, except for
two deep combinators of 264 and 345 nodes at depth 188 and 191, with no maximal island between. The
bounds in the table all stay below those two, so they differ only in which shallow islands they admit
and how deeply they splice sub-islands of the deep ones. Admitting the two deep combinators as whole
islands needs a bound past depth 188, where a single specialization pass peaks near twelve gigabytes,
because typing every closed sub-term to decide islands accumulates across the whole tree in one
interner session. The islands themselves are not the cost: the 345-node combinator compiles on its own
in about four seconds under half a gigabyte, so a staged generator that finds the islands and compiles
each independently reaches the full ceiling, the largest closed simply-typable sub-term, at the
per-island cost rather than the whole-program sum.
