...
requires a finite normal form); it stays interpreted in every engine. The islands a self-compiled engine
does carry, the simply-typable call-by-value leaves and the few small normalizing lazy sub-terms, are
negligible next to that interpreted recursion, which is why the cost is unchanged and the two regimes are
indistinguishable. The cost grows only with the target depth, because a deeper target types deeper
sub-terms: the maximal islands are small (at most 36 nodes, all shallower than depth 32) except for the
two deep combinators of 264 and 345 nodes at depth 188 and 191, which the committed depth-192 compiler
carries as islands. Making self-compilation pay would mean compiling the recursive hot path itself, which
no sound island can do; it would require redesigning the analysis to be structurally total, an island in
its own right, a change to the compiler rather than a compilation option.
