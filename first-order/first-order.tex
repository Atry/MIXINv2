...
\paragraph{Depth-bounded reduction and the one-layer reading.}
Bounding the reduction gives a family of structure maps between the raw term and the weak head normal
form. Reducing to depth $d$ fires at most $d$ contractions per application position and leaves a
still-unfired redex $(\lambda a.\,T)\,a$ as a guarded suspension, a let-stub the variant reads as a
value. At $d$ large enough this is the Levy-Longo weak head reading; at $d = 1$ it is the one-layer
reading, one contraction per position, a denotational variant distinct from both weak head
(Levy-Longo) and head (Boehm) normalization; at $d = 0$ it reads the term raw. Its denotation is the
same least fixpoint over the $\bot$-completed approximation order, and growing $d$ climbs that order.
Because firings are bounded, every head reduction halts regardless of rationality, while the reader
still folds the readout's cycles by interned-node identity.
