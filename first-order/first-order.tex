...
tree that process exposes. Two terms may thus share a behaviour yet reduce differently. This tree is the
  ...
  behavioural \emph{equivalence}, sharing that tree, is strictly coarser than the structural identity the
  tabling folds on, and in the $\lambda$-instance is B\"ohm-tree equality, hence undecidable, so it is not
  the relation the construction computes (Section~\ref{sec:intro}).} is the
...
wissmann-modular-partition-refinement} for any zippable functor on $\mathbf{Set}$, solves the dual
problem under a different identification. It is \emph{given} a finite coalgebra and computes its quotient
by \emph{behavioural equivalence} (the coarsest bisimulation, the kernel of the map into the final
coalgebra); we are given a $\lambda$-term, an infinite reduction system, and read off its reachable
rational graph. The two equivalences differ, and the distinction is the one this paper turns on. Our
tabling folds on \emph{structural identity} of states, decidable because the states are first-order graph
nodes (Section~\ref{sec:intro}); behavioural equivalence is strictly coarser, and in the $\lambda$-instance
it is B\"ohm-tree equality, which is undecidable, so it is not the relation we fold on. Behavioural
equivalence is also, for CoPaR's branching functors, an equivalence that abstracts the branching of a
state-based system; our behaviour is the first-order $\Sigma$-tree a state unfolds to, never the
extensional behaviour of a higher-order value.
