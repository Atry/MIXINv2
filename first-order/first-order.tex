...
a pointer test. The examples of
...
There is no separate notion of ``which states may be folded together'': tabling closes a cycle on
state identity, the ordinary key of a tabled computation whose core is $\outm$. Because states are
first-order (Definition~\ref{def:itcoalg}), that identity is decidable; the fold is then the standard
tabling re-entry. The implementation realises it by interning, so structurally equal states are one
object and identity is a pointer test. Structural identity is the only identity; there is no
congruence to choose.
\end{remark}
...
conversely. Either failure is genuine non-termination of the construction; there is no further
parameter to tune, since the identity on which states are compared is fixed
(Remark~\ref{rem:foldingtabling}).
\end{remark}
