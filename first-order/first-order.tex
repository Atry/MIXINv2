...
\begin{itemize}
  ...
  \item Least equals greatest (Theorem~\ref{thm:unique}): because $\outm$ exposes a constructor or
        $\bot$, the readout functional is guarded, hence a contraction with a \emph{unique} fixpoint,
...
One might hope the readout halts exactly when the least and greatest fixpoint coincide. They always
coincide (Theorem~\ref{thm:unique} gives a unique fixpoint for \emph{every} term), yet the readout does
...
readout diverges. Halting is governed by rationality, which is strictly stronger than uniqueness: the
...
\begin{enumerate}
  ...
  \item \emph{(Soundness.)} Whenever the readout halts it outputs a finite representation
        (Theorems~\ref{thm:termination}, \ref{thm:adequacy}), so it halts only on rational behaviours.
  \item \emph{(Attainment.)} The readout halts iff finitely many positions are reached up to the identity
        ...
        positions, the readout halts iff the behaviour is rational. By (1) no sound identity pushes the
...
is immediate: the term $M$ is a finite generator of $\behav(M)$ for every $M$, rational or not.

\emph{(2)} If the readout halts then by Theorem~\ref{thm:termination} the reached positions are finite
...
\emph{(3)} The readout descends through positions up to the folding identity and tables them, so it
...
no sound identity can do better.
\end{proof}
...
Rationality is undecidable (one can make $\behav(M)$ rational iff a given machine fails to halt), so the
terminating inputs cannot be pre-selected. The readout is thus a \emph{sound semi-decision procedure}
...
% =====================================================================
\section{The readout has a unique fixpoint: least equals greatest}\label{sec:unique}
% =====================================================================
The behaviour was introduced coinductively, as the unfolding of $\outm$ into $\nu\Beh$
(Definition~\ref{def:behreadout}). It is equally the order-theoretic least fixpoint of a readout
...
The \emph{readout functional} $\Psi\colon (X \to D) \to (X \to D)$ reads one $\Beh$-layer off the
...
\begin{lemma}[The readout is guarded]\label{lem:guarded}
...
fixpoint genuinely diverge. The readout instead lives over the $\bot$-completed tree domain $D$, where a
...
readout, \emph{not} of the discrete signature: completing with $\bot$ and reading guardedly is exactly
...
guarded readout is a contraction; the iteration plays no role. This order is intrinsic to the
...
Uniqueness is a property of the readout, after $\outm$ has exposed a constructor. The structure map
...
is the collapse of divergence to $\bot$; once a layer is exposed, the readout is guarded and least
equals greatest.
\end{remark}
