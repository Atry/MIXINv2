% =====================================================================
%  Computing the Fixed-Point over the Approximation Order
% ---------------------------------------------------------------------
...
\title{Computing the Fixed-Point over the Approximation Order}
...
The least fixpoint $\operatorname{lfp}(\outm)$ of the one-step observation $\outm$ that $f$ induces is
...
tree that process exposes. Two terms may thus share a behaviour yet reduce differently.} is the
...
Sections~\ref{sec:intro}, \ref{sec:bridge}, and \ref{sec:application} are reproduced as tests:
...
% =====================================================================
\section{The Rational Fragment as a Domain-Specific Language}\label{app:dsl}\label{app:examples}
...
material)]{~\cite{firstorderlambda}}. Two reusable shapes recur: a least fixpoint over a Church
...
(Proposition~\ref{prop:shisout}) enumerates the tree by unfolding and diverges on a cyclic term, whereas
$\operatorname{lfp}(\outm_{\mathrm h})$ yields the same tree, folded.
...
searches for that head normal form forever; $\operatorname{lfp}(\outm_{\mathrm h})$ reaches the same
...
normalization enumerates it by unfolding and never finishes; $\operatorname{lfp}(\outm_{\mathrm h})$ is
...
$\operatorname{lfp}(\outm_{\mathrm h})$ is the same tree, folded to the finite cyclic cell $c'$ with
$c' = \mathtt{cons}\,1\,c'$.
...
$\operatorname{lfp}(\outm_{\mathrm h})$ returns $\bot$ in finite time.
