...
\begin{abstract}
  Representing and transforming cyclic and infinite data in the pure $\lambda$-calculus normally requires adding a recursion construct, a \texttt{letrec}, a $\mu$-binder, or a built-in $Y$ combinator for graph reduction, because reduction of a term that refers to itself does not terminate, and an ordinary recursive function over such data unfolds its sharing forever instead of folding the cycle. We show no extension is needed. Cyclic data is the unique solution of a coalgebraic equation: over the $\bot$-completed approximation order on a finitary signature the behaviour functor is algebraically compact, so its initial algebra and final coalgebra coincide, the least and greatest fixpoints of a term's unfolding agree, and the equation has a unique solution, its denotation. We give two solvers that compute this solution. The first is an interpreter for the pure $\lambda$-calculus: its programs are pure $\lambda$-terms, and it tables the behaviour, the least fixpoint over the approximation order, identifying a state with its behaviour and tying a back edge when a state recurs; it halts precisely when the behaviour is rational, with finitely many distinct subtrees, and its operational semantics converges on strictly more terms than ordinary reduction without changing the denotation, returning a finite cyclic graph where the standard operational semantics unfolds a self-reference forever. The rational fragment is the home of finite-state, regular, and least-fixpoint-over-a-finite-lattice computation, so the pure $\lambda$-calculus serves as a domain-specific language for graph and coalgebra problems the interpreter solves: Datalog least Herbrand models, graph reachability and Andersen points-to analysis, minimax search, and dynamic programming, with memoization and transposition tables falling out of state identity. Halting on the rational behaviours is optimal: a non-rational behaviour has no finite representation, so no procedure that preserves the denotation halts on a larger class of inputs. Applied to the coalgebra that head reduction defines, the interpreter decides $\Omega$ as $\bot$ in finite time and represents $Y\,(\mathtt{cons}\;0)$, the cyclic stream of zeros, as a finite cyclic graph that an ordinary recursive function transforms, $\mathtt{map}\,\mathtt{succ}$ returning the cyclic stream of ones, with no construct added to the calculus. Compilation and graph optimization are themselves coalgebraic-equation-solving problems, so we use the first solver to bootstrap the second: a compiler from the $\lambda$-calculus to Python, itself a $\lambda$-term the interpreter solves, which compiles its own source in a self-hosting bootstrap.
\end{abstract}
...
accompanying implementation\anon[~(supplementary material)]{~\cite{firstorderlambda}} carries this
...
\[
  \mathtt{compile} = Z\,\big(\lambda\mathit{self}.\lambda d.\lambda q.\;
...
call-by-value regions out of an otherwise interpreted program. A bound variable is named by its
binder's syntax-tree path: the compiler emits the path as a list of naturals and a single decoder,
shared by every target, renders it as the Python identifier \texttt{v} with an underscore-separated
integer per path segment, so distinct binders get distinct names by construction. The listings are
generated from the compiler, not written by hand, and the compiler compiled whole to Python is
committed alongside as a generated module.
...
inference with an occurs check so the self-application $x\,x$ of $Y$, $Z$, and $\Omega$ fails, is
...
\[
  \mathtt{closed} = Z\,\big(\lambda\mathit{self}.\lambda d.\lambda q.\;
    q\,(\lambda i.\, i < d)\,
    (\lambda b.\,\mathit{self}\,(d{+}1)\,b)\,
  (\lambda f.\lambda a.\,\mathit{and}\,(\mathit{self}\,d\,f)\,(\mathit{self}\,d\,a))\big),
\]
recursing structurally with $Z$ and threading a Church-numeral binder depth: a variable is in scope
...
\paragraph{Bootstrap.}
The fixpoint is the strict combinator
$Z = \lambda f.\,(\lambda x.\,f\,(\lambda v.\,x\,x\,v))\,(\lambda x.\,f\,(\lambda v.\,x\,x\,v))$
rather than $Y$, so the compiled Python terminates in a strict host where compiled $Y$ would
diverge, while in the weak-head interpreter $Z$ is an ordinary fixpoint. The compiler then compiles
...
A code generator
$\mathtt{gen} = Z\,\big(\lambda\mathit{self}.\lambda s.\;
...
depth compiles more of the compiler as native call-by-value islands and leaves less to the interpreter.
A progressive bootstrap tests whether that makes the compiler faster: the compiler run on the
interpreter compiles the compiler into a small-island compiler; that compiler compiles the compiler into
a larger-island compiler; and so on, each stage compiled by the previous, climbing to the largest island
at depth 192, where the two deep combinators below become whole islands.
Table~\ref{tab:self-compilation} expands the climb into a matrix: every version of the compiler (the
interpreter, then the compiler self-compiled at each island depth) compiles the compiler's own source at
every target depth, each in a fresh process, reporting wall-clock time and peak resident memory.

\begin{table}[h]
\centering
\input{generated/self-compilation-benchmark}
\caption{The multi-stage climb as a matrix: rows are the compiler version doing the compiling (the
interpreter, then the compiler self-compiled at each island depth), columns the target island depth. The
upper block is wall-clock time in seconds, the lower block peak resident memory in gigabytes, each of one
fresh process compiling the compiler's own source. The linear bootstrap is the diagonal.}
\label{tab:self-compilation}
\end{table}

The matrix is flat down every column: for a fixed target depth the interpreter and every self-compiled
engine take the same time and the same memory, so the cost is set by the target depth alone, not by
which version of the compiler does the compiling. The reason is that the compiler's hot path is its type
checker, which decides for each closed sub-term whether it is a simply-typable island; that check is
driven by the same Y/Z recursion that makes the compiler untypable, so the recursion can never itself be
a call-by-value island and runs interpreted identically in every engine. The islands a self-compiled
engine carries are the typable leaf combinators off that hot path, so they leave the cost unchanged.
The cost grows only with the target depth, because a deeper target types deeper sub-terms: the
maximal islands are small (at most 36 nodes, all shallower than depth 32) except for the two deep
combinators of 264 and 345 nodes at depth 188 and 191, which the depth-192 column types and compiles.
Making self-compilation pay would mean compiling the recursive hot path, which needs the lazy
(call-by-name or call-by-need) islands that admit a normalizing but untypable recursion, a target the
analysis does not yet certify.
