...
\title{Solving Coalgebraic Equations}
...
Each example below pairs a source term with the Python the compiler emits for it, named so the two
read off against each other; the listings are generated from the compiler, not written by hand, and
the compiler compiled to itself is committed alongside as a generated module.
