...
the same $Z$ fixpoint that makes the compiler untypable. As a value that recursion has no normal form, so
...
so several of the analysis step bodies are simply typable and are compiled as islands. They do not change
the cost, though, because of the island-application boundary: a function island is read back to a
$\lambda$ node (the reify is nullary), and the interpreter then applies that node on every step, so the
native code runs once to produce the node and the per-application work stays interpreted. That, not the
size of the islands, is why the matrix is flat and the two regimes coincide. The cost grows only with the
target depth, because a deeper target types deeper sub-terms. Compiling part of the hot path is therefore
possible but not yet paying: it needs a positive-arity island that runs the step body natively on each
application (reflecting the node arguments to host values and reifying the result, falling back to
interpretation on a non-productive cycle), and whether that boundary nets a speedup against the small
step bodies is an empirical question. Compiling the recursion scheme itself, by contrast, no sound island
can do.
