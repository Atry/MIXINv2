...
% =====================================================================
\section{Compilation by Tabling: a Bootstrap Compiler to Python}\label{app:compiler}
A meta-tracing compiler runs a program, records the trace, and folds it at a cache keyed by the
program state, so a loop in the source becomes a loop in the compiled code. This is tabling the
behaviour: the trace cache is the runtime's fold on first-order state identity, here
\texttt{fixpoint\_cached\_property} with interning, and the rational graph is the compiled program
with its loops. The accompanying implementation\anon[~(supplementary
material)]{~\cite{firstorderlambda}} carries this out as a self-hosting compiler from the pure
$\lambda$-calculus to a Python AST, in four executable pieces, each reproduced as a test.
...
arithmetic applications execute in Python and compute what the source terms compute. The compiler
emits one of three target runtimes: a strict (eager) one, and two thunk-based ones differing only in
the thunk, a call-by-name lazy thunk and a fixpoint thunk memoised by \texttt{fixpoint\_cached\_property}
so a re-entrant force folds to $\bot$, the same least-fixpoint fold the interpreter performs. Under
either thunk runtime, the lazy semantics matching weak-head reduction, every normalizing term
computes its value, factorial and Fibonacci among them, and the prelude arithmetic is cross-checked
against the interpreter on both.
...
the runtime ties the back edge and the output is a cyclic quoted stream; a cycle-aware decoder turns
it into a Python generator with a \texttt{while} loop, which runs and yields the stream. On a finite
source the same $\mathtt{gen}$ yields a finite, loopless generator. The loop in the compiled program
is exactly the runtime's fold of the trace, with no sharing test in the $\lambda$-term.

\paragraph{Scope.}
The four pieces are executable and tested; what is left conceptual is the deeper meta-tracing of an
arbitrary computation through a $\lambda$ self-interpreter, of which the stream generator is the data
instance. Read as Futamura projections \citeTODO{futamura}, specialising an interpreter to a program
is a compiler and the trace cache is the tabling; the construction places that cache at the
runtime's first-order state identity, the same fold that decides $\Omega$ as $\bot$ and folds a
cyclic datum. Tracing just-in-time compilers \citeTODO{bolz-tracing} fold loops at a state-keyed
trace cache by the same principle.
