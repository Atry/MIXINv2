...
Table~\ref{tab:self-compilation} benchmarks three stages, each regenerating the self-compiled compiler
in a fresh process so its peak memory is its own; depth 8 is the small committed artifact and depth
128 the large one. The island counts and artifact sizes are deterministic; the time and memory are a
measured snapshot.
