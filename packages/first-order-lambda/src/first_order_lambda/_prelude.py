...
from first_order_lambda._dsl import Builder, app, build, lam, mu
...
# Example terms (built de Bruijn nodes).
IDENTITY_TERM: Node = build(IDENTITY)
KESTREL_TERM: Node = build(KESTREL)
OMEGA: Node = build(app(SELF_APPLY, SELF_APPLY))
FINITE_LIST: Node = build(cons(ZERO, SCOTT_NIL))  # cons 0 nil

# r = cons 0 r : a genuine cyclic stream via the Mu binder.
CYCLIC_ZEROS: Node = build(mu(lambda this: cons(ZERO, this)))

# Y (cons 0) : the acyclic copy with the same Berarducci tree as CYCLIC_ZEROS.
Y_CONS_ZERO: Node = build(app(Y, app(SCOTT_CONS, ZERO)))

# letrec x = x : an unproductive head cycle.
LOOP: Node = build(mu(lambda this: this))
