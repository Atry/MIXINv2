...
The console entry point ``first-order-lambda-benchmark`` (see ``pyproject.toml``) runs ``main``;
equivalently ``python -m first_order_lambda._benchmark``.
"""
...
# src/first_order_lambda/_benchmark.py -> repo root is four parents up.
...
def _run_cell(
    ...
    command = [sys.executable, "-m", "first_order_lambda._benchmark", "--cell", engine_spec, str(target), regime]
    if write_path is not None:
...
def _run_matrix() -> None:
    ...
    parts = ["% Generated by first_order_lambda._benchmark (first-order-lambda-benchmark). Do not edit.",
