...
def test_naive_query_over_cyclic_terminates() -> None:
    # The walk searches for nil and never finds it; the least fixpoint folds the cycle to a
    # definite value instead of diverging, so the naive external query terminates.
    assert render(berarducci(WALK_CYCLIC, operational)) == "⊥"
    assert render(berarducci(WALK_CYCLIC, least_model)) == "∅"
