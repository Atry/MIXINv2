...
that is untypable yet normalizing (a terminating ``Y`` recursion): compiled by the call-by-name
...
def test_lazy_island_rejects_terms_without_a_finite_normal_form() -> None:
    ...
    behaviour folds (the Y recursion never terminates structurally). The lazy-island read-back probes a
