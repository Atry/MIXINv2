...
spliced into the otherwise interpreted graph, so specialisation is local. The compiler's own
recursion is the ordinary fixpoint $Y$: its self-application is untypable and has no finite normal
form, so the analyses never send it to a compiled target and it stays interpreted, and the
