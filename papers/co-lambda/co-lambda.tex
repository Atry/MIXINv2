...
pure $\lambda$-calculus under which these very programs, unchanged and with their standard
meaning, run efficiently. Under them $Y\,(\mathtt{cons}\,0)$ evaluates to a finite cyclic graph,
a single cell pointing to itself, and the unchanged $\mathtt{map}\,\mathtt{succ}$ over it
terminates with the finite circle of ones, where ordinary evaluation runs forever; the term
$\mathtt{ed}$ runs in $O(mn)$ time and space, with no memoization written. The reason is one
idea. Read a program as an equation: it gives a value as one layer of data over sub-values that
are themselves programs, and to solve it is to compute that value. Such a system, a state
exposing one layer and naming its successor states, is a \emph{coalgebraic equation} (for the
stream of zeros a state is a stream cell, its layer the head $0$, its successor the tail), and
to evaluate a $\lambda$-term is to solve its equation. The evaluator we give, the
\emph{solver}, \emph{tables} the states it has seen, and when a state recurs it reuses the
table entry instead of expanding it again; states are interned, so a state assembled twice
from the same parts is one object and its recurrence is visible. A recurring cyclic state
closes a back edge into a finite graph, and a recurring subproblem is computed once, giving
the dynamic-programming table. What makes this computable is how states are compared: a state
stands for a program, but it is compared as a first-order graph node, the syntax it is built
from, never as the function it computes. Node identity is a decidable pointer check, while the
only identity available on functions, $\beta$-equality, is undecidable. The recognition is the solver's, never an operation a term performs, so the calculus
stays pure.\footnote{The interpreter compares nodes by identity as a step of evaluation, not as
an operation any $\lambda$-term can perform; no term gains reference equality, so referential
transparency is preserved.} And the solution is not checked against the meaning; it \emph{is}
the meaning: what a program denotes is, by definition, the tree its equation unfolds to, and
for the $\lambda$-calculus that tree is the textbook lazy meaning of a term
(Section~\ref{sec:bridge}), so computing the solution cannot change the semantics. We solve one equation by node identity, and
never compare two programs for equal meaning: the point the title turns on.
...
\begin{itemize}
  ...
  \item \emph{The instance.} Head normalisation of the pure $\lambda$-calculus is one such
    equation (Sections~\ref{sec:bridge}, \ref{sec:application}): an ordinary $\mathtt{map}$ folds a
    circular list into a circular list, and a naive walk terminates where evaluation diverges.
\end{itemize}

\paragraph{Supplementary material}
An executable interpreter accompanies the paper.\anon[ It is included as supplementary
material.]{ It is openly available at \url{https://github.com/Atry/MIXINv2}~\cite{colambda}.} It
realises the construction directly: states are interned so that identity is a pointer test, the
collapse of silent steps is a least fixpoint, and tabling folds on node identity. The examples
of this paper are reproduced as tests: $Y\,(\mathtt{cons}\,0)$ folds to a finite circular graph,
$\Omega$ (the classic looping term) and $Y\,(\lambda x.x)$ are decided as $\bot$, the naive walk
terminates over the circular stream where ordinary evaluation loops, and the $\mathtt{map}$ above
folds a circle into a circle (Sections~\ref{sec:bridge}, \ref{sec:application}).
