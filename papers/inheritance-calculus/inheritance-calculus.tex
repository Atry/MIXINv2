...
\begin{figure*}
  ...
  \caption{The calculi of this paper; nodes marked
    ...
    iterated further. Translated $\lambda$-terms never re-enter a
    lookup, so on them this approximation agrees with ordinary lazy
    evaluation; only cyclic records beyond the $\lambda$ fragment gain
    from further iteration.
    Continuing:
    fixpoint $\lambda$ the fixpoint $\lambda$-calculus, which is
    $\lambda$-IC's terms read back as a $\lambda$-calculus under the
    full fixpoint semantics;
