...
Let $\ell$ denote a label\footnote{
  A label serves as a property name.
}.
Let $k$ denote a nonnegative integer\footnote{
  The integer $k$ typically denotes a path length.
}.
...
This subsection establishes the correspondence between the
resulting mixin trees and
B\"ohm trees.%
\footnote{
...
\footnote{
  More concretely, the translation $\mathcal{T}$ turns
...
\begin{description}
  \item[Relational semantics]
    ...
    \footnote{
      Inheritance-calculus to relational algebra:
...
\footnote{
  Three successive implementations confirmed this; see the discussion above.
}
