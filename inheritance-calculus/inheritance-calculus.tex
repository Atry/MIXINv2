...
\end{lstlisting}
  ...
  \caption{A first look at inheritance-calculus (right), beside Python
    ...
    Python uses distinct language features for module, import, class,
    field, method, local, and object; inheritance-calculus uses one
    record of definitions composed by inheritance for all of them.
    ...
    number $(\pi+\tau) + (e+\pi)i$.
    Python brings these names in with \texttt{import}; for
    inheritance-calculus the same \texttt{import} is just inheritance, so
    \texttt{ComplexModule} inherits $\mathrm{BuiltIns}$, $\mathrm{Math}$, and
    $\mathrm{Operator}$, and reaches each imported definition through
    qualified $\this$, as in
    $\qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Float}$ and
    $\qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Add}$.
...
and $k$ a nonnegative integer giving the number of projections.
...
\begin{align*}
  ...
  && \text{(definition)} \\
  \mid\quad & (\qualifiedthis{\ell_{\mathrm{qualifier}}} \mid \dbi{n}).\;\ell_{\mathrm{projection},1}.\ell_{\mathrm{projection},2}\ldots\ell_{\mathrm{projection},k}
  && \text{(inheritance: qualifier + projections)} \\
  \mid\quad & \qualifiedthis{\ell_{\mathrm{qualifier}}} \mid \dbi{n}
  && \text{(inheritance: qualifier only)} \\
  \mid\quad & \ell_{\mathrm{projection},1}.\ell_{\mathrm{projection},2}\ldots\ell_{\mathrm{projection},k}
  && \text{(inheritance: projections only)}
\end{align*}
...
\begin{description}
  \item[Named form]
    $\qualifiedthis{\ell_{\mathrm{qualifier}}}.\ell_{\mathrm{projection},1}\ldots\ell_{\mathrm{projection},k}$,
  ...
  \item[Indexed form]
    $\dbi{n}.\ell_{\mathrm{projection},1}\ldots\ell_{\mathrm{projection},k}$,
...
$\ell_{\mathrm{projection},1}\ldots\ell_{\mathrm{projection},k}$ from it.
...
the shorthand
$\ell_{\mathrm{projection},1}.\ell_{\mathrm{projection},2}\ldots\ell_{\mathrm{projection},k}$ ($k \ge 1$)
resolves $\ell_{\mathrm{projection},1}$ against the innermost enclosing
...
The \emph{algebraic-datatype style} treats the AST as an inductive
type $\Node$ with a single record constructor:
\begin{equation}\label{eq:ast-adt}
  \Node \quad \cong \quad
  \bigl\langle\;
    \defs : \Label \rightharpoonup \Node,\;\;
    \refs : \mathcal{P}(\mathbb{N} \times \Label^{*})
  \;\bigr\rangle
\end{equation}
where $\Label \rightharpoonup \Node$ is a finite partial map from
labels to child nodes (one entry per definition $\ell \mapsto e$),
and $\refs$ is the set of inheritance sources, each a pair
$(n,\; \ell_{\mathrm{projection},*})$ of a de~Bruijn index $n$ and a list
of projections $\ell_{\mathrm{projection},*}$.
The whole program is a single global variable $\ASTroot : \Node$,
the AST of the top-level record.
...
de~Bruijn index pairs $(n,\; \ell_{\mathrm{projection},*})$.
...
$\ell_{\mathrm{projection},*} = (\ell_{\mathrm{projection},1}, \ldots, \ell_{\mathrm{projection},k})$.
A \emph{lexical reference} $\ell_{\mathrm{projection},1}.\ell_{\mathrm{projection},2}\ldots\ell_{\mathrm{projection},k}$ at path $p$
is resolved by finding the nearest prefix $p'$ of $p$
such that $\ell_{\mathrm{projection},1} \in \defines(p')$.
...
\begin{equation}\label{eq:bases}
  ...
  \left\{\; p_{\mathrm{target}} \;\left|\;
    \begin{aligned}
      ...
      &(n,\; \ell_{\mathrm{projection},*}) \in \inherits(p_{\mathrm{override}}), \\
      &p_{\mathrm{target}} \in \resolve(\init(p),\; p_{\mathrm{override}},\; n,\; \ell_{\mathrm{projection},*})
    \end{aligned}
  \right.\right\}
\end{equation}
...
a de~Bruijn index $n$,
and the projections $\ell_{\mathrm{projection},*}$.
...
then the downward projections $\ell_{\mathrm{projection},*}$ are appended:
\begin{multline}\label{eq:resolve}
  \resolve(p_{\mathrm{site}},\; p_{\mathrm{def}},\; n,\; \ell_{\mathrm{projection},*})
  = \\
  \bigl\{\;
...
The frontier $S$ is a \emph{set} so that $\this$ can resolve to several
targets at once, rather than rejecting the situation where more than one
inheritance route reaches the definition site, which Scala~3 and the NixOS
module system both reject
(Appendix~\ref{app:scala-multi-target});
Appendix~\ref{app:evaluation-trace} works through how such a multi-target
resolution is computed.
We defer the design-space rationale for this choice, alongside the other
semantic alternatives, to Section~\ref{sec:semantic-variants}.
...
\begin{description}
  ...
  \item[Function color blindness]
    ...
    runtimes\anon[~(supplementary material)]{~\cite{mixin2025}}, an instance of function color
    blindness~\cite{nystrom2015-function-color}.
...
Each $\inherits$ entry is a reference pair $(n,\, \ell_{\mathrm{projection},*})$,
where $n$ is the number of upward steps and $\ell_{\mathrm{projection},*}$ the
downward projections (Section~\ref{sec:definitions}).
