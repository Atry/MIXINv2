...
\end{lstlisting}
  ...
  \caption{A first look at inheritance-calculus (right), beside Python
    ...
    Qualified $\this$ reaches into the enclosing module to access its
    imported members.
    ...
    number $(\pi+\tau) + (e+\pi)i$.
    The inheritance-calculus side is slightly longer because each function
    application is encoded as a command pattern: a named record (the
      $\mathrm{\_sumReCall}$ and $\mathrm{\_sumImCall}$ locals, in
    ANF~\cite{flanagan1993-essence-compiling-continuations}) whose result
    is read back from its $\mathrm{result}$ field, rather than nested
  inline.}
