...
%% -- Calling context --
\newcommand{\callerargument}{\mathtt{callerargument}}
\newcommand{\callerscope}{\mathtt{callerscope}}
...
The direct formulation requires eleven semantic equations,
...
\begin{itemize}
  \item $\mathrm{Ref}(r)$ where
    $r \in \mathrm{ArgRef} \cup \mathrm{BindingRef}
    \cup \mathrm{InlineLambda}$:
    the tail is a reference.
    A $\lambda$-bound variable $\dbi{m}$ in tail position
    yields $\mathrm{Ref}(\mathrm{ArgRef}(m))$;
    an application-binding name $x$ in the current scope
    yields $\mathrm{Ref}(\mathrm{BindingRef}(0,\, x))$;
    an application-binding name $x$ that crosses
    $n$ intervening $\lambda$-scopes yields
    $\mathrm{Ref}(\mathrm{BindingRef}(n,\, x))$;
    an inline $\lambda$-literal $\lambda.\,M$ yields
    $\mathrm{Ref}(\mathrm{InlineLambda}(M))$.
...
$\mathrm{scope}(V,\, s)$ is called when a value
$V = \lambda.\,M'$ introduces a fresh scope at
path~$s$:
\[
...
a fresh scope path $s_\lambda$ is created with
...
The full set of calling contexts, and
the projections $\callerscope$ and $\callerargument$:
\begin{align}
  \contexts(s) &=
    \letcontexts(s) \;\cup\;
    \tailcontexts(s)
    \label{eq:lambda-contexts} \\[4pt]
  \callerscope(s) &=
    \bigl\{\; s' \;\big|\;
      (s',\; \_) \in \contexts(s)
    \;\bigr\}
    \label{eq:lambda-caller-scope} \\[4pt]
  \callerargument(s) &=
    \bigl\{\; s_a \;\big|\;
      (\_,\; s_a) \in \contexts(s)
    \;\bigr\}
    \label{eq:lambda-caller-argument}
\end{align}
...
$s_\lambda$ is the fresh scope path whose
$\mathtt{node}(s_\lambda)$ is defined by recursing
on~$M$ (see \emph{Definition of $\mathtt{node}$} above).
For $\mathrm{ArgRef}(n)$,
$\scoperesolution$ navigates up $n$ $\lambda$-scope levels,
yielding a set of scopes;
$\callerargument$ then extracts the argument passed to the
$\lambda$ at each such scope.
...
each scope in $\callerscope(s)$ and decrements~$n$.
Since $n$ is a nonnegative integer, the recursion terminates.
...
equations~(\ref{eq:lambda-result})--(\ref{eq:lambda-caller-argument})
...
\footnote{
  ...
  and $\callerscope$/$\callerargument$ specialize the
  provenance tracking in $\supers$.
}
(Section~\ref{sec:forward-translation}).
