...
\end{lstlisting}
  ...
  \caption{A first look at inheritance-calculus (right), beside Python
    ...
    Both add via Python's \texttt{operator.add} rather than a native
    operator, and both draw their constants from the host
    \texttt{math} module rather than writing literals: inheritance-calculus
    has no literals and borrows scalars and primitive operations from the
    host through a foreign-function interface (Section~\ref{sec:syntax}),
    so the figure computes $(\pi+\tau) + (e+\pi)i$ on both sides.}
...
phenomena manifested as off-label use of the adopted
design patterns.
Figure~\ref{fig:cheatsheet} is a first look at inheritance-calculus,
beside Python: the same complex-number class with an addition method,
written both ways.
The roles a working programmer reaches for, module,
import, class, field, method, local, and object, are the everyday
vocabulary of a general-purpose language, and the figure shows
inheritance-calculus expressing all of them.
Where Python spends a distinct language feature on
each, inheritance-calculus spends one: every role is the same record of
definitions composed by inheritance.
This is the sense in which
configuration alone reaches general-purpose programming, not by adding
functions back, but because the single immutable, first-order construct
already plays every part.
Where the NixOS module system unifies user
environments, macOS, and Kubernetes under one merge, inheritance-calculus
unifies the language's own constructs under the same one.
The remaining
axes of Table~\ref{tab:computational-models}, Turing-completeness and
extensibility, are discharged in Sections~\ref{sec:forward-translation}
and~\ref{sec:case-study}; here we only make the claim concrete.
