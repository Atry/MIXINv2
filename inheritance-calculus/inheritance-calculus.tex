...
not a higher-order fixed-point construction over function spaces.
...
\begin{itemize}
  \item We present inheritance-calculus, a minimal computational model for
    ...
    definition, inheritance) without functions.
  ...
  \item The semantics is \emph{observational}: there are no
    ...
    denotational, and computable; no function spaces, substitution, or
    separate operational semantics are needed.
  \item Self-referential records require no fixed-point combinator;
    the mixin tree \emph{is} the call stack.
    Self-reference resolves to \emph{multiple objects}, not one.
    Prior systems reject this
    situation, but since inheritance
  ...
  \item The translation is fully abstract with respect to
    B\"ohm tree equivalence under $\lambda$-definable contexts
    (Section~\ref{sec:bohm-tree}):
    the prior higher-order semantic frameworks of the lazy
    $\lambda$-calculus coincide with a sublanguage of
    inheritance-calculus's first-order semantics.
    As a corollary, the lazy $\lambda$-calculus has a
    first-order denotational computable semantics in its own right
    (Appendix~\ref{app:lambda-fixpoint}).
...
the lazy $\lambda$-calculus has a first-order
foundation: the translation $\mathcal{T}$ is a fully
abstract embedding into inheritance-calculus with respect
to B\"ohm tree equivalence~\cite{barendregt1984lambda},
using only recursive evaluation of set-comprehension
equations over paths and sets of paths, requiring no function
spaces, no substitution, no reduction rules, and no
separate operational semantics.
The prior higher-order semantic
frameworks~\cite{scott1972continuous,wadsworth1976semantics,abramsky2000full,hyland2000full,barendregt1984lambda}
are equivalent
to this first-order semantics.%
...
%%     - 判断结构形态并分支：需要 Visitor pattern（Section 4）。
%%       NatVisitor 对 Nat 做的事情与 F 对 SF-calculus 项做的事情
%%       完全对应：VisitZero/VisitSuccessor 对应 F 的 atom/compound
%%       分支。Visitor 可以通过 open extension 添加到已有 term 上，
%%       不修改原始定义。
%%   Visitor 在精神上就是 F：两者都是对 closed normal form 做
%%   constructor dispatch。区别在于 F 是不可扩展的 built-in primitive
%%   （硬编码了 S 和 F 两个 atom），而 Visitor 是可扩展的 encoding
%%   （新增 constructor 只需继承新的 Visit* 分支，即 Expression Problem
%%   immunity）。
%%
%% 精确差异：
...
%%   Visitor 是可扩展的（新增 constructor 只需继承新分支，
...
strategies.
None of these approaches is first-order: all three require
higher-order objects in their semantic domain.%
\footnote{Function spaces
  in denotational semantics, strategies
  in game semantics, and substitution
in operational semantics.}
None of them yields a fully abstract semantics of the lazy
$\lambda$-calculus without higher-order machinery:
Scott's $D_\infty$ is adequate
but not fully abstract, and game semantics is fully abstract
but requires arenas, strategies, and innocence conditions.
...
domain; none avoids capture-avoiding substitution; none
...
In each family the denotation of a term is a higher-order
...
\footnote{Herbrand models, abstract domains, and evaluation judgments, respectively.}
and none has been applied to a first-order domain of paths
...
DOT includes the $\lambda$-calculus as a sublanguage.
...
syntactic abstraction $A_F^{(2)}$ over~$\mathscr{L}_1$.
Each $\mathscr{L}_1$-construct~$G$ appearing in
$A_F^{(2)}$ has a fixed syntactic abstraction
$A_G^{(1)}$ over~$\mathscr{L}_0$.
Substituting $A_G^{(1)}$ for every~$G$ in $A_F^{(2)}$
yields a fixed syntactic abstraction
over~$\mathscr{L}_0$ for~$F$, satisfying~E4.
\end{proof}
...
By Full Abstraction for the lazy
$\lambda$-calculus~\cite{abramsky1990lazy}
