...
\begin{abstract}
  Just as the $\lambda$-calculus uses three primitives (abstraction, application, variable) as the foundation of functional programming, inheritance-calculus uses three primitives (record, definition, inheritance) as the foundation of declarative programming. By unifying classes, methods, objects, and properties under a single record abstraction, the calculus models inheritance simply as set union. Consequently, composition is inherently commutative, idempotent, and associative, structurally eliminating the multiple-inheritance linearization problem. Its semantics is first-order, denotational, and computable by tabling~\cite{tamaki1986old}. These three properties extend to the $\lambda$-calculus, since B\"ohm tree equivalence~\cite{barendregt1984lambda} is fully abstract for the first-iteration approximation of a sublanguage of inheritance-calculus. As a corollary, this establishes a convergence hierarchy among eager, lazy, and fixpoint $\lambda$-calculi sharing the same $\lambda$-syntax. Inheritance-calculus is distilled from MIXINv2, a practical implementation in which the same code acts as different function colors~\cite{nystrom2015color}; ordinary arithmetic yields the relational semantics of logic programming~\cite{vanemden1976semantics}; self-reference resolves to multiple objects; and programs are immune to the Expression Problem~\cite{wadler1998expression}. This makes inheritance-calculus strictly more expressive than the $\lambda$-calculus in both common sense and Felleisen's sense~\cite{felleisen1991expressive}.
\end{abstract}
...
\begin{itemize}
  ...
  \item The semantics is \emph{observational}: the observer drives computation by
    ...
    denotational, and computable by tabling~\cite{tamaki1986old}.
  ...
  \item The translation gives the lazy $\lambda$-calculus a
    ...
    As a corollary, this establishes a convergence hierarchy
...
operator~$T_P$~\cite{vanemden1976semantics}
...
(functions from self to complete object) and
\emph{wrappers} (functions that modify generators),
...
The denotational semantics of inheritance-calculus is
$\mathrm{lfp}(T_P)$
(Appendix~\ref{app:well-definedness}).
Tabled evaluation~\cite{tamaki1986old,chen1996tabled}
...
We now relate two points in this sequence to the
$\lambda$-calculus.
...
\begin{description}
  \item[$T_P\!\uparrow\!1$ corresponds to
    ...
    (Theorem~\ref{thm:adequacy}) shows that
    $\mathcal{T}(M)$ converges under $T_P\!\uparrow\!1$
    if and only if $M$ has a head normal form under
    lazy $\beta$-reduction.
    The Adequacy theorem
    (Theorem~\ref{thm:adequacy}) and the
    first-order semantics theorem
    (Theorem~\ref{thm:first-order-semantics})
    are proved at this level.

  \item[$\mathrm{lfp}(T_P)$ is strictly more
    defined than $\beta$-reduction.]
    $\mathrm{lfp}(T_P)$ may converge where
    $\beta$-reduction diverges.
    We call this sublanguage with $\mathrm{lfp}(T_P)$
    ...
    Under $T_P\!\uparrow\!1$,
    the cycle returns~$\bot$, matching the
    $\lambda$-calculus divergence.
    Under $\mathrm{lfp}(T_P)$, further iteration
...
This appendix presents a first-order semantics of the
$\lambda$-calculus in A-normal form (ANF),
which we call the \emph{fixpoint $\lambda$-calculus}:
it shares the same syntax as the lazy $\lambda$-calculus
but replaces $\beta$-reduction with $\mathrm{lfp}(T_P)$
evaluation.
...
B\"ohm tree (Theorem~\ref{thm:first-order-semantics}).
The proofs, given in
...
following pseudocode (Haskell-like syntax with
  \texttt{RebindableSyntax} semantics, where do-notation
desugars to a \texttt{Set}-based bind):
...
\footnote{Enabling the \texttt{RecursiveDo} extension does not help:
  ...
  fixed point at the \emph{container} level, i.e.\ finding a
