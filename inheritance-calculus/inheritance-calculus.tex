...
\newcommand{\bindings}{\mathtt{bindings}}
\newcommand{\tail}{\mathtt{tail}}
\newcommand{\result}{\mathtt{result}}
\newcommand{\visiblebindings}{\mathtt{visible\_bindings}}
%% -- Application-level functions --
\newcommand{\headref}{\mathtt{head\_ref}}
\newcommand{\argref}{\mathtt{arg\_ref}}
\newcommand{\head}{\mathtt{head}}
\newcommand{\argument}{\mathtt{argument}}
%% -- Calling context --
\newcommand{\callerargument}{\mathtt{caller\_arg}}
\newcommand{\callerscope}{\mathtt{caller\_scope}}
%% -- Shared resolution --
\newcommand{\scoperesolution}{\mathtt{scope\_resolve}}
\newcommand{\resolveref}{\mathtt{resolve\_ref}}
...
to consist of finite, well-founded structures, which serve as initial algebras in the
...
collection~\cite{dolstra2006-purely-functional-deployment} (over 100{,}000
...
systems~\cite{bracha1990-mixin-inheritance,barrett1996-c3-linearization} did not arise.
Inheritance is inherently commutative,
idempotent, and associative (Section~\ref{sec:mixin-trees}).
Self-reference naturally resolves to \emph{multiple targets} rather than
...
Problem~\cite{wadler1998-expression-problem} without any dedicated mechanism.
...
relational semantics of logic programming~\cite{vanemden1976-predicate-logic-semantics}
...
$\lambda$-calculus into inheritance-calculus requires six local,
...
\footnote{MIXINv2 separates symbol resolution (which resolves
  lexical references and qualified-this references) from runtime
...
Let $\ell$ denote a label\footnote{A label serves as a property name.}.
Let $k$ denote a nonnegative integer\footnote{The integer $k$ typically denotes a path length.}.
...
\begin{align*}
  ...
  && \text{(inheritance source, } k \ge 0 \text{ with qualifier,} \\
  &&& k \ge 1 \text{ without)}
\end{align*}
...
\footnote{We use \emph{record} for the syntactic construct, \emph{scope}
...
This provenance is needed by qualified this resolution, that is, the $\this$ function defined below,
...
evaluation never returns a wrong answer and analyses
termination.
...
naming intermediate results\footnote{This can optionally include naming values as well},
...
$\mathbf{let}$-bindings in the adequacy proof below.
...
B\"ohm tree~\cite{barendregt1984-lambda-calculus} of the original $\lambda$-term.
...
is fully abstract for the first-iteration approximation
$T_P\!\uparrow\!1$ of the equations.%
\footnote{More concretely, the translation $\mathcal{T}$ turns
  each $\lambda$-term into a mixin whose semantics is given by
  the equations of Section~\ref{sec:mixin-trees}: function
  application becomes record inheritance, and convergence
  reduces to path queries.
  Appendix~\ref{app:lambda-fixpoint} reformulates this
  semantics directly on ANF, without passing through
inheritance-calculus.}
The full proof is given in Appendix~\ref{app:bohm-tree-proofs}.
...
\footnote{This simple cycle could also be resolved by a
...
\footnote{We call the sublanguage of inheritance-calculus corresponding
...
\footnote{The examples in this section are rendered in
...
we define a factory containing the abstract product type and two
...
Multi-target qualified this (equation~\ref{eq:this}) is essential.
...
\footnote{Appendix~\ref{app:datalog-encoding} gives the systematic
...
\begin{description}
  ...
  \item[Function color blindness]
    ...
    \footnote{Function color~\cite{nystrom2015-function-color} arises when
...
{\small
  \begin{center}
    \begin{tabular}{lp{0.38\columnwidth}p{0.33\columnwidth}}
      ...
      type class dispatch,
      \texttt{import}, \ldots \\[3pt]
      Scala\footnote{Scala is a multi-paradigm language.}
...
\footnote{The $I$ combinator is redundant since $I = SKK$.}.
...
removing structure from an existing model:
...
operational\footnote{Typically SLD resolution.}, model-theoretic\footnote{Based on least Herbrand models.}, and fixed-point\footnote{Namely, the least fixed point of the immediate consequence operator~$T_P$ on the powerset of ground atoms.}.
...
\footnote{These are functions from self to complete object.} and
\emph{wrappers}\footnote{Wrappers are functions that modify generators.},
...
this as the unique survivor.
The resulting calculus is a minimal computational
model for declarative programming built from three constructs:
record, definition, and inheritance.
The mixin served as the single abstraction unifying module,
class, object, method, field, and local binding;
deep merge of mixin trees subsumed method-level overriding
and made inheritance commutative, idempotent, and associative,
so the linearization problem of multiple inheritance did not arise.

The observational semantics drove computation by querying paths
in a lazily constructed tree.
Six semantic equations, all first-order,
denotational, and computable by
tabling~\cite{tamaki1986-tabled-resolution}, sufficed to
define the semantics.
The mixin tree was the call stack;
self-reference resolved to multiple targets.

The $\lambda$-calculus macro-expressed into
inheritance-calculus in six local rules in
Section~\ref{sec:translation}.
The B\"ohm tree correspondence of
Section~\ref{sec:bohm-tree} gave the lazy
$\lambda$-calculus a first-order semantics:
...
Unlike those prior frameworks, the semantic domain
contained no function space: only paths and sets of paths.
...
lazy $\lambda$-calculus in Felleisen's
sense~\cite{felleisen1991-expressive-power}, as
Section~\ref{sec:asymmetry} demonstrated.

Three emergent phenomena arose from the three primitives.
...
programming; Appendix~\ref{app:datalog-encoding} gave a
systematic encoding of Datalog.
...
\begin{lemma}[Substitution]\label{lem:substitution}
  ...
  \footnote{The proof uses the Single-Path Lemma
...
This definition observes only convergence\footnote{Convergence provides exactly a single bit of information.}, but
...
The B\"ohm tree has exactly two node types---\emph{scope}
and \emph{application}---with separate path types and
separate first-order functions for each.
All dynamic functions are single-valued:
the Single-Path Lemma (Lemma~\ref{lem:single-path})
guarantees that no multi-inheritance arises for translated
$\lambda$-terms.
...
These have distinct path types $\mathcal{S}$ and
$\mathcal{A}$ and distinct first-order functions.

A \textbf{scope node} (path type~$\mathcal{S}$)
represents a body: a chain of let-bindings ending
in a tail expression.
A scope may or may not be a $\lambda$-abstraction
(recorded by $\mathrm{is\_abstraction}$).
...
$\mathbf{let}$.
In the AST, $\headref$ and $\argref$ are
\emph{references} (de~Bruijn index plus projection, or
an inline $\lambda$-literal);
in the evaluated B\"ohm tree, $\head$ and $\argument$
lazily resolve these references to scope nodes.
Navigation from an application to a scope:
$\head(a) \in \mathcal{S}$
and $\argument(a) \in \mathcal{S}$.
...
A reference encodes how to reach a target scope node:
\begin{itemize}
  \item $(n,\, \mathrm{argument})$: go up $n$
    $\lambda$-scope levels, access the argument value.
  \item $(n,\, x)$: go up $n$ $\lambda$-scope levels,
    access binding~$x$.
  \item Inline $\lambda$: immediately resolves to a
...
variable has index $1$ and the inner-$\lambda$ variable has
index $0$.

\paragraph{AST}
An ANF term is parsed into an AST that provides
separate static functions for each node type.
All are pure data extracted from the parsed term.

\subparagraph{Scope-level AST functions.}
For a scope node $s \in \mathcal{S}$:
\begin{itemize}
  \item $\bindings(s)$: the set of \emph{application}
    let-binding names
    $\{x_1, \ldots, x_k\}$ in this scope.
  \item $\mathrm{is\_abstraction}(s)$: whether $s$ is
    a $\lambda$-abstraction (that is, has a binder slot).
  \item $\tail(s)$: the tail expression, which is one of:
    \begin{itemize}
      \item $\mathrm{Ref}(n,\, \ell)$: a reference.
        A $\lambda$-bound variable $\dbi{m}$ in tail position
        yields $\mathrm{Ref}(m,\, \mathrm{argument})$;
        an application-binding name $x$ in the current scope
        yields $\mathrm{Ref}(0,\, x)$;
        an application-binding name $x$ that crosses
        $n$ intervening $\lambda$-scopes yields
        $\mathrm{Ref}(n,\, x)$.
        (Value-bound names are resolved by inlining
        and do not appear as references.)
      \item $\mathrm{TailCall}(r_1, r_2)$: a tail-position
        application, where $r_1$ and $r_2$ are references
        encoding $V_1$ and~$V_2$.
      \item $\mathrm{Lambda}(M)$: an inline $\lambda$-literal.
    \end{itemize}
\end{itemize}

\smallskip\noindent
Each ANF construct determines the scope-level AST as follows.
An abstraction $\lambda.\, M$ creates a scope node with
$\mathrm{is\_abstraction} = \mathrm{true}$
whose $\bindings$ and $\tail$ are determined by~$M$.
An application let-binding
$\mathbf{let}\; x = V_1\; V_2 \;\mathbf{in}\; M$
adds $x$ to $\bindings$; the continuation $M$ determines
further bindings and the eventual tail.
A value let-binding $\mathbf{let}\; x = V \;\mathbf{in}\; M$
does not add~$x$ to $\bindings$; instead, every reference
to~$x$ in~$M$ is replaced by the encoding of~$V$
(with appropriate de~Bruijn index adjustments for any
intervening $\lambda$-abstractions).
A tail call $V_1\; V_2$ sets
$\tail = \mathrm{TailCall}(r_1, r_2)$ where $r_1, r_2$
are the references encoding $V_1, V_2$.
A tail value $V$ sets $\tail = \mathrm{Ref}(n, \ell)$
or $\mathrm{Lambda}(M)$ according to the form of~$V$.

\subparagraph{Application-level AST functions.}
For an application binding
$\mathbf{let}\; x = V_1\; V_2$, the node
$s \snoc x$ is an application node ($a \in \mathcal{A}$)
with:
\begin{itemize}
  \item $\headref(s \snoc x)$: encodes~$V_1$
    (the function being applied).
  \item $\argref(s \snoc x)$: encodes~$V_2$
    (the argument).
\end{itemize}
A value $V$ is encoded as a reference:
$\dbi{m} \mapsto (m,\, \mathrm{argument})$;\;
let-bound name $y$ ($n$ $\lambda$-scopes away)
$\mapsto (n,\, y)$;\;
$\lambda.\,M' \mapsto \mathrm{Lambda}(M')$.

\paragraph{Dynamic scope-level functions}
The following functions are computed via the fixpoint
equations.

\subparagraph{Result.}
$\result(s)$ performs one step of head reduction,
mapping a scope to its successor scope:
\begin{equation}\label{eq:lambda-result}
  \result(s) =
  \begin{cases}
    \resolveref(r,\; s)
    & \text{if } \tail(s) = \mathrm{Ref}(r) \\[4pt]
    \resolveref(r_1,\; s)
    & \text{if } \tail(s) = \mathrm{TailCall}(r_1, r_2) \\[4pt]
    s_{\lambda}
    & \text{if } \tail(s) = \mathrm{Lambda}(M)
    \text{ (inline scope $s_{\lambda}$)}
  \end{cases}
\end{equation}
The three cases are exhaustive: every well-formed ANF scope
has a $\tail$ that is one of $\mathrm{Ref}$,
$\mathrm{TailCall}$, or $\mathrm{Lambda}$.
In the $\mathrm{Ref}$ case, the tail is a reference
$r = (n, \ell)$; $\resolveref$ resolves it to a scope.
When $\ell = x$ (an application-binding name in some
enclosing scope), this follows the binding's head to a scope.
When $\ell = \mathrm{argument}$ (a $\lambda$-bound variable),
this accesses the argument of the $n$-th enclosing $\lambda$.
In the $\mathrm{TailCall}$ case,
$r_1$ is the tail call's head reference
and $\resolveref$ resolves it to the function-body scope.
...
$\visiblebindings(s)$ collects all bindings along the
$\result$ chain:
\begin{equation}\label{eq:lambda-visible-bindings}
  \visiblebindings(s) =
  \bindings(s) \;\cup\;
  \begin{cases}
    \visiblebindings(\result(s))
    & \text{if } \result(s) \neq \bot \\
    \varnothing & \text{otherwise}
  \end{cases}
\end{equation}

\paragraph{Dynamic application-level functions}

\subparagraph{Head.}
$\head(a)$ resolves the head reference of application~$a$
to the applied function's scope:
\begin{equation}\label{eq:lambda-head}
  \head(a) = \resolveref(\headref(a),\;
  \mathrm{enclosing\_scope}(a))
\end{equation}
where $\mathrm{enclosing\_scope}(s \snoc x) = s$.

\subparagraph{Argument.}
$\argument(a)$ resolves the argument reference to a scope:
\begin{equation}\label{eq:lambda-argument}
  \argument(a) = \resolveref(\argref(a),\;
  \mathrm{enclosing\_scope}(a))
\end{equation}
...
introduced~$s$.
We define two functions that extract information from this
context:
\begin{itemize}
  \item $\callerargument(s)$: the argument scope passed to
    the $\lambda$-abstraction~$s$.
  \item $\callerscope(s)$: the scope in which the call to~$s$
    was made.
\end{itemize}
A $\lambda$-body scope~$s$ is entered in one of two ways:
\begin{enumerate}
  \item \textbf{Via a let-binding application.}\;
    An application node $a = s' \snoc x$ satisfies
    $\head(a) = s$.
    Then $\callerargument(s) = \argument(a)$
    and $\callerscope(s) = s'$.
  \item \textbf{Via a tail call.}\;
    A scope $s'$ has
    $\tail(s') = \mathrm{TailCall}(r_1, r_2)$
    with $\resolveref(r_1,\, s') = s$.
    Then $\callerargument(s) = \resolveref(r_2,\, s')$
    and $\callerscope(s) = s'$.
\end{enumerate}
For the root scope, neither function is defined
(the root has no caller).
The Single-Path Lemma
(Lemma~\ref{lem:single-path}) guarantees that each scope
is reached by exactly one calling context, so
$\callerargument$ and $\callerscope$ are single-valued.

\subparagraph{Reference resolution.}
$\resolveref$ resolves any reference to a scope node:
\begin{equation}\label{eq:lambda-resolve-ref}
  \resolveref(r,\; s) =
  \begin{cases}
    \callerargument(\scoperesolution(n, s))
    & \text{if } r = (n,\, \mathrm{argument}) \\[4pt]
    \head(\scoperesolution(n, s) \snoc x)
    & \text{if } r = (n,\, x) \\[4pt]
    s_{\lambda}
    & \text{if } r = \mathrm{Lambda}(M)
    \text{ (inline scope $s_\lambda$)}
  \end{cases}
\end{equation}
For a reference $(n,\, \mathrm{argument})$,
$\scoperesolution$ navigates up $n$ $\lambda$-scope levels,
then $\callerargument$ extracts the argument passed to the
$\lambda$ at that level.
For a reference $(n,\, x)$,
$\scoperesolution$ navigates up $n$ $\lambda$-scope levels,
then $x$ is projected from that scope and its head is
resolved.
...
$\callerscope(s)$---the scope where the call to~$s$ was
made---and decrements~$n$.
Since $n$ is a nonnegative integer, the recursion terminates.
...
equations~(\ref{eq:lambda-result})--(\ref{eq:lambda-scope})
define functions recursively:
$\result(s) = s'$ holds if and only if it can be
established by a finite chain of applications of these
equations.
Well-definedness follows from the same termination argument
as in Appendix~\ref{app:well-definedness}: if the recursive
...
\[
  \mathrm{is\_abstraction}\!\bigl(\result^k(\text{root})\bigr)
\]
...
return~$\bot$), the semantics coincides with the lazy
$\lambda$-calculus:
...
which is strictly more defined: terms that diverge under
$\beta$-reduction may converge
(Remark~\ref{rem:convergence-lfp}).
Only the iteration depth changes.
...
and $\scoperesolution$ specializes $\this$.}
...
(equation~\ref{eq:this}) degenerates to single-valued
functions.
...
\begin{description}
  ...
  \item[WithVisitorTail]
    ...
    This provides the right-hand side of a join\footnote{In the context of the visitor pattern, this acts as the visitor.}: when two relations' $\mathrm{VisitorMap}$ entries are
  ...
  \item[WithVisiteeTail]
    ...
    This provides the left-hand side\footnote{This acts as the visitee.}: the
...
This should not be read as ordinary self-reference of a
...
\footnote{Enabling the \texttt{RecursiveDo} extension does not help:
