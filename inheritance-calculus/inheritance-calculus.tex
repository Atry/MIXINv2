...
\begin{figure*}
  ...
  \begin{tikzpicture}[
    ...
    \node[calc] (flam) at (84,0)  {fixpoint $\lambda$ (ours)};

    \node[calc] (lsub) at (42,19)  {$\lambda$-lazy-IC (ours)};
    \node[calc] (sub)  at (84,19)  {$\lambda$-IC (ours)};

    \node[calc] (lic)  at (42,38)  {lazy IC (ours)};
    \node[calc] (ic)   at (84,38)  {IC (ours)};

    % horizontal edges (convergence): "more defined"
    \draw[more] (lam)  -- node[elbl,below]{more defined} (llam);
    \draw[more] (llam) -- node[elbl,below]{more defined} (flam);
    \draw[more] (lsub) -- node[elbl,above]{more defined} (sub);
    \draw[more] (lic)  -- node[elbl,above]{more defined} (ic);
  ...
  \caption{The calculi of this paper; nodes marked
    ...
    the lazy prefix denotes the first-iteration approximation
    $T_P\!\uparrow\!1$;
    fixpoint $\lambda$ the fixpoint $\lambda$-calculus;
    IC the inheritance-calculus;
    and $\lambda$-IC and $\lambda$-lazy-IC the $\lambda$ sublanguages of
    IC and of lazy IC, respectively.
    Edge labels:
    \emph{more defined} refers to convergence;
...
The remainder of the paper establishes the landscape of calculi and
their relationships shown in Figure~\ref{fig:calculi-matrix}.
