...
Declarative and configuration languages are ubiquitous in modern
software engineering.
Systems such as Jsonnet~\cite{jsonnet}, Hydra~\cite{hydra2023},
...
\emph{opinionated} proxies, grouped under its two notions.

\begin{itemize}
  \item Immutable and first-order%
    \footnote{
      In this paper, we use \emph{first-order} in three senses
      simultaneously:
      the semantic domain contains only data, with no function
      spaces, closures, or
      strategies~\cite{vanemden1976-predicate-logic-semantics};
      the defining equations are first-order functions whose
      inputs and outputs are data, never
      functions~\cite{reynolds1972-definitional-interpreters};
      and the semantics is given by first-order inductive
      definitions~\cite{aczel1977-inductive-definitions} whose
      quantifiers range over data, never over functions or predicates.
    }
    is our proxy for
    \emph{configuration}. A configuration is read, not a function that
    reads an argument. The configuration languages above all compose by
    extending existing structure from outside, their values are immutable,
    and they have no first-class functions; a function body, by contrast,
    is closed, and its internal structure cannot be extended without
    modifying the definition.
  \item Turing-completeness together with extensibility is our proxy for
    \emph{practical general-purpose} programming. Turing-completeness alone
    only means a language can in principle compute anything; it does not
    make the language practical. Software cost is dominated by
    growth~\cite{brooks1987-no-silver-bullet}, so a practical language must
    also scale by adding code rather than rewriting it; this is the
    Expression Problem~\cite{wadler1998-expression-problem}. We therefore
    require both: a Turing-complete language whose programs are extensible.
    This is the sense in which Prolog, though Turing-complete, is not a
    practical general-purpose language: it extends only by adding disjuncts
...
extension mechanism;
the $\lambda$-calculus is first-order-free and its extensibility is opt-in,
since a closed function body cannot be extended without rewriting it;
Prolog meets the other three proxies but is extensible only by adding
disjuncts to existing predicates, not by composing new observations onto
existing values.
...
\end{lstlisting}
  ...
  \begin{minipage}[t]{0.65\textwidth}
    ...
    {\small\(
        \begin{array}{@{}l@{\hspace{2.5em}}l@{}}
          \mathrm{ComplexModule} \mapsto \{ &
            \text{\scriptsize module} \\
            \quad \mathrm{BuiltIns}, &
            \text{\scriptsize import} \\
            \quad \mathrm{Math}, &
            \text{\scriptsize import} \\
            \quad \mathrm{Operator}, &
            \text{\scriptsize import} \\
            \quad \mathrm{Complex} \mapsto \{ &
              \text{\scriptsize class} \\
              \quad\quad \mathrm{re} \mapsto \{ &
                \text{\scriptsize field} \\
              \quad\quad\quad\quad \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Float} \}, &
              \text{\scriptsize imported type} \\
              \quad\quad \mathrm{im} \mapsto \{ &
                \text{\scriptsize field} \\
              \quad\quad\quad\quad \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Float} \}, &
              \text{\scriptsize imported type} \\
              \quad\quad \mathrm{Plus} \mapsto \{ &
                \text{\scriptsize method} \\
                \quad\quad\quad \mathrm{other} \mapsto \{ \mathrm{Complex} \}, &
                \text{\scriptsize parameter} \\
                \quad\quad\quad \mathrm{\_sumReCall} \mapsto \{ &
                  \text{\scriptsize function application} \\
                  \quad\quad\quad\quad \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Add}, &
                  \text{\scriptsize imported function} \\
                  \quad\quad\quad\quad \mathrm{operand0} \mapsto \{ \qualifiedthis{\mathrm{Complex}}.\mathrm{re} \}, &
                  \text{\scriptsize keyword argument} \\
                \quad\quad\quad\quad \mathrm{operand1} \mapsto \{ \qualifiedthis{\mathrm{Plus}}.\mathrm{other}.\mathrm{re} \} \}, &
                \text{\scriptsize keyword argument} \\
                \quad\quad\quad \mathrm{\_sumRe} \mapsto \{ \mathrm{\_sumReCall}.\mathrm{result} \}, &
                \text{\scriptsize local} \\
                \quad\quad\quad \mathrm{\_sumImCall} \mapsto \{ &
                  \text{\scriptsize function application} \\
                  \quad\quad\quad\quad \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Add}, &
                  \text{\scriptsize imported function} \\
                  \quad\quad\quad\quad \mathrm{operand0} \mapsto \{ \qualifiedthis{\mathrm{Complex}}.\mathrm{im} \}, &
                  \text{\scriptsize keyword argument} \\
                \quad\quad\quad\quad \mathrm{operand1} \mapsto \{ \qualifiedthis{\mathrm{Plus}}.\mathrm{other}.\mathrm{im} \} \}, &
                \text{\scriptsize keyword argument} \\
                \quad\quad\quad \mathrm{\_sumIm} \mapsto \{ \mathrm{\_sumImCall}.\mathrm{result} \}, &
                \text{\scriptsize local} \\
                \quad\quad\quad \mathrm{result} \mapsto \{ &
                  \text{\scriptsize return} \\
                  \quad\quad\quad\quad \mathrm{Complex}, &
                  \text{\scriptsize new instance} \\
                  \quad\quad\quad\quad \mathrm{re} \mapsto \{ \mathrm{\_sumRe} \}, &
                  \text{\scriptsize keyword argument} \\
            \quad\quad\quad\quad \mathrm{im} \mapsto \{ \mathrm{\_sumIm} \} \}\}\}, &
            \text{\scriptsize keyword argument} \\
            \quad \mathrm{a} \mapsto \{ \mathrm{Complex}, &
              \text{\scriptsize new instance} \\
              \quad\quad\quad\quad \mathrm{re} \mapsto \{ \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{PI} \}, &
              \text{\scriptsize imported constant} \\
            \quad\quad\quad\quad \mathrm{im} \mapsto \{ \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{E} \} \}, &
            \text{\scriptsize imported constant} \\
            \quad \mathrm{b} \mapsto \{ \mathrm{Complex}, &
              \text{\scriptsize new instance} \\
              \quad\quad\quad\quad \mathrm{re} \mapsto \{ \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{Tau} \}, &
              \text{\scriptsize imported constant} \\
            \quad\quad\quad\quad \mathrm{im} \mapsto \{ \qualifiedthis{\mathrm{ComplexModule}}.\mathrm{PI} \} \}, &
            \text{\scriptsize imported constant} \\
            \quad \mathrm{\_TotalCall} \mapsto \{ &
              \text{\scriptsize method application} \\
              \quad\quad\quad\quad \mathrm{a}.\mathrm{Plus}, &
              \text{\scriptsize method reference} \\
            \quad\quad\quad\quad \mathrm{other} \mapsto \{ \mathrm{b} \} \}, &
            \text{\scriptsize keyword argument} \\
          \quad \mathrm{total} \mapsto \{ \mathrm{\_TotalCall}.\mathrm{result} \} \} &
          \text{\scriptsize module-level variable} \\
        \end{array}
    \)}
  \end{minipage}
  \caption{A first look at inheritance-calculus (right), beside Python
    ...
    Where Python uses distinct language features for module, import,
    class, field, method, local, and object, inheritance-calculus uses
    only definitions and inheritance.
    A module, a class, a method, and a field are all introduced by the
    same definition syntax, one label bound to one record, despite their
    differing naming conventions.
    Every form of reuse is inheritance: an \texttt{import}, the
    instantiation of a class, and a method call alike.
    A function parameter and a constructor argument are alike, each
    supplied as an ordinary definition added to the inherited record.
    An imported type, function, or constant is reached through qualified
    $\this$ into the enclosing module, whether it annotates a field,
    is applied as a function, or is read as a constant.
    ...
    number $(\pi+\tau) + (e+\pi)i$.
    The inheritance-calculus side is slightly longer because it does not
    nest expressions: it is in
    ANF~\cite{flanagan1993-essence-compiling-continuations}, so each
    function application is given a name (the
    $\mathrm{\_sumReCall}$ and $\mathrm{\_sumImCall}$ locals) rather than
  written inline.}
  \label{fig:cheatsheet}
\end{figure*}

In developing real programs in MIXINv2, including a database-backed web server whose Python foreign-function interface only faithfully wraps individual host-library calls, we found that module, class, object, method, field, and local binding can all be expressed as the same construct: a record of definitions composed by inheritance. This paper distills this minimal computational model into \textbf{inheritance-calculus}. Figure~\ref{fig:cheatsheet} is a first look at inheritance-calculus,
beside Python: the same complex-number class with an addition method,
written both ways.
...
Section~\ref{sec:convergence-hierarchy} derives a convergence hierarchy
...
\begin{align*}
  ...
  && \text{(node: definitions and references)} \\[6pt]
  d \quad ::= \quad & \varepsilon
  && \text{(no definitions)} \\
  \mid\quad & (\ell \mapsto N),\; d
  && \text{(definition, then more)} \\[6pt]
  b \quad ::= \quad & \varepsilon
  && \text{(no references)} \\
  \mid\quad & (n,\; w),\; b
  && \text{(reference, then more)} \\[6pt]
...
satisfies:
\begin{enumerate}
  \item Each source $(n,\; w)$ at path $p$ satisfies $n < |p|$.
  \item A source $(n,\; \varepsilon)$ with empty $w$ is the
    ...
    $w$.
\end{enumerate} The node grammar is read \emph{coinductively}: through
...
Inheritance-calculus has $\mathrm{lfp}(T_P)$ semantics,
as established in Section~\ref{sec:mixin-trees}.
Since the $\lambda$-calculus embeds into inheritance-calculus
via~$\mathcal{T}$, translated $\lambda$-terms inherit the
same semantics.
As a corollary, B\"ohm tree equivalence is fully abstract for
the first-iteration approximation $T_P\!\uparrow\!1$,
where each equation is evaluated once and re-entrant calls
return~$\bot$; this corresponds to $\beta$-reduction.
The full $\mathrm{lfp}(T_P)$ is strictly more defined:
re-entrant calls that returned~$\varnothing$ at
$T_P\!\uparrow\!1$ may converge to non-empty sets after
further iteration.
...
---a definite result,
whereas $\beta$-reduction would diverge.
...
It shares the same syntax as the lazy $\lambda$-calculus but
has strictly greater computational power:
$M{\Downarrow}_\lambda$ implies
$\mathcal{T}(M){\Downarrow}_{\mathrm{lfp}}$,
but not conversely.
This establishes a convergence hierarchy
\begin{equation}\label{eq:convergence-hierarchy}
  \text{eager} \subsetneq \text{lazy} \subsetneq \text{fixpoint}
\end{equation}
among $\lambda$-calculi sharing the same syntax.%
\footnote{
  Appendix~\ref{app:bohm-tree-proofs},
  Remark~\ref{rem:convergence-lfp} gives further
  technical details on the observation granularity
  of the set-valued semantics.
}

The convergence hierarchy shows that
inheritance-calculus is strictly more defined than the
$\lambda$-calculus: more terms converge.
The next subsection examines whether it is also strictly
more expressive.
...
\footnote{
  ...
  inheritance-calculus notation; the executable MIXINv2
  sources\anon[~(supplementary material)]{~\cite{mixin2025}}
  are available as supplementary material.
}
This section traces the unary Nat case in detail: its data representation,
addition, and equality.
The implementation follows Gang-of-Four design
patterns~\cite{gamma1994-design-patterns} in a declarative
object-oriented style, with each concern split along two axes.
Along the operation axis, every operation is a separate mixin, which is
the extension dimension of the Expression Problem.
Along the constructor axis, each operation is split again into an
\emph{interface} mixin that declares the operation on the abstract
$\mathrm{Product}$ type, and one \emph{implementation} mixin per data
constructor ($\mathrm{Zero}$ and $\mathrm{Successor}$).
The interface keeps the concern's name (such as $\mathrm{NatPlus}$),
while the implementations append the constructor name (such as
$\mathrm{NatPlusZero}$ and $\mathrm{NatPlusSuccessor}$).
...
The interface mixin $\mathrm{NatData}$ declares only the abstract product
type and the $\mathrm{Nat}$ alias:
\[
...
\[
  \mathrm{NatDataZero} \mapsto \left\{
    \begin{aligned}
      &\mathrm{NatData},\\
      &\mathrm{NatFactory} \mapsto \{\mathrm{Zero} \mapsto \{\qualifiedthis{\mathrm{NatDataZero}}.\mathrm{NatFactory}.\mathrm{Product}\}\}
  \end{aligned}\right\}
\]
A $\mathrm{Successor}$ value inherits $\mathrm{Product}$ and
exposes a $\mathrm{predecessor}$ field:
\[
  \mathrm{NatDataSuccessor} \mapsto \left\{
    \begin{aligned}
      &\mathrm{NatData},\\
      &\mathrm{NatFactory} \mapsto \left\{\mathrm{Successor} \mapsto \left\{
        \begin{aligned}
...
\paragraph{NatPlus}
Addition exposes a $\mathrm{sum}$ field.
Its interface mixin $\mathrm{NatPlus}$ inherits $\mathrm{NatData}$ and
declares the $\mathrm{Plus}$ operation on the abstract $\mathrm{Product}$:
\[
  ...
  \end{aligned}\right\}
\]
The base case is $0 + m = m$: $\mathrm{NatPlusZero}$ inherits the $\mathrm{Zero}$
data and the $\mathrm{Plus}$ interface, and returns the addend directly:
\[
  \mathrm{NatPlusZero} \mapsto \left\{
    \begin{aligned}
...
\[
  \mathrm{NatPlusSuccessor} \mapsto \left\{
    \begin{aligned}
      &\mathrm{NatDataSuccessor},\; \mathrm{NatPlus},\\
      &\mathrm{NatFactory} \mapsto \left\{\mathrm{Successor} \mapsto \left\{\mathrm{Plus} \mapsto \left\{
              \begin{aligned}
                &\mathrm{addend} \mapsto \{\qualifiedthis{\mathrm{NatPlusSuccessor}}.\mathrm{NatFactory}.\mathrm{Product}\},\\
...
visitor pattern.
$\mathrm{NatVisitor}$ adds an $\mathrm{Acceptance}$ operation that contains a
$\mathrm{VisitorMap}$ with the appropriate $\mathrm{ZeroVisitor}$ or
$\mathrm{SuccessorVisitor}$ branch and an $\mathrm{Accepted}$ field that
delegates to the matching branch.
The interface mixin declares the empty $\mathrm{Acceptance}$ on $\mathrm{Product}$:
\[
  \mathrm{NatVisitor} \mapsto \left\{
    \begin{aligned}
      &\mathrm{NatData},\\
...
\[
  \mathrm{NatVisitorZero} \mapsto \left\{
    \begin{aligned}
...
\[
  \mathrm{NatVisitorSuccessor} \mapsto \left\{
    \begin{aligned}
      &\mathrm{NatDataSuccessor},\; \mathrm{NatVisitor},\\
      &\mathrm{NatFactory} \mapsto \left\{\mathrm{Successor} \mapsto \left\{
            \begin{aligned}
              &\mathrm{predecessor} \mapsto \{\qualifiedthis{\mathrm{NatVisitorSuccessor}}.\mathrm{NatFactory}.\mathrm{Product}\},\\
...
The interface mixin $\mathrm{NatEquality}$ inherits $\mathrm{NatVisitor}$ and $\mathrm{BooleanData}$, using each as a module,
and declares an $\mathrm{Equal}$ operation on $\mathrm{Product}$ that the per-constructor implementations provide.
Inheriting then serves as importing:
because $\mathrm{NatEquality}$ inherits $\mathrm{BooleanData}$,
the qualified this $\qualifiedthis{\mathrm{NatEquality}}$ can reach the members
defined in $\mathrm{BooleanData}$, so $\mathrm{Equal}$ returns a boolean with a
reference like $\qualifiedthis{\mathrm{NatEquality}}.\mathrm{BooleanFactory}.\mathrm{False}$,
reading $\mathrm{False}$ as a static member of the $\mathrm{BooleanFactory}$ class
that this import of $\mathrm{BooleanData}$ brings in.
Inheriting then also serves as case analysis:
inheriting $\mathrm{other}.\mathrm{Acceptance}$ dispatches on whether $\mathrm{other}$
was built by $\mathrm{Zero}$ or $\mathrm{Successor}$.
Inheriting likewise serves as a type annotation:
in $\mathrm{Accepted}$, $\mathrm{equal}$ inherits $\qualifiedthis{\mathrm{NatEquality}}.\mathrm{Boolean}$,
declaring its type to be $\mathrm{Boolean}$, once more the type imported from $\mathrm{BooleanData}$:
\[
  \mathrm{NatEquality} \mapsto \left\{
    \begin{aligned}
  ...
  \end{aligned}\right\}
\]
$\mathrm{Zero}$ is equal only to another $\mathrm{Zero}$, so its implementation
dispatches on $\mathrm{other}$ and returns $\mathrm{True}$ on the
$\mathrm{ZeroVisitor}$ branch and $\mathrm{False}$ otherwise:
{\small
  \[
    \mathrm{NatEqualityZero} \mapsto \left\{
      \begin{aligned}
        &\mathrm{NatVisitorZero},\; \mathrm{NatEquality},\\
        &\mathrm{NatFactory} \mapsto \left\{\mathrm{Zero} \mapsto \left\{\mathrm{Equal} \mapsto \left\{
                  \begin{aligned}
                      ...
                      \begin{aligned}
                        ...
                        &\left\{
                          \begin{aligned}
                            ...
                            &\{\mathrm{equal} \mapsto \{\qualifiedthis{\mathrm{NatEqualityZero}}.\mathrm{BooleanFactory}.\mathrm{True}\}\},\\
                            &\mathrm{SuccessorVisitor} \mapsto \\
                            &\{\mathrm{equal} \mapsto \{\qualifiedthis{\mathrm{NatEqualityZero}}.\mathrm{BooleanFactory}.\mathrm{False}\}\}
                        \end{aligned}\right\},\\
...
{\small
  \[
    \mathrm{NatEqualitySuccessor} \mapsto \left\{
      \begin{aligned}
        &\mathrm{NatVisitorSuccessor},\; \mathrm{NatEquality},\\
        &\mathrm{NatFactory} \mapsto \left\{\mathrm{Successor} \mapsto \left\{\mathrm{Equal} \mapsto \left\{
                  \begin{aligned}
                    ...
                    \end{aligned}\right\},\\
                    &\mathrm{\_recursiveEquality} \mapsto \\
                    &\left\{
                      \begin{aligned}
                        &\qualifiedthis{\mathrm{Successor}}.\mathrm{predecessor}.\mathrm{Equal},\\
                    ...
                    &\left\{
                      \begin{aligned}
                        ...
                        &\left\{
                          \begin{aligned}
                            &\mathrm{ZeroVisitor} \mapsto \\
                            &\{\mathrm{equal} \mapsto \{\qualifiedthis{\mathrm{NatEqualitySuccessor}}.\mathrm{BooleanFactory}.\mathrm{False}\}\},\\
                            &\mathrm{SuccessorVisitor} \mapsto \\
                            &\{\mathrm{equal} \mapsto \{\mathrm{\_recursiveEquality}.\mathrm{equal}\}\}
                        \end{aligned}\right\},\\
                        &\mathrm{Accepted} \mapsto \\
                        &\{\mathrm{equal} \mapsto \{\qualifiedthis{\mathrm{NatEqualitySuccessor}}.\mathrm{Boolean}\}\}
                    \end{aligned}\right\},\\
                    &\mathrm{equal} \mapsto \{\mathrm{\_OtherAcceptance}.\mathrm{Accepted}.\mathrm{equal}\}
                \end{aligned}\right\}\right\}\right\}
    \end{aligned}\right\}
...
In developing real programs in MIXINv2, three phenomena manifested as off-label use of the adopted design patterns.%
\footnote{
...
source code\anon[~(supplementary material)]{~\cite{mixin2025}}
and the case study of Section~\ref{sec:case-study}.
...
\begin{description}
  \item[Relational semantics]
    ...
    as demonstrated in Section~\ref{sec:case-study}.
  ...
  \item[Immunity to nonextensibility]\label{item:immunity}
    ...
    from Section~\ref{sec:syntax} is extensible via deep merge
    (equation~\ref{eq:overrides}).
    The case study of Section~\ref{sec:case-study} and the
    expressive asymmetry of Section~\ref{sec:asymmetry}
    ...
    \footnote{
      ...
      inheritance-calculus\anon[~(supplementary material)]{~\cite{mixin2025}} inherits the immunity to nonextensibility and need
  ...
  \item[Function color blindness]
    ...
    We observed that the same MIXINv2 file runs under both synchronous
    and asynchronous runtimes\anon[~(supplementary material)]{~\cite{mixin2025}},
    an instance of function color
    blindness~\cite{nystrom2015-function-color}.%
    \footnote{
      The mechanism is dependency injection via inheritance: business
      logic declares empty slots, and the assembly inherits an FFI
      module that fills them with a synchronous or asynchronous
      implementation, so the slot's color varies at assembly time
      ...
      not arise at the calculus level, and effect execution is
      delegated to the host language.
...
as a corollary in Section~\ref{sec:convergence-hierarchy}.
...
\anon[Both are also included as supplementary material, together
with a test suite that reproduces every example in this paper.]{}
...
may converge, as shown in Section~\ref{sec:convergence-hierarchy},
...
\begin{remark}[Convergence hierarchy]
  ...
  whether the $\result$ chain reaches an abstraction
...
Adding $R(b, c)$:
\[
  R \mapsto \left\{\begin{aligned}
    &a,\; b,\\
    &\mathrm{TailMap} \mapsto \left\{\begin{aligned}
...
{\small
  \[
    \mathrm{xAcc} \mapsto \left\{\begin{aligned}
      &\alpha.\mathrm{Acceptance},\; \mathrm{VisitorMap} \mapsto \{\alpha.\mathrm{WithVisiteeTail}\},\\
      &\mathrm{Visitor} \mapsto \left\{\begin{aligned}
...
{\small
  \[
    \mathrm{yAcc} \mapsto \left\{\begin{aligned}
...
{\small
  \[
    \mathrm{nilAcc}_0 \mapsto \left\{\begin{aligned}
      &\_Y_0.\mathrm{Acceptance},\\
      &\mathrm{VisitorMap} \mapsto \left\{\mathrm{Nil} \mapsto \left\{\begin{aligned}
        &\ldots,\\
...
{\small
  \[
    \mathrm{zAcc} \mapsto \left\{\begin{aligned}
...
{\small
  \[
    \mathrm{nilAcc}_1 \mapsto \left\{\begin{aligned}
