...
(vertices of~$G$), and let
$\mathcal{V}$ be the set of possible answer values
(paths, sets of paths, or sets of pairs of paths,
depending on the equation).
A \emph{query--answer map} is a function
$M : \mathcal{Q} \to \mathcal{P}(\mathcal{V})$
assigning to each query a set of answer values.
The \emph{immediate consequence
operator}~\cite{vanemden1976semantics}
$T_P$ maps a query--answer map~$M$ to a new
query--answer map~$T_P(M)$ by evaluating the
right-hand side of each
equation~(\ref{eq:properties})--(\ref{eq:this}),
substituting $M(q)$ for every sub-query~$q$.
\end{definition}

\begin{lemma}[Monotonicity]\label{lem:monotonicity}
The operator $T_P$ is monotone on the complete lattice
$(\mathcal{Q} \to \mathcal{P}(\mathcal{V}),\; \subseteq)$.
\end{lemma}
...
$(\mathcal{P}(\mathcal{V}), \subseteq)$
is complete,
so the pointwise lattice
$(\mathcal{Q} \to \mathcal{P}(\mathcal{V}), \subseteq)$
is also complete.
By the Knaster--Tarski
...
subgraph and computes the exact value.
When reentry occurs and $A = A \cup T_P(A)$,
$A$ is a post-fixed point of~$T_P$.
Since $A$ was built by iterating from
$\bot = \lambda q.\, \varnothing$,
it is the \emph{least} post-fixed point, i.e.\
$\mathrm{lfp}(T_P)$ restricted to the reachable
queries.
...
is encoded using three mechanisms that compose by nesting:
