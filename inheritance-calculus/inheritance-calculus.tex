...
Inheriting serves as importing:
...
defined in $\mathrm{BooleanData}$, so $\mathrm{Equal}$ declares its result
$\mathrm{equal}$ with the type $\qualifiedthis{\mathrm{NatEquality}}.\mathrm{Boolean}$,
reading $\mathrm{Boolean}$ as a member imported from $\mathrm{BooleanData}$:
\[
...
Inheriting here serves as case analysis:
inheriting $\mathrm{other}.\mathrm{Acceptance}$ dispatches on whether $\mathrm{other}$
was built by $\mathrm{Zero}$ or $\mathrm{Successor}$, returning
$\qualifiedthis{\mathrm{NatEqualityZero}}.\mathrm{BooleanFactory}.\mathrm{True}$
on the $\mathrm{ZeroVisitor}$ branch and $\mathrm{False}$ otherwise.
Inheriting likewise serves as a type annotation:
in $\mathrm{Accepted}$, $\mathrm{equal}$ inherits $\qualifiedthis{\mathrm{NatEqualityZero}}.\mathrm{Boolean}$,
declaring its type to be $\mathrm{Boolean}$:
{\small
  \[
