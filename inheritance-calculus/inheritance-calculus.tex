...
\begin{itemize}
  ...
  \item The semantics is \emph{observational}: the observer drives computation by
    ...
    denotational, and fixpoint-computable.
  ...
  \item The translation gives the lazy $\lambda$-calculus a
    ...
    As a corollary, some $\lambda$-terms that diverge
    in the $\lambda$-calculus converge under
    $\mathrm{lfp}(T_P)$ after translation
    (Section~\ref{sec:translation};
...
\footnote{If no matching pair
  ...
  target, and no properties are inherited.
...
evaluation}~\cite{tamaki1986old,chen1996tabled}:
each query is memoised on first encounter, and
re-entrant calls (cycles in the dependency graph)
return the current accumulator rather than diverging.
Denotationally, the six equations define a monotone
immediate consequence
operator~$T_P$~\cite{vanemden1976semantics}---their
right-hand sides use only $\cup$, $\in$, equality, and
positive set comprehension, with no negation, set
difference, or complement---so
by the Knaster--Tarski
theorem~\cite{tarski1955lattice}, $T_P$ has a unique
...
All three also inherit $\beta$-reduction's divergence on
...
reduction rules.
In particular, Van~Emden and Kowalski's $T_P$ computes
$\mathrm{lfp}$ for recursive Datalog programs, but has not
been used to give recursive $\lambda$-programs a fixpoint
semantics in which they converge instead of diverging.
...
operator}~\cite{vanemden1976semantics}
...
is also complete.
By the Knaster--Tarski
theorem~\cite{tarski1955lattice},
...
\begin{enumerate}
  ...
  \item \textbf{Correct fixpoint detection.}
    ...
    stable (no new facts were added), the
    accumulator equals
  ...
  \item \textbf{Sound failure.}
    When tabling cannot determine the answer---either
    because the iteration budget is exhausted
    (signalled by an explicit exception) or because
    of an infinite non-cyclic chain (stack
    overflow)---it raises an error rather than
...
maps on~$R$.
If the accumulator is stable, i.e.\
...
Therefore $A = \mathrm{lfp}(T_P^R)$, i.e.\ the stable
...
\item \textbf{Cyclic, finite per-query domain.}
  ...
  lattice~\cite{bancilhon1986amateur}.
...
subtrees ($z$ and $\mathrm{result}$) coincide.
...
\paragraph{Extension to $\mathrm{lfp}(T_P)$ semantics}
\label{rem:convergence-lfp}
The proof above works directly with the six
equations~(\ref{eq:properties})--(\ref{eq:this}),
not with any particular evaluation strategy.
We distinguish two consequences of switching from
single-pass evaluation to multi-pass tabled evaluation
(computing $\mathrm{lfp}(T_P)$ via iterated
application of~$T_P$;
see Appendix~\ref{app:well-definedness}).

\begin{description}
\item[Head-reduction preservation is maintained.]
...
$\mathrm{result}$ step.
This isomorphism preserves monotonicity, so
$\mathrm{lfp}(T_P)$ yields the same answer on
both sides (modulo the depth shift):
if $\mathcal{T}(M)$ converges at depth~$n$ under
$\mathrm{lfp}$, then $\mathcal{T}(N)$ converges at
depth~$n - 1$, and vice versa.
For $\lambda$-calculus--translated programs, the
Single-Path Lemma
(Lemma~\ref{lem:single-path}) ensures that no
cycles arise in the dependency graph (each scope
level has exactly one inheritance source), so
single-pass and multi-pass evaluation produce
identical results in this fragment.

\item[Agreement with the $\lambda$-calculus is
not maintained in general.]
For programs beyond the $\lambda$-calculus
fragment that introduce cyclic dependencies,
$\mathrm{lfp}(T_P)$ may converge where
the $\lambda$-calculus diverges.
This means $\mathrm{lfp}(T_P)$ evaluation is
\emph{strictly more defined} than
$\beta$-reduction on the translated image:
$M{\Downarrow}_\lambda$ implies
$\mathcal{T}(M){\Downarrow}_{\mathrm{lfp}}$,
but not conversely.

As a minimal illustration, the $\lambda$-term
$\mathbf{let}\; x = x \;\mathbf{in}\; x$
diverges under $\beta$-reduction (unfolding
$x$ produces $x$ again, \emph{ad infinitum}).
Translation~$\mathcal{T}$ gives
\[
  \{x \mapsto \{\mathrm{result} \mapsto
    x.\mathrm{result}\},\;
  \mathrm{result} \mapsto x.\mathrm{result}\}
\]
where $x.\mathrm{result}$ inherits from
itself, creating a cyclic dependency.
Under single-pass evaluation
($\mathit{max\_fixpoint\_iterations} = 0$),
the cycle raises~$\bot$, matching the
$\lambda$-calculus divergence.
Under $\mathrm{lfp}(T_P)$, fixpoint iteration
detects the cycle and converges:
$\mathrm{properties}(\mathrm{root}) =
\{x, \mathrm{result}\}$ and
$\mathrm{properties}(\mathrm{root} \triangleright
\mathrm{result}) = \varnothing$,
reflecting that the value is undefined
(no abstraction shape).
Appendix~\ref{app:datalog-encoding} gives a
more motivated example (Datalog transitive
closure on cyclic graphs).
\end{description}
...
$\mathbf{let}$-binding.
In each case, the head form is a $\beta$-redex in the
underlying $\lambda$-term: the function position is occupied by
a closed ANF value, and the only closed ANF value that can
appear at head position is a $\lambda$-abstraction.
By Lemma~\ref{lem:result-step} (tail-call,
  application let-binding, or value let-binding
...
route is taken.
Because attribute resolution aggregates results via set union (which is intrinsically idempotent), identical property definitions from the two routes merge without conflict. Thus, the calculus eliminates the need for any artificial route-priority or linearization rules.
...
amount of boilerplate per constant is $O(1)$---it does not
grow with the domain size $|D|$---so adding a new constant
...
\item[WithVisitorTail]
  ...
  This provides the \emph{right-hand} (visitor) side of a
  join: when two relations' $\mathrm{VisitorMap}$ entries are
...
\item[WithVisiteeTail]
  ...
  This provides the \emph{left-hand} (visitee) side: the
...
\item[Replacement]
  ...
  $\mathrm{NewTail}$.
  This is the \emph{output construction} mechanism: it
...
evaluation}~\cite{bancilhon1986amateur} of the
$T_P$
operator~\cite{vanemden1976semantics}:
...
\emph{Semi-Naive} evaluation~\cite{bancilhon1986amateur},
...
a single output tuple $(a, c)$---not all of $\{a,b,c\}$.
