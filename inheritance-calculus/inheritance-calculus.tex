...
When evaluation uses multi-pass tabled evaluation
(computing $\mathrm{lfp}(T_P)$ via iterated
application of~$T_P$;
see Appendix~\ref{app:well-definedness}),
convergence preservation still holds:
\begin{itemize}
\item
...
\item
  ...
  multi-pass evaluation may converge where
  single-pass evaluation diverges; in this case,
  convergence preservation under
  $\mathrm{lfp}(T_P)$ follows from the
  isomorphism argument: if $\mathcal{T}(M)$
  converges at depth~$n$ under $\mathrm{lfp}$,
  then $\mathcal{T}(N)$ converges at
  depth~$n - 1$, and vice versa.

  As a minimal illustration, consider transitive
  closure on a cyclic graph.  In the lazy
  $\lambda$-calculus, the na\"ive recursive
  definition
  \[
    \mathbf{let}\; \mathit{path} =
    \lambda x.\, \lambda y.\,
    \mathit{edge}\; x\; y
    \;\lor\;
    \exists z.\,
    ...
    \mathit{path}\; z\; y
  \]
  diverges (stack overflow) when the graph
  contains cycles, because
  $\mathit{path}\; a\; b$ unfolds
  $\mathit{path}\; a\; b$ again without
  memoization.
  The inheritance-calculus can express the same
  computation using cyclic self-reference
  (multiple inheritance paths that revisit the
  same scope), and $\mathrm{lfp}(T_P)$
  converges by iterating from~$\bot$ until
  the set of reachable pairs stabilizes---exactly
  as in Datalog's bottom-up evaluation.
  Appendix~\ref{app:datalog-encoding} gives the
  full encoding.
\end{itemize}
