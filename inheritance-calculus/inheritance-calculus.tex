...
Its package collection, \texttt{nixpkgs}, is one of the largest
in existence.%
\footnote{
...
and multi-language package building~\cite{dream2nix}---%
\footnote{
  dream2nix~\cite{dream2nix} uses the NixOS module system for
  package building, entering the very domain that
  the asymmetric overlay mechanism was designed for.
}
the original domain of \texttt{nixpkgs}'s asymmetric mechanism.
...
The NixOS module system already suggests that such a model may exist:
...
Nix is an exception: its lazy evaluation makes values
