...
exposes a $\mathrm{predecessor}$ field whose type is $\mathrm{Product}$:
\[
  \begin{aligned}
    ...
    \begin{aligned}
      ...
      &\left\{\mathrm{Successor} \mapsto \left\{
        \begin{aligned}
          &\qualifiedthis{\mathrm{NatFactory}}.\mathrm{Product},\\
...
The mixin $\mathrm{NatPlus}$ inherits $\mathrm{NatData}$ and
declares the $\mathrm{Plus}$ operation on the abstract $\mathrm{Product}$.
Read in object-oriented terms, $\mathrm{Plus}$ is a method on
$\mathrm{Product}$ whose return value is the $\mathrm{sum}$ field:
\[
...
visitor pattern.
$\mathrm{NatVisitor}$ adds an $\mathrm{Acceptance}$ operation to $\mathrm{Product}$.
The mixin $\mathrm{NatVisitor}$ declares it empty, leaving a $\mathrm{VisitorMap}$ and an
$\mathrm{Accepted}$ field for the per-constructor implementations to fill in:
\[
  \begin{aligned}
...
Although this acceptance machinery is isomorphic to the GoF visitor pattern,
it escapes that pattern's characteristic rigidity: in GoF the set of data
constructors is fixed once the $\mathrm{accept}$ interface is written, so a new
constructor forces every existing visitor to be edited. Here each constructor
is its own mixin that supplies its own branch, so adding a new data constructor
is as modular as adding a new operation, neither requires touching the existing
code:
\[
  \begin{aligned}
  ...
  \[
    \begin{aligned}
      ...
      \begin{aligned}
        ...
        &\left\{
                  \begin{aligned}
                    &\mathrm{other} \mapsto \{\qualifiedthis{\mathrm{NatFactory}}.\mathrm{Product}\},\\
