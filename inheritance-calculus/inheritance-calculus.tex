...
\begin{itemize}
  ...
  \item The mixin is a single abstraction that unifies module,
  ...
  \item The semantics is \emph{observational}: the observer drives computation by
    ...
    denotational, and computable by tabling~\cite{tamaki1986-tabled-resolution}.
  \item The mixin tree \emph{is} the call stack.
    Self-reference resolves to \emph{multiple targets}.
  \item The $\lambda$-calculus macro-expresses into
    inheritance-calculus in six local rules
  ...
  \item The translation gives the lazy $\lambda$-calculus a
    ...
    to be faithful to the first-iteration approximation
    of a sublanguage of
    inheritance-calculus's first-order equations.
    Appendix~\ref{app:lambda-fixpoint} reformulates this
    semantics directly on ANF\@.
  \item As a corollary, this establishes a convergence hierarchy
    among the eager, lazy, and fixpoint $\lambda$-calculi
    sharing the same $\lambda$-syntax
    (Section~\ref{sec:translation};
