...
%% Indexed hylomorphism notation
%% (Meijer--Fokkinga--Paterson 1991; L\"oh--Magalh\~aes 2011)
%%   \hylo{I}        — full indexed hylomorphism over index set I
%%   \hyloi{i}       — component at index value i
\newcommand{\Idx}{\mathcal{I}}
\newcommand{\hylo}[1]{[\![(\varphi_i,\; \alpha_i)_{i \in #1}]\!]}
\newcommand{\hyloi}[1]{[\![\varphi_{#1},\; \alpha_{#1}]\!]}
...
for any finite observation. The \texttt{nixpkgs} package
...
one. Prior systems such as Scala and the NixOS module system assume
single-object resolution and reject the multi-path situation.
Self-referential records require no fixed-point combinator
in the object language.
The mixin tree \emph{is} the call stack, and qualified this resolution
is a first-order set-theoretic query over paths.
...
adding $\{1,2\}$ to $\{3,4\}$ yields $\{4,5,6\}$---the
relational semantics of logic programming~\cite{vanemden1976semantics}
(Section~\ref{sec:case-study}).
Furthermore, the same web
...
(Section~\ref{sec:forward-translation}), but the lazy
$\lambda$-calculus cannot macro-express the inheritance
constructors.
...
\begin{itemize}
  ...
  \item The mixin is a single abstraction that unifies class,
    ...
    and associative, and the linearization problem in OOP does not arise.
  \item The semantics is \emph{observational}: the observer drives computation by
    querying paths in a lazily constructed tree.
    The six semantic equations are first-order%
    \footnote{First-order in three senses simultaneously:
      the semantic domain contains only paths (finite
      sequences of labels) and sets of paths, with no function
      spaces, closures, or strategies;
      ...
      definitions~\cite{aczel1977inductive} over paths
      and labels, where quantifiers range over paths and labels,
    never over functions or predicates.},
    denotational, and computable.
  \item The mixin tree \emph{is} the call stack.
    Self-reference resolves to \emph{multiple objects}.
    Since inheritance
    is idempotent, all objects contribute equally.
  \item The $\lambda$-calculus macro-expresses into
    inheritance-calculus in five local rules
    (Section~\ref{sec:translation}).
  \item The translation gives the lazy $\lambda$-calculus a
    first-order semantics
    (Section~\ref{sec:bohm-tree}):
    the prior higher-order semantic frameworks are shown
    to be faithful to a sublanguage of
...
The $\mapsto$ in $\ell \mapsto e$ defines a property.
...
The definitions~(\ref{eq:properties})--(\ref{eq:this})
...
these functions.
The six functions decompose into an indexed
hylomorphism~\cite{meijer1991functional,loh2011generic}
$\hylo{\Idx}$, a pattern of structured mutual
recursion~\cite{mcbride2015turing};
Appendix~\ref{app:well-definedness} gives the explicit
coalgebra--algebra decomposition for each function,
proves well-definedness for terminating queries,
and characterises divergence via the final coalgebra.

Since hylomorphisms are a computation
model~\cite{hinze2015conjugate}, the functions \emph{are}
the interpreter: the observer drives the computation by
choosing which path to inspect, and each coalgebra
unfolds on demand.
The functions are pure and order-independent, so the
...
%%
%% 这也解释了与 Scala 的关系：Scala 拒绝 multi-path inheritance
...
\footnote{Standard ANF allows values inline as operands;
...
naming intermediate results (and, optionally, naming values),
and every ANF term is a $\lambda$-term, so ANF is neither a
restriction nor an extension of the $\lambda$-calculus.
...
that the lazy $\lambda$-calculus has a
first-order semantics: the translation $\mathcal{T}$
embeds the $\lambda$-calculus into a sublanguage of
inheritance-calculus,
using only recursive evaluation of set-comprehension
equations over paths and sets of paths.
The prior higher-order semantic frameworks
of~\cite{scott1972continuous,wadsworth1976semantics,abramsky2000full,hyland2000full,barendregt1984lambda}
are thereby shown to be faithful to this first-order
semantics: they agree with the equations on B\"ohm tree
equivalence~\cite{barendregt1984lambda}.%
\footnote{Composing $\mathcal{T}$ with the equations of
  Section~\ref{sec:mixin-trees} yields a first-order semantics
...
\footnote{The data representation (Zero/Successor) is a Peano
  ...
  The Visitors pattern (NatVisitor, below) adds the
...
Visitors pattern.
$\mathrm{NatVisitor}$ adds a $\mathrm{Visitors}$ operation to each constructor,
with a $\mathrm{Accepted}$ field that delegates to the appropriate
$\mathrm{ZeroVisitor}$ or $\mathrm{SuccessorVisitor}$ branch:
\begin{align*}
  \mathrm{NatVisitor} \mapsto \{\;&
    \mathrm{NatData},\\
    &\mathrm{NatFactory} \mapsto \{\\
      &\quad \mathrm{Product} \mapsto \{\mathrm{Visitors} \mapsto \{
      \mathrm{Accepted} \mapsto \{\}\}\},\\[2pt]
      &\quad \mathrm{Zero} \mapsto \{\mathrm{Visitors} \mapsto \{
          \mathrm{ZeroVisitor} \mapsto \{\},\;
      \mathrm{Accepted} \mapsto \{\mathrm{ZeroVisitor}\}\}\},\\[2pt]
      &\quad \mathrm{Successor} \mapsto \{\mathrm{Visitors} \mapsto \{
          \mathrm{SuccessorVisitor} \mapsto \{\},\;
  \mathrm{Accepted} \mapsto \{\mathrm{SuccessorVisitor}\}\}\}\}\}
\end{align*}
...
Case analysis on $\mathrm{other}$ is done via its $\mathrm{Visitors}$:
{\small
  \begin{align*}
    \mathrm{NatEquality} \mapsto \{\;&
      ...
      &\mathrm{NatFactory} \mapsto \{\\
        ...
        &\quad \mathrm{Zero} \mapsto \{\mathrm{Equal} \mapsto \{
            ...
            &\qquad
            \mathrm{equal} \mapsto \{
        \mathrm{OtherVisitor}.\mathrm{Accepted}.\mathrm{equal}\}\}\},\\[2pt]
        &\quad \mathrm{Successor} \mapsto \{\mathrm{Equal} \mapsto \{
            ...
            &\qquad
            \mathrm{equal} \mapsto \{
    \mathrm{OtherVisitor}.\mathrm{Accepted}.\mathrm{equal}\}\}\}\}\}
\end{align*}}
%% === 审稿人备忘：关于代码"冗长"的回应 ===
%%
...
%%    OtherVisitor），步骤写得详细。名字本身是零成本的语义锚点。
...
scopes lack labels.
After ANF
transformation~\cite{flanagan1993essence}, however, every intermediate
result is bound to a name, and the remaining anonymous positions%
\footnote{For example, the body of a $\mathbf{let}$.}
...
\begin{description}
  ...
  \item[Conflict rejection]
    ...
    labels~\cite{harper1991record}, precisely the composability that the
    Expression Problem~\cite{wadler1998expression} requires.

  \item[Priority merge]
    One source takes
    precedence~\cite{jsonnet,dolstra2010nixos},
    so $A + B \neq B + A$, introducing
  ...
  \item[$\beta$-reduction]
    ...
    is function application, whether functional, object-oriented, or
    multi-paradigm, because method calls, virtual
...
\begin{description}
  ...
  \item[Single-target $\this$]
    ...
    combinator solves for a single self-reference.
...
\footnote{Three successive implementations confirmed this; see the discussion above.}
...
Meyer~\cite{meyer1982model} showed that combinatory algebras
...
%%     - 判断结构形态并分支：需要 Visitors pattern（Section 4）。
...
%%       不修改原始定义。
%%   Visitors 在精神上就是 F：两者都是对 closed normal form 做
%%   constructor dispatch。区别在于 F 是不可扩展的 built-in primitive
%%   （硬编码了 S 和 F 两个 atom），而 Visitors 是可扩展的 encoding
...
%%   Visitors pattern 显式编码。
%%   换言之：inheritance-calculus 中 F 的 *读取* 能力是免费的
%%   （所有值都是透明 record），但 F 的 *分支* 能力需要 Visitor。
%%   SF-calculus 的 F 则把读取和分支捆绑在一个不可扩展的 primitive 里。
%%   Visitors 是可扩展的（新增 constructor 只需继承新分支，
...
calculi~\cite{abadi1991explicit}.
...
strategies.
None of these approaches is first-order;%
\footnote{Function spaces
  in denotational semantics, strategies
  in game semantics, and substitution
in operational semantics.}
none eliminates the function space from the semantic domain;
and none unifies denotational and operational semantics
into a single first-order definition.
...
finite multisets;
Ehrhard~\cite{ehrhard2005finiteness} showed that a standard
reflexive object for the pure $\lambda$-calculus cannot
exist in this category.
All three families are semantic \emph{models}: they interpret
$\lambda$-terms in an external mathematical structure and
validate $\beta$-reduction as an equation in that structure.
None is a self-contained calculus with its own primitives
into which the $\lambda$-calculus embeds.
None is first-order, since the function space is encoded, not
eliminated.
And none provides a fully abstract denotational semantics of
the lazy $\lambda$-calculus without higher-order machinery.
...
rests on.
Leroy and Grall~\cite{leroy2006coinductive} treated divergence
in big-step semantics via coinductive definitions;
in inheritance-calculus, divergence already emerges as
non-termination of recursive evaluation, so no separate
coinductive formulation is needed.
None of these prior frameworks has been applied to a first-order
domain of paths to obtain a fully abstract semantics of the lazy
...
Inheritance is composition of \emph{generators}---functions from self
...
algorithm~\cite{c3linearization} was later developed to impose a
deterministic order.
Sch\"arli et al.~\cite{scharli2003traits} introduced traits,
whose composition is commutative but rejects same-name
conflicts~\cite{ducasse2006traits}.
...
Neither mixins nor traits are therefore deep-mergeable.

\paragraph{Object and record calculi}
Abadi and Cardelli~\cite{abadi1996theory} treat objects as the
primitive notion, but methods are functions that take self as a
parameter and object extension is asymmetric.
Boudol~\cite{boudol2004recursive} showed that self-referential
records require an unsafe fixed-point operator whose
well-definedness depends on the evaluation strategy.
Harper and Pierce~\cite{harper1991record} gave a record
calculus with symmetric concatenation that rejects same-label
composition;
Cardelli~\cite{cardelli1992extensible} studied extensible records
with subtyping;
R\'emy~\cite{remy1989type} introduced row polymorphism for
...
and self-reference requires an explicit fixed-point combinator.

\paragraph{Family polymorphism and DOT}
Ernst~\cite{ernst2001family} introduced family polymorphism;
Ernst, Ostermann, and Cook~\cite{ernst2006virtual}
formalized it in the virtual class calculus, where classes are
accessed via path expressions \texttt{this.out.C}.
Amin et al.~\cite{amin2016dot} formalized path-dependent types
...
constructs.
This section exploits the first-order character to establish
well-definedness via an indexed hylomorphism over the
dependency graph.
...
\begin{definition}[Coalgebras, algebras, and the indexed functor]%
\label{def:indexed-functor}
Each equation~(\ref{eq:properties})--(\ref{eq:this})
decomposes into a \emph{coalgebra}~$\alpha_i$
that extracts the sub-queries on its right-hand side
(the outgoing edges from a vertex in~$G$)
and an \emph{algebra}~$\varphi_i$
that assembles the sub-query results into the
equation's value.
We write each decomposition below;
the hylomorphism at index~$i$ recovers the
original equation as
$\hyloi{i} = \varphi_i \circ F_i(\hylo{\Idx}) \circ \alpha_i$%
~\cite{meijer1991functional,loh2011generic}.

\medskip\noindent\textbf{(1)\;$\properties$.}
\begin{align}
  \alpha_{\properties}(p)
  &\;=\;
  \{\,\supers(p)\,\}
  \\
  \varphi_{\properties}(p,\; R_{\supers(p)})
  &\;=\;
  \{\; \ell \;|\;
    (\_,\; p_{\mathrm{override}}) \in R_{\supers(p)},\;
    \ell \in \defines(p_{\mathrm{override}})
  \;\}
\end{align}

\noindent\textbf{(2)\;$\supers$.}
\begin{align}
  \alpha_{\supers}(p)
  &\;=\;
  \{\,\overrides(p),\;\bases(p)\,\} \notag \\
  &\quad{}\cup\;
  \{\,\supers(p_{\mathrm{base}}) \;|\;
  p_{\mathrm{base}} \in R_{\bases(p)}\,\}
  \\
  \varphi_{\supers}(
    p,\;R_{\overrides(p)},\;
    R_{\bases(p)}, \notag \\
    \quad
    (R_{\supers(p_{\mathrm{base}})})_{
    p_{\mathrm{base}} \in R_{\bases(p)}}
  ) &\;=\;
  \{\,(\init(p),\; p_{\mathrm{override}})
    \;|\;
    p_{\mathrm{override}}
  \in R_{\overrides(p)}\,\}%
  \footnote{The functions $\init$ and $\last$ are
    defined only for non-root paths
    (Section~\ref{sec:mixin-trees}).
    When $p = ()$, the value $\init(p)$ in the first
    component is never consumed:
    $\varphi_{\properties}$ uses only the second
    component~$p_{\mathrm{override}}$,
    and $\alpha_{\overrides}$ calls
    $\supers(\init(p))$
    only for non-root~$p$.
    In $\alpha_{\bases}$ and $\varphi_{\bases}$,
    $\init(p)$ is guarded by the condition
    $(n, \ell_{\mathrm{down},*})
    \in \inherits(p_{\mathrm{override}})$;
    since $\inherits = \varnothing$ at the root,
    $\init(p)$ is never reached.
    Likewise, $\alpha_{\resolve}$ and
    $\alpha_{\this}$ use
    $\init(p_{\mathrm{def}})$
    where $p_{\mathrm{def}}$ always originates
    from an $\inherits$ entry and is therefore
  non-root.}
  \notag \\
  &\quad{}\cup\;
  \{\; (p_{\mathrm{site}},\; p_{\mathrm{override}})
    ...
    &\qquad
    (p_{\mathrm{site}},\; p_{\mathrm{override}})
    \in R_{\supers(p_{\mathrm{base}})}
  \;\}
\end{align}
The second batch of sub-queries depends on
the result~$R_{\bases(p)}$ of the first batch,
making $F_{\supers}$ a dependent polynomial
(container) functor~\cite{abbott2005containers,hancock2000interactive}
rather than a simple polynomial.
Equivalently, the closed-form $\bases^*$
in~(\ref{eq:supers}) is the reflexive-transitive
closure of~$\bases$---itself a hylomorphism
(coalgebra~$= \bases$, algebra~$= \cup$)
embedded via the cross-index recursion
$\supers \to \bases \to \supers$.

\noindent\textbf{(3)\;$\overrides$.}
\begin{align}
  \alpha_{\overrides}(p)
  &\;=\;
  \begin{cases}
    \emptyset & \text{if } p = () \\[4pt]
    \{\,\supers(\init(p))\,\}
    & \text{if } p \neq ()
  \end{cases}
  \\
  \varphi_{\overrides}(p,\; R_{\supers(\init(p))})
  &\;=\;
  \begin{cases}
    \{p\} & \text{if } p = () \\[4pt]
    \{p\}
    \;\cup\;
    \{\; p_{\mathrm{branch}} \snoc \last(p) \;|\; \\
      \qquad
      (\_,\; p_{\mathrm{branch}}) \in R_{\supers(\init(p))}, \\
      \qquad
...
\begin{align}
  ...
  \{\,
    ...
    \notag \\
    &\qquad\;|\;
    p_{\mathrm{override}} \in R_{\overrides(p)}, \notag \\
    &\qquad\phantom{\;|\;}
    (n,\; \ell_{\mathrm{down},*})
    \in \inherits(p_{\mathrm{override}})
  \,\}
  ...
  \;\}
\end{align}
Like $F_{\supers}$, the functor $F_{\bases}$ is a
dependent polynomial:
the $\resolve$-positions depend on the
result~$R_{\overrides(p)}$.

\noindent\textbf{(5)\;$\resolve$.}
\begin{align}
  \alpha_{\resolve}(p_{\mathrm{site}},\; p_{\mathrm{def}},\;
  n,\; \ell_{\mathrm{down},*})
  &\;=\;
  \{\,
    \this(\{p_{\mathrm{site}}\},\; \init(p_{\mathrm{def}}),\; n)
  ...
  \\
  \varphi_{\resolve}(
    p_{\mathrm{site}},\; p_{\mathrm{def}},\;
    n,\; \ell_{\mathrm{down},*}, \notag \\
    \quad
    R_{\this(\{p_{\mathrm{site}}\},\,
    \init(p_{\mathrm{def}}),\,n)}
  ) &\;=\;
  \{\;
    p_{\mathrm{current}}
    \snoc \ell_{\mathrm{down},1}
    \snoc \cdots
    \snoc \ell_{\mathrm{down},k}
    \notag \\
    &\quad\;|\;
    p_{\mathrm{current}} \in
    R_{\this(\{p_{\mathrm{site}}\},\,
...
\noindent\textbf{(6)\;$\this$.}
\begin{align}
  \alpha_{\this}(S,\; p_{\mathrm{def}},\; n)
  &\;=\;
  \begin{cases}
    \emptyset & \text{if } n = 0 \\[4pt]
  ...
  \intertext{where
    $S' = \{\,p_{\mathrm{site}} \;|\;
      p_{\mathrm{current}} \in S,\;
      (p_{\mathrm{site}},\; p_{\mathrm{override}})
      \in R_{\supers(p_{\mathrm{current}})},\;
  p_{\mathrm{override}} = p_{\mathrm{def}}\,\}$.}
  \varphi_{\this}(
    S,\; p_{\mathrm{def}},\; n, \notag \\
    \quad
    (R_{\supers(p_{\mathrm{current}})})_{
    p_{\mathrm{current}} \in S}, \notag \\
    \quad
    R_{\this(S',\,\init(p_{\mathrm{def}}),\,n-1)}
  ) &\;=\;
  \begin{cases}
    S & \text{if } n = 0 \\[4pt]
    R_{\this(S',\,
  ...
  \end{cases}
\end{align}
Like $F_{\supers}$ and $F_{\bases}$, the functor
$F_{\this}$ is a dependent polynomial:
the $\this$-position depends on the frontier~$S'$
computed from the $\supers$~results.
The self-call is structural recursion on~$n$
and terminates independently of the dependency graph.

\medskip
Because every $\alpha_i$ and $\varphi_i$ operates on paths
and sets of paths---first-order data with no function
spaces---the construction is purely set-theoretic.
The \emph{indexed functor}
$F_{\Idx} = (F_i)_{i \in \Idx}$ is the family of
container
endofunctors~\cite{abbott2005containers}
on $\prod_{j \in \Idx}\mathbf{Set}$
whose shape at each index~$i$ is determined by the
sub-queries that $\alpha_i$
emits~\cite{loh2011generic}.
For indices where the set of sub-queries depends on
earlier results ($\supers$, $\bases$, $\this$),
$F_i$ is a dependent polynomial
functor~\cite{hancock2000interactive};
for the remaining indices ($\properties$, $\overrides$,
$\resolve$) it is a simple polynomial.
The cross-index dependencies
($\supers \leftrightarrow \overrides$,
$\supers \to \bases \to \resolve \to \this \to \supers$)
are encoded in the index arguments of each~$F_i$.
The initial algebra $\mu F_{\Idx}$ consists of finite
dependency trees, and the final coalgebra $\nu F_{\Idx}$
includes the infinite trees that arise from diverging queries.

The equivalence between the decompositions above and the
original equations~(\ref{eq:properties})--(\ref{eq:this})
is constructive:
for each index~$i$, substituting the sub-query results
produced by $F_i(\hylo{\Idx}) \circ \alpha_i$ into
$\varphi_i$ recovers the corresponding equation
in Section~\ref{sec:mixin-trees}.
The reader may verify this by inlining each
$\alpha_i$ into its $\varphi_i$ and checking that the
resulting expression coincides with
equations~(\ref{eq:properties})--(\ref{eq:this}).
\end{definition}
...
$\hylo{\Idx}$~(\ref{eq:properties})--(\ref{eq:this}) on the
reachable dependency subgraph.
\end{theorem}

\begin{proof}
Termination implies that the reachable subgraph of~$G$ is
finite (finitely many vertices visited) and acyclic
(a cycle $v_0 \to \cdots \to v_0$ would require
  the value of~$v_0$ before it has been determined,
giving infinite recursion).
On this finite DAG the restriction of each~$\alpha_i$
is a well-founded (recursive)
coalgebra~\cite{taylor1999practical,capretta2006recursive},
so $\hylo{\Idx}$ has a unique solution by the general
recursion theorem for recursive
coalgebras~\cite{adamek2019well}.
Memoized evaluation computes exactly this solution,
visiting each vertex at most once.
\end{proof}

\paragraph{Divergence}
When evaluation does not terminate, the
$\hylo{\Idx}$ framework accounts for divergence
without additional apparatus.
The dependency graph either contains a cycle (circular
inheritance, giving infinite recursion) or an infinite
acyclic chain (unbounded inheritance hierarchy).
In both cases~$\alpha_i$ is non-well-founded
and the unfold produces an infinite
tree~\cite{capretta2006recursive}---an element of the
final coalgebra
$\nu F_{\Idx}$~(Definition~\ref{def:indexed-functor}),
the carrier of the terminal
$F_{\Idx}$-coalgebra~\cite{rutten2000universal}.
Convergence and divergence are therefore \emph{internal}
to the first-order framework: convergent queries yield
elements of $\mu F_{\Idx}$ (finite dependency trees),
divergent queries yield elements of
$\nu F_{\Idx} \setminus \mu F_{\Idx}$
(infinite trees).
...
(Theorem~\ref{thm:adequacy}) and the first-order
semantics theorem
...
(substituting a closed value for the only free variable),
...
$\lambda$-calculus~\cite{abramsky1993lazy}.
The prior higher-order frameworks are thereby shown to be
faithful to the first-order semantics, without needing to
define or compare absolute paths inside mixin trees.
...
($\lambda$-abstraction, application, variable), and
$\mathbf{let}$-binding.
...
\begin{enumerate}
  \item Convert every maximal $\lambda$-calculus
    subexpression (i.e.\ every subtree whose root
      and all internal nodes use only $\lambda$-calculus
    constructors) to A-normal
    form~\cite{flanagan1993essence}.
...
Felleisen's framework~\cite{felleisen1991expressive}.
...
transformation~\cite{flanagan1993essence} consists of
...
Felleisen~\cite[Definition~3.11]{felleisen1991expressive}:
...
Felleisen~\cite{felleisen1991expressive} to the pair
...
By B\"ohm tree
equivalence~\cite{abramsky1990lazy}
(Theorem~\ref{thm:full-abstraction}), for closed
...
of $\mathrm{false}$.
Note that $\mathrm{eq}$ is a \emph{named child}
...
itself.\footnote{These indices use the condensed
...
(i.e.\ $\mathrm{false}$) to the $\lambda a$ scope, so
...
\begin{enumerate}
  \item \emph{ANF conversion.}
    The A-normal form
    transformation~\cite{flanagan1993essence}
...
\section{First-Order Semantics of the ANF Lazy
\texorpdfstring{$\lambda$}{λ}-Calculus}
\label{app:lambda-fixpoint}

This appendix presents a first-order semantics of the lazy
$\lambda$-calculus in A-normal form (ANF).
...
\end{array}
\]
The grammar includes both application let-bindings and
value let-bindings (the latter bind a name to a value
without application).
Every $\lambda$-term can be mechanically converted to ANF by
...
\smallskip\noindent
A value let-binding $\mathbf{let}\; x = V \;\mathbf{in}\; M$
at path $p$:
\begin{align*}
\children(p) &= \{x,\; \mathrm{result}\},
&\refs(p) &= \varnothing.
\end{align*}
The subtree at $p \snoc \mathrm{result}$ is determined by $M$.
The subtree at $p \snoc x$ wraps the value in a
$\mathrm{result}$ scope:
\begin{align*}
\children(p \snoc x) &= \{\mathrm{result}\},
&\refs(p \snoc x) &= \varnothing.
\end{align*}
The subtree at $p \snoc x \snoc \mathrm{result}$ is
determined by $V$.
The accessor $x.\mathrm{result}$ thus yields the
translated value, matching the convention for
application let-bindings.
...
\smallskip\noindent
A $\lambda$-bound variable~$\dbi{n}$ used as an
inheritance source at an application
node~$p$ contributes
$(n,\; (\mathrm{argument}))$
to~$\refs(p)$: navigate up~$n$ scope levels,
then project $\mathrm{argument}$.
The same applies when~$\dbi{n}$ occurs as a bare
value in tail position or as an argument value,
contributing the reference to~$\refs$
...
reference resolution~(\ref{eq:resolve}), which calls
$\this(\{p_{\mathrm{site}}\},\; \init(p_{\mathrm{def}}),\; n)$.
...
equivalent under this semantics iff they have the same
B\"ohm tree (Theorem~\ref{thm:full-abstraction}).
The proofs, given in
Appendix~\ref{app:bohm-tree-proofs}, apply verbatim:
each equation above is the specialization of the
corresponding mixin tree equation
...
Single-Path Lemma (Lemma~\ref{lem:single-path})
ensures that the set-valued $\this$ function
(equation~\ref{eq:this}) degenerates to the
single-valued $\scope$
(equation~\ref{eq:lambda-scope}).
