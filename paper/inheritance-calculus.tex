...
\ifInheritanceChinese
诸如 Jsonnet~\cite{cunningham2014-jsonnet}、Hydra~\cite{yadan2023-hydra}、CUE~\cite{van-lohuizen2019-cue}、Dhall~\cite{gonzalez2017-dhall}、Kustomize~\cite{kubernetes-sig-2018-kustomize} 与 JSON Patch~\cite{bryan2013-json-patch} 这样的配置语言,都提供了通过继承或合并来组合结构化数据的机制。

在这些之中,Nix 生态系统~\cite{dolstra2010-nixos} 格外突出,无论是就其规模还是其设计的走向而言。它的软件包集合 \texttt{nixpkgs} 收录了超过 113{,}000 个软件包,%
\footnote{
  据 Repology~\cite{repology-contributors-2025-statistics},这接近 Debian 或 Ubuntu 软件包数量的三倍,而贡献者数量相近。并非每个软件包都用到这里所描述的可扩展性机制,但同样的告诫也适用于其他发行版。
}
...
但该生态系统已日益汇聚到一种对称的替代方案上:NixOS 模块系统~\cite{dolstra2010-nixos, nixos-contributors-nixos-modules},它通过对嵌套的属性集进行递归合并、配以自引用求值与延迟模块~\cite{nixos-contributors-nixos-modules} 来组合,而这种组合是可交换、幂等且可结合的。模块系统最初为系统配置而设计,如今已被用于用户环境~\cite{helgesson2017-home-manager}、macOS 配置~\cite{jordan2017-nix-darwin}、磁盘分区~\cite{lassulus2022-disko}、flake 结构~\cite{hensing2022-flake-parts}、开发者环境~\cite{kozar2022-devenv}、Kubernetes 集群~\cite{xtruder2022-kubenix, ingolfsson2024-nixidy},以及多语言软件包构建~\cite{hauer2021-dream2nix},而最后这一项正是 \texttt{nixpkgs} 那套不对称机制原本的领域。Nix 是一门带头等函数的函数式语言;然而一份典型的模块配置几乎完全由嵌套的属性集构成,函数只是偶尔作为模块系统的 DSL 语法出现,例如 \texttt{mkIf} 与 \texttt{mkMerge}。没有任何计算理论刻画了这一机制所展现的可扩展性。
\else
Configuration languages such as Jsonnet~\cite{cunningham2014-jsonnet}, Hydra~\cite{yadan2023-hydra},
CUE~\cite{van-lohuizen2019-cue}, Dhall~\cite{gonzalez2017-dhall},
Kustomize~\cite{kubernetes-sig-2018-kustomize}, and JSON
Patch~\cite{bryan2013-json-patch} all provide mechanisms for
...
113{,}000 packages,%
\footnote{
  According to Repology~\cite{repology-contributors-2025-statistics},
...
system~\cite{dolstra2010-nixos, nixos-contributors-nixos-modules}, which composes by
recursively merging nested attribute sets with self-referential
evaluation and deferred modules~\cite{nixos-contributors-nixos-modules}---commutative,
idempotent,
and associative.
Originally designed for system configuration, the module system
has been adopted for user environments~\cite{helgesson2017-home-manager},
macOS configuration~\cite{jordan2017-nix-darwin},
disk partitioning~\cite{lassulus2022-disko},
flake structure~\cite{hensing2022-flake-parts},
developer environments~\cite{kozar2022-devenv},
Kubernetes clusters~\cite{xtruder2022-kubenix, ingolfsson2024-nixidy},
and multi-language package building~\cite{hauer2021-dream2nix}---the
...
and subsequently
MIXINv2\anon[~(included as supplementary material)]{~\cite{yang2026-mixinv2}}, now implemented in Python: a declarative programming
...
\footnote{
  ...
  这是无类型演算最为自洽的
  语义。
  MIXINv2\anon[~(补充材料)]{~\cite{yang2026-mixinv2}} 更为严格:它的解析器拒绝
  前沿变为空的引用,在编译期报告一个
  解析错误。
}
...
\footnote{
  ...
  untyped calculus.
  MIXINv2\anon[~(supplementary material)]{~\cite{yang2026-mixinv2}} is stricter: its resolver rejects
...
\footnote{
  一元 Nat 的字典树可以被视为一个稀疏链表,其深度等于它所代表的数值:在字典树并中,链表中的某些 $\mathrm{Successor}$ 节点指向 $\mathrm{Zero}$(叶节点),因此链表中并非每个位置都被占用。\anon[{} 补充材料]{{} MIXINv2 源码~\cite{yang2026-mixinv2}}{} 中包含一个字典树深度为对数级的二进制自然数(BinNat)算术。
}
...
\footnote{
  ...
  The\anon[{} supplementary material]{{} MIXINv2 source~\cite{yang2026-mixinv2}}{} includes a binary natural number
...
\begin{description}
  ...
  \item[对不可扩展性的免疫]\label{item:immunity}
    ...
    在其他地方恢复第二个扩展维度需要专门的机制~\cite{liang1995-monad-transformers, lammel2003-scrap-your-boilerplate, loh2006-open-data-types, swierstra2008-data-types-a-la-carte, carette2009-finally-tagless, oliveira2012-object-algebras, kiselyov2013-extensible-effects, wu2014-effect-handlers-in-scope, kiselyov2015-freer-monads, wang2016-expression-problem-trivially, leijen2017-row-typed-algebraic-effects, poulsen2023-hefty-algebras}。
...
更多现象依赖于 FFI，而本文中的示例排除了 FFI。其中一种是函数颜色盲~\cite{nystrom2015-function-color}：同一个 MIXINv2 文件可以在同步和异步运行时下运行，这是通过继承进行依赖注入的非预期用法。补充材料\anon[]{~\cite{yang2026-mixinv2}}给出了相关示例。
\else
...
\begin{description}
  ...
  \item[Immunity to nonextensibility]\label{item:immunity}
    ...
    mechanisms~\cite{liang1995-monad-transformers, lammel2003-scrap-your-boilerplate, loh2006-open-data-types, swierstra2008-data-types-a-la-carte, carette2009-finally-tagless, oliveira2012-object-algebras, kiselyov2013-extensible-effects, wu2014-effect-handlers-in-scope, kiselyov2015-freer-monads, wang2016-expression-problem-trivially, leijen2017-row-typed-algebraic-effects, poulsen2023-hefty-algebras}.
...
supplementary material\anon[]{~\cite{yang2026-mixinv2}} gives the example.
\fi
...
\footnote{
  Nix 是一门具有一等函数的函数式语言；仅凭其属性集并不构成命名树 DSL。然而，NixOS 模块系统~\cite{nixos-contributors-nixos-modules}将用户界面限制为通过递归合并组合的嵌套属性集，而正是这种 DSL 层面的结构自然地构成了命名树。
}
...
\begin{description}
  ...
  \item[\bilingual{Priority merge}{优先级合并}]
    一个来源具有优先权~\cite{cunningham2014-jsonnet, dolstra2010-nixos}，因此 $A + B \neq B + A$，引入了顺序依赖。
...
\begin{description}
  ...
  \item[\bilingual{Single-target $\this$}{单目标 $\this$}]
    单目标 $\this$ 是一个有前途的候选方案：单路径引理~(\ref{lem:single-path})表明，对于翻译后的 $\lambda$-项，$\this$~(\ref{eq:this}) 中的前沿集 $S$ 始终恰好包含一条路径，因此 $\lambda$-演算嵌入不需要多目标解析。然而，单目标 $\this$ 因另一原因而对不可扩展性脆弱。当多条继承路径到达同一作用域时，单目标解析必须拒绝该情况或选择其中一条路径。NixOS 模块系统~\cite{nixos-contributors-nixos-modules}会引发重复声明错误（附录~\ref{app:scala-multi-target}）。这拒绝了两个独立编写的模块定义同一选项的嵌套扩展，而这种组合在求解表达问题~\cite{wadler1998-expression-problem}时会出现，如第~\ref{sec:expression-problem}~节所示。
...
\begin{center}
  \begin{tabular}{llll}
    ...
    & Jsonnet~\texttt{+}~\cite{cunningham2014-jsonnet},
    ...
    & CUE~\cite{van-lohuizen2019-cue} \\
    & \bilingual{Dynamic scope}{动态作用域}
    & \bilingual{mixin cannot replace function}{mixin 无法替代函数}
    & Jsonnet~\cite{cunningham2014-jsonnet}, union FS \\
    & \bilingual{Single-target $\this$}{单目标 $\this$}
    & \bilingual{nested extensions rejected}{嵌套扩展被拒绝}
    & NixOS modules~\cite{nixos-contributors-nixos-modules} \\
  \end{tabular}
\end{center}
...
\footnote{
  ...
  The NixOS module system~\cite{nixos-contributors-nixos-modules}, however,
...
\begin{description}
  ...
  \item[Priority merge]
    One source takes
    precedence~\cite{cunningham2014-jsonnet, dolstra2010-nixos},
    so $A + B \neq B + A$, introducing
...
\begin{description}
  \item[Early binding]
    ...
    CUE~\cite{van-lohuizen2019-cue} exemplifies this: field references are
  ...
  \item[Dynamic scope]
    ...
    Jsonnet~\cite{cunningham2014-jsonnet} follows this approach: \texttt{self}
  ...
  \item[Single-target $\this$]
    ...
    one route.
    The NixOS module system~\cite{nixos-contributors-nixos-modules} raises a
...
\ifInheritanceChinese
CUE~\cite{van-lohuizen2019-cue} 在单一格中统一了类型与值,其中以 \texttt{\&} 表示的合一是可交换、可结合且幂等的。CUE 的设计受到 Google 的 Borg 配置语言 (GCL) 的启发,后者使用了图合一。CUE 的格包含标量类型,其冲突语义规定合一不相容标量会得到 $\bot$;标量对于配置语言是否必要,还是仅凭树结构就已足够,是一个有趣的设计问题。Jsonnet~\cite{cunningham2014-jsonnet} 通过 \texttt{+} 运算符提供了具有 mixin 语义的对象继承:复合\emph{不是}可交换的,因为在标量冲突上右侧胜出,而深合并需要在每个字段上通过 \texttt{+:} 语法显式选择加入,因此深合并虽可用但并非默认行为。Dhall~\cite{gonzalez2017-dhall} 采取函数式方法:它是一个带记录的类型化 $\lambda$-演算,其中复合是记录合并算子,而非继承机制。
\else
CUE~\cite{van-lohuizen2019-cue} unifies types and values in a single lattice
...
Jsonnet~\cite{cunningham2014-jsonnet} provides object inheritance via the
...
Dhall~\cite{gonzalez2017-dhall} takes a functional approach: it is a typed
...
\ifInheritanceChinese
NixOS 模块系统~\cite{dolstra2010-nixos, nixos-contributors-nixos-modules} 通过\emph{递归合并嵌套属性集}来组合模块,而非通过线性化或方法层级的覆盖,这与先前的 mixin 和 trait 演算中区分树层级继承与方法层级复合的深合并语义相同。当两个模块定义了相同的嵌套路径时,其子树被合并而非覆盖。NixOS 模块系统集成了深合并、递归的 $\this$、模块的延迟求值,以及其上的丰富类型检查层。同一机制为 NixOS 系统配置、Home Manager~\cite{helgesson2017-home-manager}、nix-darwin~\cite{jordan2017-nix-darwin}、flake-parts~\cite{hensing2022-flake-parts} 以及数十个其他生态系统提供支持,共同管理任意复杂度的配置。一个似乎尚未被完全解决的方面是 $\this$ 在多重继承下的语义。当两个各自继承自同一延迟模块的独立模块被组合时,每次导入都会引入对共享选项自己的声明;模块系统的 \texttt{merge\-Option\-Decls} 以静态错误拒绝重复声明(附录~\ref{app:scala-multi-target})。模块系统无法表达两条路径对同一选项的贡献是均等的,即递归合并自然产生的多目标情形,正是这一情形促成了第~\ref{sec:mixin-trees} 节的观测语义。
\else
The NixOS module system~\cite{dolstra2010-nixos, nixos-contributors-nixos-modules}
...
Manager~\cite{helgesson2017-home-manager}, nix-darwin~\cite{jordan2017-nix-darwin},
flake-parts~\cite{hensing2022-flake-parts}, and dozens of other ecosystems,
...
\ifInheritanceChinese
插件系统、组件框架与依赖注入框架通过声明式继承实现了软件的可扩展性。第~\ref{sec:mixin-trees} 节所确立的继承的对称性与可结合性解释了这类系统为何有效:组件可以以任意顺序继承而不改变行为。MIXINv2\anon[~(补充材料)]{~\cite{yang2026-mixinv2}} 本身就是一个依赖注入框架:每个作用域将其依赖声明为具名槽,复合跨越作用域边界按名解析它们,从而使继承演算的模式显式化而非随意拼凑。
\else
...
behavior.
MIXINv2\anon[~(supplementary material)]{~\cite{yang2026-mixinv2}}
...
\ifInheritanceChinese
联合文件系统,如 UnionFS、OverlayFS 与 AUFS,将多个目录层次叠加以呈现统一视图。其语义与继承演算相似,表现在后面的层覆盖前面的层且所有层的文件均可访问,但其设计在具体之处有所不同:继承是不对称的,标量文件的冲突解决是不可交换的,且层与层之间不存在晚绑定或动态分派。我们将继承演算直接实现为联合文件系统\anon[~(作为补充材料附上)]{~\cite{yang2025-ratarmount}},仅凭三个原语就获得了文件查询的晚绑定语义与动态分派,无需任何额外机制。在软件包管理器、构建系统与操作系统发行版中,外部工具链将配置转换为文件树;而在这里,文件系统作为自身的配置,转换即是继承:这是一种\emph{文件系统即编译器},其中目录树既是源程序又是编译目标。
\else
...
system\anon[~(included as supplementary material)]{~\cite{yang2025-ratarmount}},
...
\ifInheritanceChinese
\anon[辅助实现]{MIXINv2~\cite{yang2026-mixinv2}}{} 是继承演算的一个可执行实现,它已经超越了本文所呈现的无类型演算:它包含编译期检查,以验证所有引用都解析到 mixin 树中的有效路径,以及一个从宿主语言引入标量值的外部函数接口 (FFI)。以下所描述的未来工作,关注的是这一实现与一门完全实用的语言之间的差距,横跨理论基础与库层面的编码两个方面。
\else
\anon[The supplementary implementation]{MIXINv2~\cite{yang2026-mixinv2}}{} is an executable implementation of inheritance-calculus
...
\ifInheritanceChinese
可执行实现 (MIXINv2) 以及本文中的所有示例以 MIT 许可证作为开源软件公开发布\anon[{} (匿名审查期间省略 URL)]{~\cite{yang2026-mixinv2}}。每个以继承演算记法呈现的示例都有对应的可执行 MIXINv2 源码,且一套测试套件复现了论文中的每个示例。案例研究 (第~\ref{sec:case-study}~节) 的布尔逻辑和一元自然数算术、附录~\ref{app:datalog-encoding}~的 Datalog 编码 (包括双跳、常量连接和多变量连接关系程序以及重复访问者自连接回归示例),以及函数颜色盲性段落 (第~\ref{sec:practical-function-color}~节) 中讨论的同步和异步运行时实例,均包含在这些源码中。第~\ref{sec:practical-union-fs}~节所讨论的 ratarmount 补丁\anon[也已公开发布 (匿名审查期间省略 URL)]{作为一个开放的拉取请求~\cite{yang2025-ratarmount}可以获取}。\anon[以上所有内容也作为辅助材料一并附上。]{}
\else
The executable implementation (MIXINv2) and all examples from
this paper are openly available as open-source software under
the MIT license\anon[{} (URL omitted for anonymous review)]{~\cite{yang2026-mixinv2}}.
...
request~\cite{yang2025-ratarmount}}.
...
\section{\bilingual{B\"ohm Tree Correspondence: Proofs}{Böhm 树对应：证明}}
\label{app:bohm-tree-proofs}

\ifInheritanceChinese
本附录证明与 $\lambda$-演算对应的继承演算子语言的两个结论
（第~\ref{sec:bohm-tree}~节）。
首先，第一次迭代近似 $T_P\!\uparrow\!1$ 关于 Böhm 树等价是完全抽象的
（定理~\ref{thm:adequacy} 与~\ref{thm:first-order-semantics}），
从而确立了第一次迭代与惰性 $\lambda$-演算相吻合。
其次，完整的 $\mathrm{lfp}(T_P)$ 语义在图~\ref{fig:calculi-matrix}
的幂集意义下是严格更精确定义的：
在 $\beta$-归约下发散的已翻译 $\lambda$-项可能收敛到确定的集合，
如第~\ref{sec:bohm-tree}~节所示。
\else
...
\subsection{\bilingual{B\"ohm Trees}{Böhm 树}}

\ifInheritanceChinese
我们简要回顾 Böhm 树的定义~\cite{barendregt1984-lambda-calculus}。
\emph{首部归约} $M \to_h M'$ 仅收缩最外层的 $\beta$-redex：
若 $M$ 的形式为 $(\lambda x.\, e)\; v\; M_1 \cdots M_k$，则
$M \to_h e[v/x]\; M_1 \cdots M_k$。
若项 $M$ 没有首部 redex，则称其处于\emph{首部范式}（HNF），
即 $M = \lambda x_1 \ldots x_m.\; y\; M_1 \cdots M_k$，其中 $y$ 是变量。
\else
...
\ifInheritanceChinese
$\lambda$-项 $M$ 的 \emph{Böhm 树} $\mathrm{BT}(M)$ 是一棵无限标记树，定义如下：
\else
...
\[
  \mathrm{BT}(M) =
  \begin{cases}
    \bot & \text{\bilingual{if}{若} } M \text{ \bilingual{has no HNF}{无 HNF}} \\[6pt]
    \lambda x_1 \ldots x_m.\;
    y\bigl(\mathrm{BT}(M_1),\; \ldots,\; \mathrm{BT}(M_k)\bigr)
    &
    \begin{aligned}[t]
      &\text{\bilingual{if}{若} } M \to^*_h \\[-2pt]
      &\lambda x_1 \ldots x_m.\; y\; M_1 \cdots M_k
    \end{aligned}
  \end{cases}
\]
\ifInheritanceChinese
Böhm 树等价 $\mathrm{BT}(M) = \mathrm{BT}(N)$ 是惰性 $\lambda$-演算的标准观测等价~\cite{abramsky1993-full-abstraction-lazy-lambda, abramsky1995-games-full-abstraction-lazy-lambda}。
\else
...
\subsection{\bilingual{Path Encoding}{路径编码}}

\ifInheritanceChinese
我们定义 Böhm 树中的位置与 mixin 树中的路径之间的对应关系。
当翻译 $\mathcal{T}$ 作用于一个 ANF $\lambda$-项时，所得 mixin 树具有特定形状：
\else
...
\fi
\begin{itemize}
  \item \ifInheritanceChinese
    抽象 $\lambda x.\, M$ 翻译为一条具有自有属性
    $\{\mathrm{argument}, \mathrm{result}\}$ 的记录，称之为\emph{抽象形状}。
    \else
    An abstraction $\lambda x.\, M$ translates to a record
    with own properties $\{\mathrm{argument}, \mathrm{result}\}$,
    which is referred to as the \emph{abstraction shape}.
    \fi
  \item \ifInheritanceChinese
    $\mathbf{let}$-绑定
    $\mathbf{let}\; x = V_1\; V_2 \;\mathbf{in}\; M$ 翻译为一条具有自有属性
    $\{x, \mathrm{result}\}$ 的记录，其中 $x$ 存放封装的应用
    $\{\mathcal{T}(V_1),\; \mathrm{argument} \mapsto \mathcal{T}(V_2)\}$。
    \else
    A $\mathbf{let}$-binding
    ...
    \fi
  \item \ifInheritanceChinese
    尾调用 $V_1\; V_2$ 翻译为一条具有自有属性
    $\{\mathrm{tailCall}, \mathrm{result}\}$ 的记录，其中
    $\mathrm{tailCall}$ 存放封装的应用，而 $\mathrm{result}$ 投影
    $\mathrm{tailCall}.\mathrm{result}$。
    \else
    A tail call $V_1\; V_2$ translates to a record with own
    ...
    \fi
  \item \ifInheritanceChinese
    de~Bruijn 指标为 $n$ 的变量 $x$ 翻译为索引引用 $\dbi{n}.\mathrm{argument}$。
    \else
    A variable $x$ with de~Bruijn index $n$ translates to
...
\noindent
\ifInheritanceChinese
Böhm 树中的每个位置都是从根出发的一系列导航步骤。
在 Böhm 树中，节点 $\lambda x_1 \ldots x_m.\; y\; M_1 \cdots M_k$ 处
可能的步骤为：通过剥去一层抽象进入函数体，以及进入首变量的第 $i$ 个参数 $M_i$。
在 mixin 树中，这些步骤分别对应于跟随 $\mathrm{result}$ 标签（进入函数体）
和在 $\this$ 解析之后跟随 $\mathrm{argument}$ 标签（进入参数位置）。
\else
...
\ifInheritanceChinese
与其在不同 mixin 树之间比较绝对路径（这对内部结构敏感），
我们只观测通过 $\mathrm{result}$ 投影可访问的\emph{收敛}行为
（定义~\ref{def:convergence}）。
每个跟随 $\mathrm{result}$ 的步骤对应 $\lambda$-演算中的一次首部归约步骤：
尾调用的 $\mathrm{result}$ 透过封装的应用进行投影，而 $\mathbf{let}$-绑定的
$\mathrm{result}$ 进入延续。
末尾的抽象形状表明已到达首部范式。
\else
...
\subsection{\bilingual{Single-Path Lemma}{单路径引理}}

\ifInheritanceChinese
$\this$ 函数（等式~\ref{eq:this}）为多目标 $\this$ 解析的一般情形而设计。
对于已翻译的 $\lambda$-项，我们证明它退化为单目标解析。
\else
...
\begin{lemma}[\bilingual{Single-Path}{单路径}]\label{lem:single-path}
  \ifInheritanceChinese
  对于任意闭合 ANF $\lambda$-项 $M$，在求值 $\mathcal{T}(M)$ 过程中每次调用
  $\this(S,\; p_{\mathrm{def}},\; n)$ 时，前沿集合 $S$ 恰好包含一条路径。
  \else
...
\begin{proof}
  \ifInheritanceChinese
  对 $\mathcal{T}$ 的翻译规则进行情形分析。
  由于 $\mathcal{T}$ 是合成的，为每条规则建立的结构不变量传播到所有被继承的子树：
  mixin 树中出现的每棵子树本身均是某个子项在 $\mathcal{T}$ 下的像，
  因此该不变量在运行时合成过程中始终成立。
  \else
  ...
  \paragraph{\bilingual{Case $\mathcal{T}(\lambda x.\, M)
    = \{\mathrm{argument} \mapsto \{\},\;
  \mathrm{result} \mapsto \mathcal{T}(M)\}$}{情形 $\mathcal{T}(\lambda x.\, M)
    = \{\mathrm{argument} \mapsto \{\},\;
  \mathrm{result} \mapsto \mathcal{T}(M)\}$}}
  \ifInheritanceChinese
  这是一个记录字面量，具有 $\defines = \{\mathrm{argument}, \mathrm{result}\}$
  且无继承来源，故 $\inherits = \varnothing$。
  在根路径 $p$ 处，由于不存在引入额外分支的继承来源，
  $\overrides(p) = \{p\}$（单一路径）。
  在 $\mathcal{T}(M)$ 内部，对 $x$ 的任何引用均为
  $\dbi{n}.\mathrm{argument}$，其中 $n$ 是指向该作用域层的 de~Bruijn 指标。
  由于 $\mathcal{T}(\lambda x.\, M)$ 恰好引入一个作用域层，
  该层恰有一个 mixin，故 $\this$ 在 $\supers$ 中恰好找到一个匹配对
  $(p_{\mathrm{site}},\; p_{\mathrm{override}})$。
  \else
  ...
  \paragraph{\bilingual{Case
    $\mathcal{T}(\mathbf{let}\; x = V_1\; V_2
    \;\mathbf{in}\; M)
    = \{x \mapsto \{\mathcal{T}(V_1),\;
      \mathrm{argument} \mapsto \mathcal{T}(V_2)\},\;
  \mathrm{result} \mapsto \mathcal{T}(M)\}$}{情形
    $\mathcal{T}(\mathbf{let}\; x = V_1\; V_2
    \;\mathbf{in}\; M)
    = \{x \mapsto \{\mathcal{T}(V_1),\;
      \mathrm{argument} \mapsto \mathcal{T}(V_2)\},\;
  \mathrm{result} \mapsto \mathcal{T}(M)\}$}}
  \ifInheritanceChinese
  外层记录具有自有属性 $\{x, \mathrm{result}\}$ 且外层无继承来源
  （$\inherits = \varnothing$），故 $\overrides(\text{root}) = \{\text{root}\}$。
  在 $x$ 子树内部，继承
  $\{\mathcal{T}(V_1),\; \mathrm{argument} \mapsto \mathcal{T}(V_2)\}$
  恰好有一个继承来源（$\mathcal{T}(V_1)$），从而恰好创建一个 $\bases$ 条目。
  对 $V_1$ 归纳，单路径性质在 $\mathcal{T}(V_1)$ 内部成立。
  在 $\mathcal{T}(M)$ 内部，归纳假设直接适用。
  \else
  ...
  \ifInheritanceChinese
  外层记录具有自有属性 $\{x, \mathrm{result}\}$ 且外层无继承来源，
  故 $\overrides(\text{root}) = \{\text{root}\}$。
  $x$ 子树本身是一个具有自有属性 $\{\mathrm{result}\}$ 且无继承来源的记录字面量。
  因此，进入绑定值或进入函数体均不会在 $\supers$ 中引入分支。
  对 $\mathcal{T}(V)$ 和 $\mathcal{T}(M)$ 归纳，单路径性质均成立。
  \else
  The outer record has own properties $\{x, \mathrm{result}\}$
  ...
  \paragraph{\bilingual{Case $\mathcal{T}(V_1\; V_2) =
    \{\mathrm{tailCall} \mapsto
      \{\mathcal{T}(V_1),\; \mathrm{argument} \mapsto
      \mathcal{T}(V_2)\},\;
      \mathrm{result} \mapsto
  \{\mathrm{tailCall}.\mathrm{result}\}\}$}{情形 $\mathcal{T}(V_1\; V_2) =
    \{\mathrm{tailCall} \mapsto
      \{\mathcal{T}(V_1),\; \mathrm{argument} \mapsto
      \mathcal{T}(V_2)\},\;
      \mathrm{result} \mapsto
  \{\mathrm{tailCall}.\mathrm{result}\}\}$}}
  \ifInheritanceChinese
  与 $\mathbf{let}$-绑定情形完全相同：外层记录具有自有属性
  $\{\mathrm{tailCall}, \mathrm{result}\}$ 且外层无继承来源，
  而 $\mathrm{tailCall}$ 子树恰好包含一个继承来源。
  \else
  ...
  \paragraph{\bilingual{Case $\mathcal{T}(x)
  = \dbi{n}.\mathrm{argument}$}{情形 $\mathcal{T}(x)
  = \dbi{n}.\mathrm{argument}$}}
  \ifInheritanceChinese
  这是一个引用，而非记录。
  $\this$ 函数从包围作用域向上走 $n$ 步。
  根据上述合成性论证，遍历过程中遇到的每个作用域层，
  包括通过引用解析到达的被继承子树，都是某个子项在 $\mathcal{T}$ 下的像，
  因此 $\this$ 的每一步在 $\supers$ 中恰好遇到一个匹配对。
  \else
  ...
  \ifInheritanceChinese
  关键的结构不变量如下。
  对于 $\mathcal{T}$ 生成的每棵子树，其根要么没有继承来源，要么恰有一个继承来源。
  此外，唯一拥有继承来源的根是由应用 $\mathbf{let}$-绑定规则和尾调用规则
  引入的应用包装器，而在两种情形中，该来源均是唯一的，并封装在一个专属子项
  （$x$ 或 $\mathrm{tailCall}$）之下，从不出现在外围作用域层。
  因此，$\this$ 遍历的每个作用域层在 $\supers$ 中最多贡献一个匹配对，
  前沿集合在整个求值过程中始终保持为单元素集。
  \else
...
\subsection{\bilingual{Substitution Lemma}{替换引理}}

\ifInheritanceChinese
翻译的核心机制是：带有 $\mathrm{argument} \mapsto \mathcal{T}(V)$ 的继承
扮演替换的角色。我们将此精确化。
\else
...
\begin{lemma}[\bilingual{Substitution}{替换}]\label{lem:substitution}
  \ifInheritanceChinese
  设 $M$ 为 $x$ 可能自由出现的 ANF 项，$V$ 为闭合值。%
  \footnote{
    该证明使用单路径引理（引理~\ref{lem:single-path}），
    该引理要求被继承树出现在闭合项内。
    只要 $\lambda x.\, M$ 应用于 $V$ 是某个闭合 ANF 程序的子项，此条件即成立。
  }
  定义\emph{被继承树}
  $C = \{\mathcal{T}(\lambda x.\, M),\;
  \mathrm{argument} \mapsto \mathcal{T}(V)\}$。
  则对于 $C \snoc \mathrm{result}$ 下的每条路径 $p$ 和每个标签 $\ell$：
  \[
    \ell \in \properties(p)
...
\begin{proof}
  ...
  \paragraph{\bilingual{Base case: $M = x$}{基础情形：$M = x$}}
  \ifInheritanceChinese
  则 $\mathcal{T}(x) =
  \dbi{n}.\mathrm{argument}$（$n$ 为 $x$ 的 de~Bruijn 指标），
  且 $C \snoc \mathrm{result}$ 包含该引用。
  在被继承树 $C$ 中，$\resolve$ 和 $\this$ 从引用的包围作用域向上走 $n$ 步，
  到达绑定 $\lambda$，那里的继承
  $\{\mathcal{T}(\lambda x.\, x),\;
  \mathrm{argument} \mapsto \mathcal{T}(V)\}$
  继承到 $\mathrm{argument}$ 槽，提供 $\mathcal{T}(V)$。
  由于 $C$ 是 $\mathcal{T}\bigl((\lambda x.\, M)\; V\bigr)$ 的
  $\mathrm{tailCall}$ 子树，而该项是闭合 ANF 项，
  引理~\ref{lem:single-path} 适用，$\this$ 恰好找到一条路径，
  故 $\resolve$ 返回 $\mathcal{T}(V)$ 的子树。
  由于 $M[V/x] = V$，有 $\mathcal{T}(M[V/x]) = \mathcal{T}(V)$，属性吻合。
  \else
  Then $\mathcal{T}(x) =
  \dbi{n}.\mathrm{argument}$ (where $n$ is the de~Bruijn index
  of $x$), and
  ...
  \paragraph{\bilingual{Base case: $M = y$ where $y \neq x$ and $y$ is
  $\lambda$-bound}{基础情形：$M = y$，其中 $y \neq x$ 且 $y$ 由 $\lambda$ 绑定}}
  \ifInheritanceChinese
  则 $\mathcal{T}(y) = \dbi{m}.\mathrm{argument}$，$m$ 为 $y$ 的 de~Bruijn 指标。
  这会向上走到绑定 $y$ 的作用域，该作用域不同于绑定 $x$ 的作用域，
  故在 $x$ 绑定作用域处带有 $\mathrm{argument} \mapsto \mathcal{T}(V)$ 的继承无效。
  由于 $M[V/x] = y$，属性完全相同。
  \else
  ...
  \paragraph{\bilingual{Base case: $M = y$ where $y \neq x$ and $y$ is
  let-bound}{基础情形：$M = y$，其中 $y \neq x$ 且 $y$ 由 let 绑定}}
  \ifInheritanceChinese
  则 $\mathcal{T}(y) = y.\mathrm{result}$。
  这是对包围记录中兄弟属性 $y$ 的词法引用，故被继承树 $C$ 不改变其解析。
  由于 $M[V/x] = y$，属性完全相同。
  \else
  ...
  \paragraph{\bilingual{Inductive case: $M = \lambda y.\, M'$}{归纳情形：$M = \lambda y.\, M'$}}
  \ifInheritanceChinese
  则 $\mathcal{T}(M) =
  \{\mathrm{argument} \mapsto \{\},\;
  \mathrm{result} \mapsto \mathcal{T}(M')\}$。
  $C \snoc \mathrm{result}$ 处的子树是另一棵抽象形状的记录。
  $\mathcal{T}(M')$ 内部对 $x$ 的引用通过 $\this$ 以完全相同的方式解析
  （多一个来自内层 $\lambda$ 的作用域层），
  且对 $M'$ 归纳，$C \snoc \mathrm{result} \snoc \mathrm{result}$ 下的属性
  与 $\mathcal{T}(M'[V/x])$ 的属性吻合。
  由于 $(\lambda y.\, M')[V/x] = \lambda y.\, (M'[V/x])$（假设 $y$ 是新鲜的），
  结论成立。
  \else
  Then $\mathcal{T}(M) =
  \{\mathrm{argument} \mapsto \{\},\;
  ...
  \paragraph{\bilingual{Inductive case:
    $M = \mathbf{let}\; z = V_1\; V_2$
  $\mathbf{in}\; M'$}{归纳情形：$M = \mathbf{let}\; z = V_1\; V_2\; \mathbf{in}\; M'$}}
  \ifInheritanceChinese
  则
  \[
    \mathcal{T}(M) =
  ...
  \;\mathbf{in}\; M'[V/x]$。
  对 $V_1$、$V_2$ 和 $M'$ 归纳，每棵子树（$z$ 和 $\mathrm{result}$）下的属性吻合，
  因为每棵子树内部对 $x$ 的引用均通过相同的 $\this$ 机制解析到 $\mathcal{T}(V)$。
  \else
  Then
  \[
    \mathcal{T}(M) =
  ...
  \paragraph{\bilingual{Inductive case:
    $M = \mathbf{let}\; z = V'
  \;\mathbf{in}\; M'$}{归纳情形：$M = \mathbf{let}\; z = V'\; \mathbf{in}\; M'$}}
  \ifInheritanceChinese
  则
  \[
    \mathcal{T}(M) =
    ...
    \mathrm{result} \mapsto \mathcal{T}(M')\}.
  \]
  替换分配：$M[V/x] = \mathbf{let}\; z = V'[V/x]\; \mathbf{in}\; M'[V/x]$。
  对 $V'$ 和 $M'$ 归纳，两棵子树（即 $z$ 和 $\mathrm{result}$）下的属性均吻合。
  特别地，$M'$ 中对 $z$ 的出现在两侧均翻译为词法引用 $z.\mathrm{result}$，
  故由值 $\mathbf{let}$-绑定引入的包装器被替换所保留。
  \else
  Then
  \[
    \mathcal{T}(M) =
  ...
  \paragraph{\bilingual{Inductive case: $M = V_1\; V_2$ (tail call)}{归纳情形：$M = V_1\; V_2$（尾调用）}}
  \ifInheritanceChinese
  与 $\mathbf{let}$-绑定情形类似，以 $\mathrm{tailCall}$ 取代 $z$。
  \else
...
\subsection{\bilingual{Convergence Preservation}{收敛保持}}

\ifInheritanceChinese
收敛准则（定义~\ref{def:convergence}）从根出发跟随 $\mathrm{result}$ 链。
该链中的每一步对应 $\lambda$-演算中的一次首部归约步骤。
我们将此精确化。
\else
...
\begin{lemma}[\bilingual{Result-step}{结果步}]\label{lem:result-step}
  \ifInheritanceChinese
  设 $(\lambda x.\, M)\; V$ 为一个尾调用，即一个 $\beta$-redex。
  则对于每条路径 $p$ 和每个标签 $\ell$：
  \[
    ...
    \text{ in } \mathcal{T}(M[V/x])
  \]
  即，在尾调用的 mixin 树中跟随一次 $\mathrm{result}$ 投影，
  所得属性与归约项的 mixin 树根处的属性相同。
  \else
...
\begin{proof}
  ...
  \]
  \ifInheritanceChinese
  根处的 $\mathrm{result}$ 标签定义为 $\mathrm{tailCall}.\mathrm{result}$，
  因此 $\text{root} \snoc \mathrm{result}$ 解析为继承
  $\mathrm{tailCall} = \{\mathcal{T}(\lambda x.\, M),\;
  \mathrm{argument} \mapsto \mathcal{T}(V)\}$ 内部的 $\mathrm{result}$ 路径。
  该继承正是引理~\ref{lem:substitution} 中的被继承树 $C$，
  而 $\mathrm{tailCall} \snoc \mathrm{result}$ 即为 $C \snoc \mathrm{result}$。
  由引理~\ref{lem:substitution}，$C \snoc \mathrm{result}$ 下的属性与
  $\mathcal{T}(M[V/x])$ 的属性吻合。
  \else
  ...
  \ifInheritanceChinese
  \emph{Let-绑定变体。}
  对于 $\mathbf{let}\; x = V_1\; V_2 \;\mathbf{in}\; M'$（其中
  $V_1 = \lambda y.\, M''$），类似的结论成立。
  翻译给出：
  \else
  ...
  \]
  \ifInheritanceChinese
  由于 $\mathrm{result} \mapsto \mathcal{T}(M')$ 是一个属性定义，
  $\text{root} \snoc \mathrm{result}$ 处的子树为 $\mathcal{T}(M')$。
  在 $\mathcal{T}(M')$ 内部，对 $x$ 的每个引用均为 $x.\mathrm{result}$，
  其解析到 $x$ 子树
  $\{\mathcal{T}(\lambda y.\, M''),\;
  \mathrm{argument} \mapsto \mathcal{T}(V_2)\}$ 内的 $\mathrm{result}$ 路径。
  这正是引理~\ref{lem:substitution} 中的被继承树 $C$，
  故 $x.\mathrm{result}$ 与 $\mathcal{T}(M''[V_2/y])$ 具有相同的属性。
  在 $\lambda$-演算中，$M = (\lambda x.\, M')(V_1\; V_2)$，首部归约项为
  $N = M'[(V_1\; V_2)/x]$。
  在 $\mathcal{T}(N)$ 中，$x$ 的每次出现均被 $(V_1\; V_2)$ 替换，
  而根据上面的尾调用变体，其结果也产生 $\mathcal{T}(M''[V_2/y])$。
  因此，$\mathcal{T}(M)$ 中 $\text{root} \snoc \mathrm{result} \snoc p$ 处的属性
  与 $\mathcal{T}(N)$ 中 $\text{root} \snoc p$ 处的属性对所有 $p$ 均吻合。
  \else
  ...
  \ifInheritanceChinese
  \emph{值 let-绑定变体。}
  对于 $\mathbf{let}\; x = V \;\mathbf{in}\; M'$，类似的结论成立。
  翻译给出：
  \else
  ...
  由于 $\mathrm{result} \mapsto \mathcal{T}(M')$ 是一个属性定义，
  $\text{root} \snoc \mathrm{result}$ 处的子树为 $\mathcal{T}(M')$。
  在 $\mathcal{T}(M')$ 内部，$x$ 的每次出现均翻译为词法引用 $x.\mathrm{result}$，
  其解析到子树 $\mathcal{T}(V)$。
  这正是归约项 $M'[V/x]$ 的翻译。
  因此，$\mathcal{T}(\mathbf{let}\; x = V \;\mathbf{in}\; M')$ 中
  $\text{root} \snoc \mathrm{result} \snoc p$ 处的属性与
  $\mathcal{T}(M'[V/x])$ 中 $\text{root} \snoc p$ 处的属性对所有 $p$ 均吻合。
  \else
  Since $\mathrm{result} \mapsto \mathcal{T}(M')$ is a property
  definition, the subtree at
  $\text{root} \snoc \mathrm{result}$ is $\mathcal{T}(M')$.
...
\begin{theorem}[\bilingual{Convergence preservation}{收敛保持}]%
  \label{thm:convergence-preservation}
  \ifInheritanceChinese
  若 $M \to_h N$（首部归约），则 $\mathcal{T}(M){\Downarrow}$ 当且仅当
...
\begin{proof}
  \ifInheritanceChinese
  ANF 中的首部归约步骤取以下三种形式之一：
  尾调用 $(\lambda x.\, M')\; V \to_h M'[V/x]$，
  应用 $\mathbf{let}$-绑定
  $\mathbf{let}\; x = V_1\; V_2 \;\mathbf{in}\; M'
  \to_h M'[(V_1\; V_2)/x]$，
  或值 $\mathbf{let}$-绑定
  $\mathbf{let}\; x = V \;\mathbf{in}\; M'
  \to_h M'[V/x]$。
  第二和第三种形式只是底层 $\lambda$-项中一次 $\beta$-步骤的 ANF 呈现。
  在所有三种情形中，引理~\ref{lem:result-step}
  （尾调用、应用 let-绑定和值 let-绑定变体）表明：
  $\mathcal{T}(M)$ 中 $\text{root} \snoc \mathrm{result}^n$ 处的属性
  与 $\mathcal{T}(N)$ 中 $\text{root} \snoc \mathrm{result}^{n-1}$ 处的属性
  对所有 $n \ge 1$ 均吻合，其中 $N$ 为首部归约项。
  因此，抽象形状出现在归约前项的深度 $n$ 处，当且仅当它出现在归约后项的深度 $n - 1$ 处。
  \else
...
\begin{remark}[\bilingual{Iteration levels}{迭代层级}]
  \label{rem:convergence-lfp}
  \ifInheritanceChinese
  第~\ref{sec:bohm-tree}~节表明，Böhm 树对应在第一次迭代 $T_P\!\uparrow\!1$
  处成立，且 $\mathrm{lfp}(T_P)$ 在图~\ref{fig:calculi-matrix}
  的幂集意义下是严格更精确定义的。
  我们在此精确化该联系。
  tabling 求值~\cite{tamaki1986-tabled-resolution}
  通过迭代计算 $\mathrm{lfp}(T_P)$：
  $T_P\!\uparrow\!0 = \bot$，
  $T_P\!\uparrow\!1 = T_P(\bot)$，
  $T_P\!\uparrow\!2 = T_P(T_P(\bot))$，
  \ldots\ 直至稳定（定理~\ref{thm:tabling-soundness}）。
  \else
  ...
  \begin{description}
    \item[$T_P\!\uparrow\!1$ \bilingual{corresponds to
      $\beta$-reduction}{对应于 $\beta$-归约}.]
      \ifInheritanceChinese
      $T_P\!\uparrow\!1$ 对每个方程求值一次，
      重入调用返回 $\bot$（来自 $T_P\!\uparrow\!0$ 的值）。
      结果步引理（引理~\ref{lem:result-step}）在 $\mathcal{T}(M)$
      和 $\mathcal{T}(N)$ 的方程组之间建立了一个结构同构，
      偏移一个 $\mathrm{result}$ 步骤。
      该同构是纯结构性的，对任意次迭代均成立。
      定理~\ref{thm:adequacy} 和~\ref{thm:first-order-semantics}
      在该层级处得到证明。
      \else
  ...
  \paragraph{\bilingual{Observation granularity}{观测粒度}}%
  \label{rem:set-vs-domain}
  \ifInheritanceChinese
  $T_P\!\uparrow\!1$ 处的集值语义严格细于惰性 $\lambda$-演算的域论语义。
  在后者中，不终止的计算产生单一的无差别 $\bot$；
  在前者中，原则上可以区分 $\varnothing$（在有限时间内确定到达的空集）
  与 Kleene 迭代的不稳定（没有有限近似足够）。
  收敛准则（定义~\ref{def:convergence}）有意投影掉这一区别：
  它只观测 $\mathrm{result}$ 链是否到达抽象形状，
  将观测归约为单个比特。
  定理~\ref{thm:adequacy} 和~\ref{thm:first-order-semantics}
  在这一粗化的观测层级上陈述。
  \else
  ...
  \begin{description}
    \item[$\mathrm{lfp}(T_P)$ \bilingual{restores full precision}{恢复完整精度}.]
      \ifInheritanceChinese
      在 $\mathrm{lfp}(T_P)$ 处，集值语义的完整精度得以恢复：
      在 $T_P\!\uparrow\!1$ 时返回 $\varnothing$ 的重入调用
      现在可能收敛到非空集合，
      因此语义区分了"定义上为空"与"经过进一步迭代后收敛"，
      且 $M{\Downarrow}_\lambda$ 蕴含
      $\mathcal{T}(M){\Downarrow}_{\mathrm{lfp}}$，但反之不然。
      \else
...
\subsection{\bilingual{Adequacy}{Adequacy（充分性）}}

\begin{definition}[\bilingual{Inheritance-convergence}{继承收敛}]\label{def:convergence}
  \ifInheritanceChinese
  若存在 $n \ge 0$ 使得：
  \else
  A mixin tree $T$ \emph{converges}, written $T{\Downarrow}$,
  ...
  \]
  \ifInheritanceChinese
  在第~\ref{sec:mixin-trees}~节的语义下成立，则称 mixin 树 $T$ \emph{收敛}，
  记作 $T{\Downarrow}$。
  \else
  in the semantics of
...
\begin{theorem}[\bilingual{Adequacy}{Adequacy（充分性）}]\label{thm:adequacy}
  \ifInheritanceChinese
  闭合 ANF $\lambda$-项 $M$ 有首部范式，当且仅当 $\mathcal{T}(M){\Downarrow}$。
  \else
...
\ifInheritanceChinese
顶层的闭合 ANF 项要么是抽象（一个值），要么是计算（应用 $\mathbf{let}$-绑定、
值 $\mathbf{let}$-绑定或尾调用）。
抽象翻译为根处立即具有抽象形状的记录；
计算的根具有 $\{x, \mathrm{result}\}$ 或
$\{\mathrm{tailCall}, \mathrm{result}\}$ 的形式，
必须沿 $\mathrm{result}$ 链跟随才能找到最终值。
\else
...
\begin{proof}[\bilingual{Proof of Theorem~\ref{thm:adequacy}}{定理~\ref{thm:adequacy} 的证明}]
  \ifInheritanceChinese
  （$\Rightarrow$）
  设 $M \to^*_h \lambda x.\, M'$ 经过 $k$ 步首部归约。
  我们对 $k$ 归纳，证明 $\mathcal{T}(M)$ 在深度 $\le k$ 处收敛。
  \else
  ($\Rightarrow$)
  ...
  \paragraph{\bilingual{Base case ($k = 0$)}{基础情形（$k = 0$）}}
  \ifInheritanceChinese
  $M$ 已是抽象 $\lambda x.\, M'$。
  则 $\mathcal{T}(M) =
  \{\mathrm{argument} \mapsto \{\},\;
  \mathrm{result} \mapsto \mathcal{T}(M')\}$，
  其根具有 $\defines = \{\mathrm{argument}, \mathrm{result}\}$。
  这些标签直接在根的 $\defines$ 中，故递归求值无需进一步递归即可返回它们，
  $\mathcal{T}(M)$ 在深度 $n = 0$ 处收敛。
  \else
  ...
  \paragraph{\bilingual{Inductive step ($k \ge 1$)}{归纳步骤（$k \ge 1$）}}
  \ifInheritanceChinese
  $M$ 不是抽象，故 $M$ 是尾调用、应用 $\mathbf{let}$-绑定或值 $\mathbf{let}$-绑定之一。
  每种形式对应底层 $\lambda$-项中的一次首部 $\beta$-步骤。
  设 $N$ 为首部归约项，故
  $M \to_h N \to^{k-1}_h \lambda x.\, M'$。
  注意 $N$ 是闭合的：首部归约将一个闭合值代入只有替换目标这一个自由变量的项，
  从而保持闭合性。
  由引理~\ref{lem:result-step}（尾调用、应用 let-绑定或值 let-绑定变体），
  $\mathcal{T}(M)$ 中 $\text{root} \snoc \mathrm{result} \snoc p$ 处的属性
  与 $\mathcal{T}(N)$ 中 $\text{root} \snoc p$ 处的属性吻合。
  由归纳假设，$\mathcal{T}(N)$ 在深度 $\le k - 1$ 处收敛，
  故 $\mathcal{T}(M)$ 在深度 $\le k$ 处收敛。
  \else
  ...
  \ifInheritanceChinese
  （$\Leftarrow$）
  设 $\mathcal{T}(M){\Downarrow}$ 在深度 $n$ 处收敛。
  我们对 $n$ 归纳，证明 $M$ 有首部范式。
  \else
  ($\Leftarrow$)
  ...
  \paragraph{\bilingual{Base case ($n = 0$)}{基础情形（$n = 0$）}}
  \ifInheritanceChinese
  $\mathcal{T}(M)$ 的根具有抽象形状 $\{\mathrm{argument}, \mathrm{result}\}$。
  只有抽象规则 $\mathcal{T}(\lambda x.\, M')$ 才能产生具有自有属性
  $\mathrm{argument}$ 的根。
  （对于尾调用和 $\mathbf{let}$-绑定，根具有
    $\defines = \{\mathrm{tailCall}, \mathrm{result}\}$
    或 $\{x, \mathrm{result}\}$，而 $\mathrm{argument}$ 不能通过 $\supers$ 出现，
    因为根处 $\inherits = \varnothing$，从而
  ...
  \paragraph{\bilingual{Inductive step ($n \ge 1$)}{归纳步骤（$n \ge 1$）}}
  \ifInheritanceChinese
  抽象形状出现在深度 $n$ 处而不在深度 $0$ 处。
  由基础情形论证，$M$ 不是抽象，因此在这个闭合顶层 ANF 项的首部位置，
  $M$ 是尾调用、应用 $\mathbf{let}$-绑定或值 $\mathbf{let}$-绑定之一。
  在每种情形中，ANF 语法要求函数位置为一个值 $V_1$
  （$V ::= x \mid \lambda x.\, M$）。
  由于 $M$ 是闭合的，$V_1$ 不能是自由变量，
  故 $V_1$ 必须是 $\lambda$-抽象；首部形式因此是一个 $\beta$-redex。
  由引理~\ref{lem:result-step}（尾调用、应用 let-绑定或值 let-绑定变体），
  $\mathcal{T}(N)$ 中深度 $n - 1$ 处的属性与 $\mathcal{T}(M)$ 中深度 $n$ 处的属性吻合，
  其中 $N$ 为首部归约项。
  故 $\mathcal{T}(N)$ 在深度 $n - 1$ 处收敛。
  由于 $M$ 是闭合的，且首部归约保持闭合性（它将一个闭合值代入只有一个自由变量的项），
  $N$ 是一个闭合 ANF 项。
  由归纳假设，$N$ 有首部范式，而由 $M \to_h N$，$M$ 也有首部范式。
  \else
...
\subsection{\bilingual{First-Order Semantics of the Lazy \texorpdfstring{$\lambda$}{λ}-Calculus}{惰性 $\lambda$-演算的一阶语义}}

\ifInheritanceChinese
Adequacy（充分性）将单个项的首部范式与其继承收敛联系起来。
下面的定理将此提升为项之间的等价关系，确立翻译 $\mathcal{T}$
赋予惰性 $\lambda$-演算一个一阶语义，该语义保持并反映 Böhm 树等价。
\else
...
\begin{definition}[\bilingual{$\lambda$-contextual equivalence under $\mathcal{T}$}{$\mathcal{T}$ 下的 $\lambda$-上下文等价}]%
  \label{def:ctx-equiv}
  \ifInheritanceChinese
  对于闭合 $\lambda$-项 $M$ 和 $N$，若对每个封闭的 $\lambda$-演算上下文 $C[\cdot]$：
  \else
  ...
  \]
  \ifInheritanceChinese
  则记 $\mathcal{T}(M) \approx_\lambda \mathcal{T}(N)$。
  \fi
\end{definition}

\noindent
\ifInheritanceChinese
下标 $\lambda$ 强调量化范围是 $\lambda$-演算上下文，而非所有继承演算上下文。
该定义只观测收敛\footnote{
  收敛恰好提供一个比特的信息。
}，但对所有 $\lambda$-上下文量化使其成为细粒度的等价：
不同的上下文可以提供不同的参数并投影不同的结果，
探测项在 $\lambda$-可定义片段内行为的每个方面。
\else
...
\begin{theorem}[\bilingual{First-order semantics of the lazy $\lambda$-calculus}{惰性 $\lambda$-演算的一阶语义}]%
...
\begin{proof}[\bilingual{Proof of Theorem~\ref{thm:first-order-semantics}}{定理~\ref{thm:first-order-semantics} 的证明}]
  \ifInheritanceChinese
  翻译 $\mathcal{T}$ 是合成的：对每个 $\lambda$-演算上下文 $C[\cdot]$，
  mixin 树 $\mathcal{T}(C[M])$ 由 mixin 树 $\mathcal{T}(M)$ 和上下文的翻译所确定。
  这意味着 $\mathcal{T}$ 保留了论证所需的上下文结构。
  \else
  ...
  \ifInheritanceChinese
  （$\Leftarrow$）
  设 $\mathrm{BT}(M) = \mathrm{BT}(N)$。
  Böhm 树等价是一个同余~\cite{barendregt1984-lambda-calculus}，
  故对每个上下文 $C[\cdot]$，$\mathrm{BT}(C[M]) = \mathrm{BT}(C[N])$。
  特别地，$C[M]$ 有首部范式当且仅当 $C[N]$ 有首部范式。
  由定理~\ref{thm:adequacy}，$\mathcal{T}(C[M]){\Downarrow}$
  当且仅当 $\mathcal{T}(C[N]){\Downarrow}$。
  故 $\mathcal{T}(M) \approx_\lambda \mathcal{T}(N)$。
  \else
  ($\Leftarrow$)
  ...
  （$\Rightarrow$）
  设 $\mathrm{BT}(M) \neq \mathrm{BT}(N)$。
  Böhm 树等价与惰性 $\lambda$-演算的观测等价吻合~\cite{abramsky1993-full-abstraction-lazy-lambda, abramsky1995-games-full-abstraction-lazy-lambda}：
  存在上下文 $C[\cdot]$ 使得 $C[M]$ 收敛而 $C[N]$ 发散，或反之。
  由定理~\ref{thm:adequacy}，$\mathcal{T}(C[M]){\Downarrow}$
  且 $\mathcal{T}(C[N]){\Uparrow}$。
...
\noindent
\ifInheritanceChinese
该证明依赖两个支柱：Adequacy（充分性）（定理~\ref{thm:adequacy}），这是我们的贡献；
以及 Böhm 树等价等于惰性 $\lambda$-演算的观测等价这一经典结论~\cite{abramsky1993-full-abstraction-lazy-lambda, abramsky1995-games-full-abstraction-lazy-lambda}。
两个支柱都不需要定义或比较 mixin 树中的绝对路径。
\else
...
\section{\bilingual{Encoding Datalog in Inheritance-Calculus}{在继承演算中编码 Datalog}}
\label{app:datalog-encoding}

\ifInheritanceChinese
第~\ref{sec:emergent-phenomena}~节观察到继承演算原生产生逻辑编程的关系语义。
本附录通过给出 Datalog 到继承演算的系统性编码,使该观察精确化。
该编码不使用演算的任何扩展;它可用第~\ref{sec:syntax}~节的三个基本原语表达。
本附录的所有示例均有可执行的 MIXINv2 实现\anon[~(补充材料)]{~\cite{yang2026-mixinv2}}。
需要区分三个要素。
首先,关系以 trie 形式存储,因此元组查找通过前缀索引,而非扫描平面事实集。
其次,控制骨架采用第~\ref{sec:case-study}~节已使用的访问者模式风格编写;从函数式编程的角度看,这与 Scott 编码的情况分析所扮演的角色相同。
第三,继承并和 tabling 求值赋予最终程序关系式的最小不动点语义。
本附录按此顺序展开:先是数据表示,然后是控制流,最后是递归。
\else
...
implementations\anon[~(supplementary material)]{~\cite{yang2026-mixinv2}}.
...
\subsection{\bilingual{Relations as Tries}{关系作为 Trie}}

\ifInheritanceChinese
有限域 $D = \{a, b, c, \ldots\}$ 上的二元关系表示为 \emph{trie}:一棵路径编码元组的 mixin 树。
该 trie 由三个基础作用域构成:
$\mathrm{Relation}$（抽象基础）、
$\mathrm{Cons}$（非空节点）和 $\mathrm{Nil}$（终止符）。
每个常量 $d \in D$ 继承自 $\mathrm{Cons}$,并携带一个 $\mathrm{TailMap}$,该映射提供逐常量的子 trie:
\begin{align*}
...
\end{align*}
事实 $R(d_1, d_2)$ 编码为深度为 2 的 trie 路径,带有 $\mathrm{Nil}$ 终止符。
逐常量的子 trie 通过 $\mathrm{TailMap}.d$ 访问:
\begin{align*}
  R(a, b) \;\leadsto\quad
  R \mapsto \{a,\; \mathrm{TailMap} \mapsto \{a \mapsto
  \{b,\; \mathrm{TailMap} \mapsto \{b \mapsto \{\mathrm{Nil}\}\}\}\}\}
\end{align*}
同一关系的多个事实通过继承（trie 并）累积。
添加 $R(b, c)$:
\[
  R \mapsto \left\{
    \begin{aligned}
  ...
  \end{aligned}\right\}
\]
trie 表示的意义不仅在于它是树形编码。
它还提供后续连接所使用的索引结构:一旦选定常量 $d$,编码就直接移至 $\mathrm{TailMap}.d$ 并从匹配的子 trie 继续。
这就是为什么本附录使用 trie 这一说法,而非有限元组集的任意编码。
\else
...
\subsection{\bilingual{Infrastructure}{基础设施}}

\ifInheritanceChinese
每个常量 $d$ 携带编码所使用的四组样板定义。
这些定义的作用类似于 Haskell 中派生的类型类实例:每个常量必须提供编码的通用模式所依赖的一组固定属性。
继承演算没有内置的派生机制,因此样板需手动写出;但每个常量的样板量为 $O(1)$,不随域大小 $|D|$ 增长,因此添加新常量不需要修改任何现有作用域,从而保留了表达式问题属性。
\else
...
\begin{description}
  \item[\bilingual{Acceptance}{Acceptance}]
    \ifInheritanceChinese
    第~\ref{sec:case-study}~节的访问者模式基础设施。
    操作上,这是对当前常量进行情况分析的面向对象写法。
    每个常量 $d$ 定义一个 $\mathrm{VisitorMap}$,其中包含以 $d$ 为键的分支、一个带有逐常量结果槽 $\mathrm{Visited}$ 的 $\mathrm{Visitor}$,以及一个将匹配分支的 $\mathrm{Visited}$ 值委托给外部的 $\mathrm{Accepted}$ 属性。
    这是分派机制:给定一个可能包含多个常量的作用域,$\mathrm{Acceptance}$ 选择每个存在常量对应的分支,并收集逐常量的 $\mathrm{Visited}$ 结果。
    \else
  ...
  \item[\bilingual{WithVisitorTail}{WithVisitorTail}]
    \ifInheritanceChinese
    将每个常量 $d$ 映射到其子 trie 条目 $\mathrm{TailMap}.d$,标记为 $\mathrm{VisitorTail}$。
    这提供了连接的右端\footnote{
      在访问者模式的语境中,这充当访问者。
    }:当两个关系的 $\mathrm{VisitorMap}$ 条目组合时,$\mathrm{VisitorTail}$ 携带已连接关系中匹配常量的行数据。
    \else
  ...
  \item[\bilingual{WithVisiteeTail}{WithVisiteeTail}]
    \ifInheritanceChinese
    将每个常量 $d$ 映射到其子 trie 条目 $\mathrm{TailMap}.d$,标记为 $\mathrm{VisiteeTail}$。
    这提供了左端\footnote{
      这充当被访问者。
    }:即被迭代的关系。$\mathrm{WithVisitorTail}$ 和 $\mathrm{WithVisiteeTail}$ 共同支持值级连接,其中只有\emph{两个}关系中均存在的常量才产生输出。
    \else
    Maps each constant $d$ to its sub-trie entry
  ...
  \item[\bilingual{Replacement}{Replacement}]
    \ifInheritanceChinese
    给定一个 $\mathrm{NewTail}$ 值,产生一个 $\mathrm{Replaced}$ 作用域,该作用域继承标签 $d$,但将 $\mathrm{TailMap}.d$ 替换为 $\mathrm{NewTail}$。
    这是输出构造机制:它通过将新子 trie 替换到常量包装器中来构建输出元组。
    关键在于,$\mathrm{Replacement}$ 定义在逐常量的 $\mathrm{TailMap}$ 条目上（而非合并后的作用域上）,确保输出构造作用于单个常量而非其并集。
    \else
...
为贯穿示例,取自
\anon[\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~\cite{yang2026-mixinv2}}。
其顶层作用域按编码各阶段命名:\texttt{\_XAcceptance} 迭代第一列,\texttt{\_YAcceptance} 执行共享变量连接,\texttt{\_NilAcceptance0} 确认第一个事实的末尾,\texttt{\_ZAcceptance} 迭代输出列,\texttt{\_NilAcceptance1} 触发最终的 \texttt{Replacement} 链。
下面的示意编码应理解为该可执行 MIXINv2 文件的抽象。
若暂时忽略关系累积并将每次分派读作普通的情况分析,则控制流为:从第一个关系中选取 $X$,选取匹配的 $Y$,确认第一个元组已完整,从第二个关系匹配行中选取 $Z$,确认第二个元组已完整,并输出 $(X, Z)$。
本附录的其余部分解释这一熟悉的单值控制骨架如何通过对 trie 的访问者分派实现,并随后通过继承并提升为关系语义。
\else
...
\subsection{\bilingual{Encoding Rules}{编码规则}}

\ifInheritanceChinese
Datalog 规则 $H(\bar{X}) \mathrel{{:}\!{-}} B_1(\bar{Y}_1), \ldots, B_n(\bar{Y}_n)$ 通过在该控制骨架内嵌套三种机制来编码:

\begin{enumerate}
  \item \textbf{列迭代。}
    对每个列位置,一次 $\mathrm{Acceptance}$ 分派迭代存在的常量。
    $\mathrm{VisitorMap}$ 从 $\mathrm{WithVisiteeTail}$ 填充,因此每个匹配常量的 $\mathrm{Visitor}$ 接收 $\mathrm{VisiteeTail}$:该位置处逐常量的子 trie。
    处理每个常量的结果放入 $\mathrm{Visited}$,$\mathrm{Accepted}$ 收集所有逐常量的结果。
    （详见第~\ref{sec:encoding-variables}、\ref{sec:encoding-constants}~和~\ref{sec:encoding-multi}~节。）

  \item \textbf{连接。}
    对于体原子之间的每个共享变量,内层 $\mathrm{Acceptance}$ 的 $\mathrm{VisitorMap}$ 同时继承自 $\mathrm{WithVisitorTail}$（来自被连接关系）和 $\mathrm{WithVisiteeTail}$（来自被迭代关系）。
    常量 $d$ 仅当出现在两个关系中时才产生 $\mathrm{Visitor}$ 条目,从而实现值级连接。
    随后 $\mathrm{Visitor}$ 接收 $\mathrm{VisiteeTail}$ 和 $\mathrm{VisitorTail}$ 两者,从而可访问匹配常量在两个关系中的数据。

  \item \textbf{输出构造。}
    在最内层的 $\mathrm{Nil}$ 确认（验证每个体原子的元组已完整）内部,从最内层输出列向外链式执行 $\mathrm{Replacement}$ 操作,每步将 $\mathrm{NewTail}$ 设为前一个 $\mathrm{Replaced}$ 值,最终由 $\mathrm{Nil}$ 终止。
    最终的 $\mathrm{Replaced}$ 值赋给 $\mathrm{Visited}$。
\end{enumerate}
\else
...
\subsection{\bilingual{Variable Join}{变量连接}}
...
\anon[\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~\cite{yang2026-mixinv2}},
其作用域名称在下文括号中给出。
这里可以清楚地看到节引言中的分工:访问者分派提供控制流,trie 提供对匹配尾部的直接访问,而 $\supers$ 累积每次成功匹配产生的输出。

共享变量 $Y$ 要求在 $\alpha$ 的第二列和 $\beta$ 的第一列之间匹配常量。
编码嵌套五次 $\mathrm{Acceptance}$ 分派:从 $\alpha$ 迭代 $X$、在 $\alpha$ 与 $\beta$ 之间对 $Y$ 连接、确认 $\alpha$ 中 $Y$ 之后的 $\mathrm{Nil}$、从 $\beta$ 匹配行迭代 $Z$、确认 $Z$ 之后的 $\mathrm{Nil}$。
我们将每一层作为一个命名子作用域呈现。
\else
...
\ifInheritanceChinese
\paragraph{第~1~层: 从 \texorpdfstring{$\alpha$}{α} 迭代 \texorpdfstring{$X$}{X}}
(可执行作用域: \texttt{\_XAcceptance}。)
\else
...
\ifInheritanceChinese
\paragraph{第~2~层: 将 \texorpdfstring{$\alpha$}{α} 的 \texorpdfstring{$Y$}{Y} 列与 \texorpdfstring{$\beta$}{β} 的第一列连接}
(可执行作用域: \texttt{\_YAcceptance}。)
\else
...
\ifInheritanceChinese
$\mathrm{VisitorMap}$ 同时继承自 $\beta.\mathrm{WithVisitorTail}$ 和 $\_X.\mathrm{WithVisiteeTail}$,仅当常量 $d$ 同时出现在 $\alpha$ 的第二列（通过 $\_X$）和 $\beta$ 的第一列时才产生条目。
\else
...
\ifInheritanceChinese
\paragraph{第~3~层: 确认 \texorpdfstring{$\alpha$}{α} 中 \texorpdfstring{$Y$}{Y} 之后的 \texorpdfstring{$\mathrm{Nil}$}{Nil}}
(可执行作用域: \texttt{\_NilAcceptance0}。)
\else
...
\ifInheritanceChinese
此步骤检查 $\alpha(X, Y)$ 是一个完整的二列事实（其 trie 路径在 $Y$ 之后终止于 $\mathrm{Nil}$）。
在 $\mathrm{Nil}$ 分支内部,编码继续通过 $\mathrm{VisitorTail}$ 迭代 $\beta$ 的第二列:
\else
...
\ifInheritanceChinese
\paragraph{第~4~层: 从 \texorpdfstring{$\beta$}{β} 的匹配行迭代 \texorpdfstring{$Z$}{Z}}
(可执行作用域: \texttt{\_ZAcceptance}。)
\else
...
\ifInheritanceChinese
\paragraph{第~5~层: 确认 \texorpdfstring{$Z$}{Z} 之后的 \texorpdfstring{$\mathrm{Nil}$}{Nil} 并构造输出}
(可执行作用域: \texttt{\_NilAcceptance1},包含 \texttt{\_ZReplacement} 和 \texttt{\_XReplacement}。)
\else
...
\paragraph{\bilingual{Final result}{最终结果}}
\[
  \mathrm{composed} \mapsto \{\mathrm{xAcc}.\mathrm{Accepted}\}
\]

\paragraph{\bilingual{Semantics}{语义}}
\ifInheritanceChinese
在第~2~层,组合的 $\mathrm{VisitorMap}$ 确保常量 $d$ 仅当同时出现在 $\alpha$ 的第二列\emph{和} $\beta$ 的第一列时才产生 $\mathrm{Visitor}$。
每个匹配常量 $d$ 的 $\mathrm{Visitor}$ 独立接收 $\mathrm{VisiteeTail}$（$\alpha$ 中 $d$ 的数据）和 $\mathrm{VisitorTail}$（$\beta$ 中 $d$ 的行）。
输出构造（第~5~层）在这个逐常量的 $\mathrm{Visitor}$ 内部执行,因此 $\mathrm{Replacement}$ 作用于\emph{单个}常量的数据,而非合并后的作用域。
每个匹配常量 $d$ 在 $\mathrm{Visited}$ 中产生其自身的输出元组,这些元组由 $\mathrm{Accepted}$ 通过 trie 并（$\supers$）收集。
这产生了标准 Datalog 元组级语义:只有实际在两个关系中均匹配的常量才对输出有贡献。
对于具体实例
$\mathrm{edge} = \{(a,b),\; (b,c),\; (c,a)\}$,如编码于
\anon[\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~\cite{yang2026-mixinv2}},
第~1~层选取 $a$,第~2~层匹配 $b$,第~3~层确认 $\mathrm{edge}(a,b)$ 终止于 $\mathrm{Nil}$,第~4~层迭代 $\mathrm{edge}.\mathrm{TailMap}.b$ 并选取 $c$,第~5~层再次确认 $\mathrm{Nil}$,随后两次 $\mathrm{Replacement}$ 操作重建 $(a,c)$。
\else
...
\subsection{\bilingual{Constant Match and Navigation}{常量匹配与导航}}
\label{sec:encoding-constants}

\ifInheritanceChinese
Datalog 原子中的常量使用两种机制,均非连接。
它们比变量连接更简单,因为不需要将第二个关系的值与当前选定的常量进行匹配:
\else
...
\paragraph{\bilingual{Type existence match}{类型存在匹配}}
\ifInheritanceChinese
原子 $R(X, b)$（第二位置为常量）要求检查常量 $b$ 是否存在于 $R$ 在匹配行处的第二列。
这编码为一次 $\mathrm{Acceptance}$,其手工构造的 $\mathrm{VisitorMap}$ \emph{仅}包含 $b$ 分支（而非从 $\mathrm{WithVisitorTail}$ 组合,后者会为所有常量创建分支）:
{\small
  \begin{align*}
    \mathrm{VisitorMap} \mapsto \{b \mapsto \{\ldots\}\}
\end{align*}}
若 $b$ 存在于分派作用域中,则 $b$ 分支触发；否则 $\mathrm{Accepted}$ 不接收任何匹配分支。
\else
...
\paragraph{\bilingual{Direct navigation}{直接导航}}
\ifInheritanceChinese
原子 $R(b, Z)$（第一位置为常量）直接导航到 $b$ 的子 trie:
\[
  \mathrm{edgeB}_Z \mapsto \{R.\mathrm{TailMap}.b\}
\]
这完全绕过了访问者机制,通过访问逐常量的 $\mathrm{TailMap}$ 条目将作用域限制为仅 $b$ 的数据。
\else
...
\paragraph{\bilingual{Example}{示例}}
\ifInheritanceChinese
$\mathrm{viaB}(X, Z) \mathrel{{:}\!{-}} \mathrm{edge}(X, b),\; \mathrm{edge}(b, Z)$
结合了两种机制。
可执行的 MIXINv2 编码为
\anon[\nolinkurl{RelationalConstant.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalConstant.mixin.yaml}~\cite{yang2026-mixinv2}}。
编码迭代 $\mathrm{edge}$ 以得到 $X$（第~1~层）,然后用手工构造的 $\mathrm{VisitorMap}$ 检查 $X$ 的行中是否存在常量 $b$（第~2~层）,导航 $\mathrm{edge}.\mathrm{TailMap}.b$ 以得到 $Z$（第~3~层）,确认 $\mathrm{Nil}$（第~4~层）,并通过 $\mathrm{Replacement}$ 构造 $(X, Z)$。
\else
$\mathrm{viaB}(X, Z) \mathrel{{:}\!{-}} \mathrm{edge}(X, b),\; \mathrm{edge}(b, Z)$
...
\subsection{\bilingual{Multi-Body Rules}{多体规则}}
\label{sec:encoding-multi}

\ifInheritanceChinese
含 $n > 2$ 个体原子的规则链接额外的连接层。
每个共享变量在前一层的 $\mathrm{Visitor}$ 内引入一个组合的 $\mathrm{WithVisitorTail}$/$\mathrm{WithVisiteeTail}$ 连接,输出构造放置在最内层的 $\mathrm{Nil}$ 确认内部。
因此,二元规则的编码在结构上已是完整故事:更大的规则体只是增加同一模式的更多嵌套副本。
\else
...
\paragraph{\bilingual{Three-body rule}{三体规则}}
\ifInheritanceChinese
$\mathrm{chain}(X, W) \mathrel{{:}\!{-}} \alpha(X, Y),\; \beta(Y, Z),\; \gamma(Z, W)$
需要两次连接:对 $Y$（在 $\alpha$ 和 $\beta$ 之间）以及对 $Z$（在 $\beta$ 和 $\gamma$ 之间）。
结构为:从 $\alpha$ 迭代 $X$、对 $Y$ 连接（获取 $\alpha$ 和 $\beta$ 两者的数据）、确认 $\alpha$ 中 $Y$ 之后的 $\mathrm{Nil}$,然后在 $\mathrm{Nil}$ 分支内将 $\beta$ 的 $Z$ 列与 $\gamma$ 的第一列连接、从 $\gamma$ 匹配行迭代 $W$、确认 $\mathrm{Nil}$,并构造 $(X, W)$。
$Z$ 连接嵌套在 $Y$ 连接的 $\mathrm{Nil}$ 确认内部,确保 $\beta$ 中匹配的 $Y$ 常量的数据（通过 $\mathrm{VisitorTail}$ 可得）被用于迭代 $\beta$ 的第二列。
\else
$\mathrm{chain}(X, W) \mathrel{{:}\!{-}} \alpha(X, Y),\; \beta(Y, Z),\; \gamma(Z, W)$
...
\paragraph{\bilingual{Mixed constants and variables}{混合常量与变量}}
\ifInheritanceChinese
$\mathrm{constJoin}(X, W) \mathrel{{:}\!{-}} \mathrm{edge}(X, Y),\; \mathrm{edge}(Y, b),\; \mathrm{edge}(b, W)$
结合了对 $Y$ 的变量连接、对 $b$ 的常量匹配（手工构造的仅含 $b$ 条目的 $\mathrm{VisitorMap}$）以及通过 $\mathrm{edge}.\mathrm{TailMap}.b$ 对 $W$ 的直接导航。
对应的可执行 MIXINv2 文件为
\anon[\nolinkurl{RelationalConstantJoin.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalConstantJoin.mixin.yaml}~\cite{yang2026-mixinv2}}。
\else
$\mathrm{constJoin}(X, W) \mathrel{{:}\!{-}} \mathrm{edge}(X, Y),\; \mathrm{edge}(Y, b),\; \mathrm{edge}(b, W)$
...
\subsection{\bilingual{Multi-Variable Join}{多变量连接}}

\ifInheritanceChinese
当一条规则在两个体原子之间共享\emph{多个}变量时,每个共享变量需要其自身的 $\mathrm{Acceptance}$ 检查。
这些检查链式排列:外层 $\mathrm{Acceptance}$ 使用组合的 $\mathrm{WithVisitorTail}$/$\mathrm{WithVisiteeTail}$ 对一对列连接,其 $\mathrm{Visitor}$ 内含对另一对列进行连接的内层 $\mathrm{Acceptance}$。
\else
...
共享 $X$ 和 $Y$ 两者。
可执行的 MIXINv2 编码为
\anon[\nolinkurl{RelationalMultiVariableJoin.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalMultiVariableJoin.mixin.yaml}~\cite{yang2026-mixinv2}}。
外层 $\mathrm{Acceptance}$ 使用组合的 $\mathrm{WithVisitorTail}$/$\mathrm{WithVisiteeTail}$ 连接 $r$ 和 $s$ 的第一列（变量 $X$）。
在外层 $\mathrm{Visitor}$ 内部,内层 $\mathrm{Acceptance}$ 类似地连接 $r$ 和 $s$ 的第二列（变量 $Y$）。
两次连接均须匹配才能产生输出,逐常量的 $\mathrm{Visited}$ 模式确保只有两个关系中均存在的常量才有贡献。
\else
$\mathrm{multiJoin}(X, Y) \mathrel{{:}\!{-}} r(X, Y),\; s(X, Y)$
shares both $X$ and $Y$.
The executable MIXINv2 encoding is
...
\subsection{\bilingual{Recursive Rules}{递归规则}}
\label{sec:recursive-rules}

\ifInheritanceChinese
递归 Datalog 规则（例如传递闭包
$\mathrm{path}(X, Y) \mathrel{{:}\!{-}} \mathrm{edge}(X, Z),\; \mathrm{path}(Z, Y)$）的编码与非递归规则完全相同:头部关系 $\mathrm{path}$ 既作为已定义的作用域出现,也作为规则体中的引用出现（为 $Z$ 连接提供 $\mathrm{WithVisitorTail}$）。
这不应被理解为单值惰性程序中普通的 $\this$。
在操作上,tabling 求值（第~\ref{sec:mixin-trees}~节,附录~\ref{app:well-definedness}）计算 $\mathrm{lfp}(T_P)$:对 $\mathrm{path}$ 的每次查询通过递归规则体触发进一步求值,trie 并（$\supers$）累积所有可达元组。
这与 $T_P$ 算子~\cite{vanemden1976-predicate-logic-semantics}的\emph{朴素求值}~\cite{bancilhon1986-recursive-query-strategies}完全对应:每次迭代将所有规则应用于当前事实集并添加新导出的事实。
由于 Datalog 编码的关系在每次查询中只有有限多个常量,查询的答案域是有限的,收敛性由有限格上的升链条件保证。
\emph{半朴素求值}~\cite{bancilhon1986-recursive-query-strategies}（限制每次迭代仅使用至少一个新导出事实的规则）是一种自然的优化,留待未来工作。
\else
...
\paragraph{\bilingual{The recursive rule as monadic pseudocode}{递归规则作为单子伪代码}}
\ifInheritanceChinese
上述递归规则大致对应以下伪代码（类 Haskell 语法,带有 \texttt{RebindableSyntax} 和 \texttt{OverloadedLists} 语义,其中 do 记法脱糖为基于 \texttt{Set} 的绑定,列表字面量表示集合）:
\else
...
\noindent
\ifInheritanceChinese
伪代码的每一行在继承演算编码（第~\ref{sec:encoding-variables}~节）中都有直接对应:
\else
...
\begin{description}
  \item[\texttt{edge}]
    \ifInheritanceChinese 对应基础事实 trie;\else corresponds to the base-fact trie;\fi
  \item[\texttt{path = edge <> do \ldots}]
    \ifInheritanceChinese
    对应 $\mathrm{path}$ 继承 $\mathrm{edge}$（基础事实）和递归规则体（通过 $\supers$ 得到的导出事实）;
    \else
  ...
  \item[\texttt{(x, y0) <- edge}]
    \ifInheritanceChinese
    对应第~1~层（$\mathrm{xAcc}$）,从 $\mathrm{edge}$ 迭代 $X$;
    \else
  ...
  \item[\texttt{(y1, z) <- path}]
    \ifInheritanceChinese
    对应第~2~层（$\mathrm{yAcc}$）,与 $\mathrm{path}$ 对 $Y$ 进行连接,以及第~4~层（$\mathrm{zAcc}$）,迭代 $Z$;
    \else
  ...
  \item[\texttt{guard (y0 == y1)}]
    \ifInheritanceChinese
    对应第~2~层中 $\mathrm{WithVisitorTail} \cap \mathrm{WithVisiteeTail}$ 的交集:常量仅当出现在两个关系中时才产生 $\mathrm{Visitor}$ 条目;
    \else
  ...
  \item[\texttt{return (x, z)}]
    \ifInheritanceChinese
    对应第~5~层的 $\mathrm{Replacement}$ 链,构造输出元组 $(X, Z)$。
    \else
...
\end{description}
\ifInheritanceChinese
唯一的区别是求值策略:Haskell 惰性求值 \texttt{path}（发散）,而继承演算通过 tabling $T_P$ 迭代求值（收敛到 $\mathrm{lfp}$）。

然而,该单子程序并不收敛,因为 \texttt{path} 是递归定义的。%
\footnote{
  启用 \texttt{RecursiveDo} 扩展并无帮助:\texttt{mfix} 通过惰性在\emph{元素}级别打结,而传递闭包方程需要在\emph{容器}级别求不动点,即找到满足 $S = F(S)$ 的集合 $S$。
}
继承演算通过 tabling 求值（第~\ref{sec:mixin-trees}~节）解决了这一问题,它通过在 mixin 树的格上迭代来计算 $T_P$ 算子的最小不动点,恰好是上述伪代码无法表达的容器级不动点。
\else
...
\subsection{\bilingual{Semantic Correspondence with Standard Datalog}{与标准 Datalog 的语义对应}}
\label{sec:datalog-semantics}

\ifInheritanceChinese
该编码产生标准 Datalog 的元组级语义。
综合前几节:trie 提供带索引的元组存储,访问者分派提供控制骨架,逐常量的 $\mathrm{Replacement}$ 将输出构造局限于每个见证。
关键机制是逐常量的 $\mathrm{Visited}$ 模式:输出构造（$\mathrm{Replacement}$）发生在每个匹配常量的 $\mathrm{Visitor}$ 作用域内部,作用于\emph{单个}常量的数据,而非合并后的作用域。
随后 $\mathrm{Accepted}$ 属性通过 trie 并（$\supers$）收集所有逐常量的 $\mathrm{Visited}$ 结果。

例如,给定 $\mathrm{edge} = \{(a,b),\; (b,c),\; (c,a)\}$（一个 3-环），两跳自连接
$\mathrm{twoHop}(X, Z) \mathrel{{:}\!{-}} \mathrm{edge}(X, Y),\; \mathrm{edge}(Y, Z)$
恰好产生 $3$ 对:$\{(a,c),\; (b,a),\; (c,b)\}$,与标准 Datalog 一致。
这是
\anon[\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~(补充材料)]{\nolinkurl{RelationalTwoHopRepeatedVisitor.mixin.yaml}~\cite{yang2026-mixinv2}}
所呈现的行为。
当 $Y$ 连接匹配到常量 $b$（同时出现在 $(a,b)$ 产生的 $\mathrm{edge}$ 第二列和 $\mathrm{edge}$ 的第一列中）时,$b$ 的 $\mathrm{Visitor}$ 接收 $\mathrm{VisitorTail} = \mathrm{edge}.\mathrm{TailMap}.b$（仅含 $c$），$\mathrm{Replacement}$ 构造单个输出元组 $(a, c)$,而非 $\{a,b,c\}$ 的全部。

这种值级精确性源于每个常量的 $\mathrm{Replacement}$ 作用于 $\mathrm{TailMap}.d$（逐常量的子 trie）,而非合并后的 trie。
同样的观点在重复访问者自连接回归示例\anon[~(补充材料)]{~\cite{yang2026-mixinv2}}中也可见:重复的访问者分派阻止不同常量的见证被错误合并。
尽管 $\bases$（方程~\ref{eq:bases}）在基本层仍计算叉积,该编码在任何合并发生之前将计算路由通过逐常量的 $\mathrm{TailMap}$ 条目,从而达到与标准 Datalog 元组级求值相同的效果。
\else
...
the repeated-visitor self-join regression example\anon[~(supplementary material)]{~\cite{yang2026-mixinv2}},
...
\section{\bilingual{Evaluation Trace of Multi-Target \texorpdfstring{$\this$}{this} Resolution}{多目标 \texorpdfstring{$\this$}{this} 解析的求值轨迹}}
\label{app:evaluation-trace}

\ifInheritanceChinese
本附录给出附录~\ref{app:scala-multi-target}~程序的逐步求值轨迹,通过第~\ref{sec:definitions}~节的六个函数（方程~\ref{eq:properties}~至~\ref{eq:this}）计算 $\properties((\text{``HasMultipleOuters''},\, \text{``outer''}))$。
$\overrides$、$\bases$、$\resolve$、$\this$ 和 $\supers$ 的每次调用均连同其参数和返回集合一起展示。
求值轨迹存在于元语言中:每个参数和结果都是一条\emph{路径},即标识树中某个位置的标签序列 $(\ell_1, \ldots, \ell_n)$（第~\ref{sec:definitions}~节），而非继承演算表达式。
为使标签与第~\ref{sec:definitions}~节的元语言变量（斜体 $p$、$\ell$、$n$、$S$）区分,我们对每个具体标签加引号,因此路径写作带括号的逗号分隔引号标签序列。
从而 $(\text{``HasMultipleOuters''}, \text{``outer''})$ 是继承演算投影 $\mathrm{HasMultipleOuters}.\mathrm{outer}$ 所标识的路径,根路径为空序列 $()$。
\else
...
\paragraph{\bilingual{Program}{程序}}
\begin{align*}
  \{ \quad \mathrm{MyOuter} &\mapsto \{
      \mathrm{MyInner} \mapsto \{
    \mathrm{outer} \mapsto \{\qualifiedthis{\mathrm{MyOuter}}\}\}\}, \\
...
\paragraph{\bilingual{AST primitives}{AST 基本原语}}
\ifInheritanceChinese
解析填充 $\defines$ 和 $\inherits$（第~\ref{sec:definitions}~节）:
\else
...
\end{align*}
\ifInheritanceChinese
所有其他 $\defines$ 和 $\inherits$ 为空。
每个 $\inherits$ 条目是引用对 $(n,\, w)$,其中 $n$ 是向上步数,$w$ 是向下投影列表（第~\ref{sec:definitions}~节）。
\else
...
\paragraph{\bilingual{Trace}{轨迹}}
\ifInheritanceChinese
我们以依赖顺序将求值轨迹呈现为语义函数调用的编号列表:某次调用出现在其所依赖的调用之后,每个条目说明它使用了哪些较早的条目、如何计算以及产生什么结果。
每个不同的调用列出一次,因此单个条目可被多个后续条目使用,这正是实现可以通过 tabling~\cite{tamaki1986-tabled-resolution}实现的复用。
参数和结果是元语言值:标签是引号字符串（如 $\text{``outer''}$），路径是标签的元组（如 $(\text{``HasMultipleOuters''}, \text{``outer''})$），根路径写作 $()$。
查询为 $\properties((\text{``HasMultipleOuters''}, \text{``outer''}))$。
\else
...
\begin{remark}
\ifInheritanceChinese
  关键在于 $\this$ 步骤（条目~\ref{trace:this-Has-MyOuter-MyInner-1}）。
  $(\text{``MyOuter''}, \text{``MyInner''}, \text{``outer''})$ 处的继承引用向上走一步,因此 $\this$ 从 $(\text{``MyOuter''}, \text{``MyInner''})$ 向上走一步,产生前沿 $\{(\text{``Object1''}), (\text{``Object2''})\}$,即合并了 $\text{``MyInner''}$ 的两个继承站点。
  这正是 Scala~3 和 NixOS 模块系统拒绝的多目标情况（附录~\ref{app:scala-multi-target}）:在那里 $\this$ 必须选择单个站点。
  之所以能到达 $(\text{``MyOuter''})$ 的属性,是因为 $(\text{``Object1''})$ 和 $(\text{``Object2''})$ 都以 $(\text{``MyOuter''})$ 为基础,因此最终的 $\supers$ 通过两条路径都包含 $(\text{``MyOuter''})$；由于属性解析通过集合并聚合,两条路径折叠为唯一答案
  $\properties((\text{``HasMultipleOuters''}, \text{``outer''}))
